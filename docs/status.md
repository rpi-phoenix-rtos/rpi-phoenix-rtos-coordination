...
1. Split the post-`go!` Pi 4 handoff boundary so the `raspi4b` lane clearly shows whether it stops inside `cmd_go()`, inside `hal_done()`, inside `hal_cpuJump()`, or only after the jump attempt.
