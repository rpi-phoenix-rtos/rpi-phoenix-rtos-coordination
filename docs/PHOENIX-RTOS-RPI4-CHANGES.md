...
| 20 | **`sync()` and `fsync()` both silently did nothing — broken at THREE levels** | libphoenix `40a1efb`, kernel `482b54c2`, ext2 `017eb3a` | **(a)** `void sync(void) { }` — an empty stub, so nothing in the system could ask for a flush. **(b)** `posix_fsync()` built its message with `hal_memset(&msg, 0, …)` and then passed the oid only in `msg.i.raw`, under a `FIXME: Replace this hack`. That FIXME was describing a **bug**: every storage driver implementing `mtSync` reads **`msg->oid.id`** (`bcm2711-emmc`, `zynq7000-sdcard`, `pc-ata`, `flashdrv`) and **none** reads `i.raw` — so `fsync()` reached the right port and asked it to sync **storage id 0**, not the file's device. And id 0 does not merely name the wrong device — it names **no** device: `storage_sync()` gates the flush on `IS_BLOCK_DEVICE_ID(id)`, i.e. `(id & DEVTYPE_BLOCK) != 0`, which is **false for 0**, so control fell to the `else` and returned **`-EINVAL` unconditionally**, whatever id 0 happened to resolve to. ⚠ Scoped precisely: this bites whenever the *receiver* uses the oid, i.e. `fsync()` on a **raw block-device fd**, where it was a hard, always-on failure. A filesystem server that flushes wholesale from its own `fdata` (as `libext2_sync()` does) is unaffected by the zeroed oid — but it was the only reason the bug stayed invisible, not a reason it is harmless. **(c)** No filesystem handled `mtSync` at all, so `libext2_handler()` fell to `default: break;` and left `o.err` at the 0 the caller memset — **`fsync()` returned SUCCESS while flushing nothing**, which is worse than failing. The driver side was always ready (`.sync = sdstorage_cachedFlush` → `cache_flush()`); nothing called it. ★ Affects every target: **SQLite is ported here and calls `fsync()` for durability** — note that SQLite is hit by **(a)** and **(c)**, not (b), since it syncs regular files whose server ignores the oid. Acute on any root filesystem that cannot be unmounted. |
...
| SError handling (upstream again) | `hal/aarch64/_exceptions.S`, `cpu.c`, `spinlock.c` | SError was masked for two releases because an unmask test hit PCIe/VL805 external aborts (`esr=0xbf000002`); those came from diagnostic PCIe reads that were later removed. Since kernel `df9da09d` (2026-09-26) SError is unmasked exactly as upstream and takes upstream's route (an EL0 fault kills the process with a dump; EL1 asserts) — verified by a deliberate bad PCIe read that killed only the reading program. One deliberate deviation: a held spinlock also masks SError, so the handler (which prints under the console lock) can never run on a CPU already holding it. |
...
| ★★ Freeing a `MAP_CONTIGUOUS` buffer emptied the kernel object tree | `vm/object.c` (`d0fb0ca9`) | `vm_objectContiguous()` never inserts its object into the tree, but `vm_objectPut()` rb-removed every object on its last reference; `lib_rbRemove()` of a never-inserted (zeroed, parent NULL) node sets `tree->root = NULL`, so every last unmap of a contiguous buffer — each GPU BO free — orphaned every cached file object (duplicate objects, re-reads, and later removals able to re-point the root into freed nodes). Same code upstream. Reproduced on the host with the kernel's own `lib/rb.c`; found by the GPU new-lane E1 work. Not C1's cause (C1 fired 3/3 with the fix). |
| ★★ Clients of a dead server hung forever | `proc/msg.c`, `proc/ports.c` (merge `38ad32cf`: `ac5ee1a6`, `9ce228c7`, `ee437693`, `3adc9950`) | When a server process died, nothing walked its port's rid tree or queue: a request it had already received left the sender asleep **uninterruptibly with no timeout**, and a queued request was rejected only when a receiver next called `msgRecv` — which a dead server never does. Every client of a crashed driver wedged, and its own process could then never finish exiting. Now the owner's exit fails every received and queued request with `-EINVAL`, and rids rotate per port instead of being reused at once (a late `msgRespond` could answer the wrong request). Waking those senders would have exposed two latent use-after-frees, fixed in the same series: `proc_portDestroy` dropped the owner's reference but left the port on the owner's list, so the exit dropped it a second time; and the final `port_put` locked `p->owner` after the owner was freed. Same code upstream. Pi probe `tools/gpu-lane/portdeath/`: baseline FAIL (client hangs), fixed PASS. Not fixed: a client parked on a live server still ignores `SIGKILL` ([design](gpu-new-lane/port-death.md) §4). |
...
| SD card write | ~11× multi-block; now on **ADMA2**, all three engines within 6 % of each other | ⚠ **Corrected 2026-09-20.** This row used to say DMA writes were "gated off deliberately" because "SDMA writes silently corrupted the first block 1–2 runs in 10 on this Arasan/BCM2711 controller". Both halves were wrong: the corruption was **whole-transfer and every time**, and the cause was **ours** (see the bus-address row above), not a controller quirk. Writes now run on ADMA2 scatter-gather — the engine Linux uses on this SoC — falling back to SDMA if a descriptor list cannot be built. Measured end-to-end (`/usr/bin/dd`, 64 MiB): PIO 12.2 · SDMA 12.3 · **ADMA2 12.7–12.9 MB/s**; the card bounds all three, so the win is structural (no 512 KiB request cap, no contiguous buffer), not a rate. ⓘ The older "17.4 MB/s" was a raw in-driver figure and is not comparable with these end-to-end ones. |
