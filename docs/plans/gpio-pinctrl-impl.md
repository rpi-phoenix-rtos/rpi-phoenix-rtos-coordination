...
Cross-references:
- Research: [docs/research/gpio-pinctrl.md](../research/gpio-pinctrl.md)
- Non-Linux survey: [docs/research/gpio-pinctrl-non-linux.md](../research/gpio-pinctrl-non-linux.md)
...
GPL v3 (`docs/research/gpio-pinctrl-non-linux.md` §4 and
`docs/circle-reference-review.md`). Phoenix-RTOS is BSD-3 — direct
copy is forbidden.
...
test harness (`docs/netboot-test-cycle.md`).
...
Manual tests live in `docs/pi4-first-hardware-trial.md` with photos of
the bench rig.
...
- **Depends on**: kernel IRQ subsystem reaching steady state (it
  ...
  (the kernel has reached `_hal_init` per `docs/status.md`, so the
