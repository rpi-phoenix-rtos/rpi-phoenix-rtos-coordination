...
### 4a-stkblocked. The STK attribution test is blocked on THREE separate environment bugs
...
3. **★ Environment variables do not reach an exec'd program.** With the assets fully on tmpfs
   and `stk_config.xml` present there, STK *still* reported
   `fetched from: '/usr/share/supertuxkart/data/'`. Its `getenv("SUPERTUXKART_DATADIR")` branch
   is **unconditional and first** in the resolution chain (`file_manager.cpp:179`), so falling
   through to the compile-time default proves `getenv` returned NULL — the exported variable
   never arrived. libphoenix's `execve` **does** forward `envp` to the `exec` syscall
   (`unistd/sys.c:199`), so the loss is below that: kernel-side `exec`, or `_env_init` in the
   new image. **A systemic libphoenix/kernel gap worth its own fix** — it silently defeats
   every env-configured program, not just this test.
