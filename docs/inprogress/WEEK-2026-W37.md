...
**Which also explains why the guard is silent, and what it does and does not cover.** The
target here is *object payload* (a vtable pointer inside a live allocation), not allocator
metadata, so a header check at `free()` cannot see it. The guard remains a real safety fix —
it stops `free()` turning a header overflow into a second wild write, which is what the
earlier `malloc_cmp`/`malloc_chunkSetFooter` faults were — but it is the wrong instrument for
*this* fault. Next: find the buffer whose indices those are (SP mesh index upload), rather than
add more allocator checks.
