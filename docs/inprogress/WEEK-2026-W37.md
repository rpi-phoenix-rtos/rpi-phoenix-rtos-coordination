...
## 2c. ✅ X desktop **2.5× faster**; bottleneck now narrowed to Mesa's CPU tiling
...
`rpi4-v3d` CPU **76.9 % → 14.1 %**. 0 faults, desktop correct.
⚠️ My *explanation* was wrong though: I said each GL call was an RPC to `/dev/v3d-srv`. It isn't —
the daemon client forwards **ioctls**, and Mesa runs in the X server's own process, so pixels never
crossed a socket. The cost was per-call overhead **inside Mesa**. The fix stands; the reason didn't.

**Where the rest goes — measured, 376 ms/frame:** draw+`glFinish` **1.5 ms** · `glReadPixels` 10.7 ·
CPU repack 6.3 · **`XPutImage`+`XFlush` 337 (90 %)**. So **the GPU was never the problem.**

**AF_UNIX exonerated:** `rpi4-ipcprobe` gained a throughput mode (`devices 7e58045`) — **213 MB/s**
at 64 KiB and 212 at 4 KiB, i.e. ~59× faster than the X path and chunk-size-independent. 1.2 MB
should cost 5.6 ms, not 337 — so **~331 ms is server-side.**

⏳ **Next, specific:** `glamor_put_image_gl` does *not* bail, so the cost is inside the bulk upload.
Candidate: Mesa's **CPU tiling** — V3D textures are UIF/LT tiled, so `v3d_store_tiled_image()`
(`v3d_tiling.c:481`) re-lays-out 1.2 MB per frame, per pixel, into a possibly-uncached BO. That is
exactly the pattern that cost 10× in `hevc-play`'s blit. **Measure it before touching it.**
Analysis doc §18–§19.
