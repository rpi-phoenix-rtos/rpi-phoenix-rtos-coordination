...
**Fix design (next):** on a transfer error, issue Reset Endpoint + Set TR Dequeue Pointer to
clear the halt; and on an enumeration retry, Disable Slot so the next attempt starts from a
clean slot instead of a poisoned one. Both are standard xHCI recovery that this driver simply
never implemented.
