...
## 2b. 🐞 YOUR BUG #5 — glamor paints screen content at its exact vertical MIRROR

Still open, but **two of three suspects are now eliminated by experiment**, not by argument. Both
artefacts you saw are one bug: pixel-exact vertical mirrors of *other* screen regions about the full
screen height (mean|diff| 4.6 / 5.0 against 21 / 17 for the next-best offset), and the mirror is
screen-global, so the faulty coordinate is in the screen pixmap's texture space.

| suspect | verdict |
|---|---|
| `glamor_spans.c` | ❌ **refuted** — its own probe: 0 hits, neither span path ever runs on the screen pixmap |
| `glamor_copy.c` (5 routes) | ❌ **refuted** — **one** copy in a whole X session, and neither side was the screen pixmap |
| the DDX presentation **readback** | ⏳ **now the suspect** — it is the only path that touches whole *bands* of screen rows, and both artefacts are band-shaped |

Two specifics make the readback the strongest candidate: its axis `H` comes from
`glGetTexLevelParameteriv(GL_TEXTURE_HEIGHT)` — a **texture** height, and glamor recycles FBOs from a
size-bucketed cache, so it need not be 1080; and the two measured mirror constants are **not equal**
(`dst_y+src_y` = 1092 for the xterm band, 1081 for the clip icon), so the axis varies per flush,
which a fixed screen height would not. **Next:** log `(y0, rows, H, width)` per readback call and
compare against the two measured pairs — *not* another patch first. Analysis doc §9.
