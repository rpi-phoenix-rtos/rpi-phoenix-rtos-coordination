...
**Next (cheap and decisive):** `stat()` the same file via paths of different depth in ONE
filesystem. If cost scales linearly with component count, the per-component lookup is
confirmed and the fix belongs in the path-resolution round-trip — libphoenix `resolve_path`,
posixsrv lookup, or the kernel `proc_lookup` message path.
