...
- **⏳ IN PROGRESS — `std::chrono` 1-second resolution: FIX IT** (owner, 2026-09-09: "rebuild during
  the night and fix this problem"). Confirmed from the shipped `c++config.h`: **all five** backends are
  off — `_GLIBCXX_USE_CLOCK_MONOTONIC`, `_GLIBCXX_USE_CLOCK_REALTIME`, `_GLIBCXX_USE_GETTIMEOFDAY`,
  `_GLIBCXX_USE_NANOSLEEP`, `_GLIBCXX_USE_SCHED_YIELD` — so it is broader than `chrono`:
  `this_thread::sleep_for` and `yield` degrade too. All four primitives **do** exist in
  `libphoenix.a` (verified by symbol), so only libstdc++'s cross-compile *detection* failed — these
  are autoconf **link** tests and a freestanding cross build cannot link them at that stage.

  Fix committed to `phoenix-rtos-build` (`--enable-libstdcxx-time=yes` plus cache overrides for the
  two checks that flag does not reach; **`=yes` not `=rt`**, because `=rt` appends `-lrt` and this
  target has no librt). **Rebuilding now into a separate root `.toolchain-chrono/` so the working
  toolchain stays intact** — this is the overnight job. ⚠ Not pushed and not adopted until the
  rebuilt `c++config.h` shows the five defines; then the C++ consumers (SuperTuxKart above all) need
  rebuilding and a hardware run. If it works, STK's per-port `CLOCK_MONOTONIC` patch becomes
  redundant.
