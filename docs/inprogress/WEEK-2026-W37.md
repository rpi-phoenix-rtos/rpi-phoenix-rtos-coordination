...
⏳ **Boot-scoped hunt for the reopened `premain-hang` running:** 12 trials, one **cold** quakespasm
launch per boot (the original failing scenario), on a **traced** build so a stall names the failing
initialiser instead of costing another hunt. ⚠ The export therefore carries a **diagnostic** build
again; the delivered SD image is untouched.
