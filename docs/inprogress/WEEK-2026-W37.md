...
the second texture unit / the MODULATE combine (or, if quake3e is using its own GLES
shaders here, at the lightmap sampler uniform binding — an unset sampler defaults to unit
0, and an unbound unit samples black). It also explains why bot models are fine (entity
lighting, single texture) and why the outdoor courtyard survives (different shader stages).
Code analysis of the render path is running; next A/B is whichever multitexture cvar our
fork actually has — I will not guess the name.
