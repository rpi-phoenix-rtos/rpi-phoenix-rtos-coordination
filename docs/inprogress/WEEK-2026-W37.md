...
## 2c. ✅ X desktop **2.5× faster**; the rest is WAITING, not computing — and one suspect fits numerically

**Fixed (verified 3 ways):** our screen-flip patch uploaded the Y-mirror one `glTexSubImage2D` **per
row**; now one bulk call. GL window **1.0 → 2.5 updates/s** · Game of Life **12.2 → 17.0 gen/s** ·
`rpi4-v3d` CPU **76.9 % → 14.1 %**. 0 faults.

**Everything that does work is now measured, and none of it is the cost:**

| | cost/frame | verdict |
|---|---|---|
| GPU (`draw` + `glFinish`) | **1.5 ms** | the GPU was never the problem |
| `glReadPixels` + client repack | 17 ms | small |
| AF_UNIX transfer (213 MB/s measured) | 5.6 ms | **exonerated** — 59× faster than needed |
| Mesa upload (`transfer_unmap` 0.15 ms × ~10) | ~1.5 ms | **exonerated** |
| **`XPutImage` + `XFlush`** | **~360 ms** | ← all of it |

Nothing is CPU-bound (the server sits at 37 % of one core of four), so the request is **blocked, not
computed**. ⏳ **Suspect with an arithmetic fit:** this port's `poll()` work leaves a **20 ms timed
fallback** for fds that aren't readiness-woken. A 1.2 MB request drains in chunks:
`1.2 MB / 64 KiB = 18.75`, `× 20 ms = 375 ms` vs **measured 337–360**. If the server's client fd
takes that fallback, every chunk of a large request costs a full tick — which would slow **every** X
client in proportion to request size, not just this window.

**Next: one test** — count `read()`s per `XPutImage` and time the wait before each (three outcomes
recorded). Also flagged: ~10 uploads per frame for one 640×480 window looks like its own bug.
**Four hypotheses refuted by measurement, one confirmed.** Analysis doc §18–§20.
