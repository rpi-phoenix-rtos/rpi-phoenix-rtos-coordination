...
### ★★★★ 4a-stkproof. The stack window CONFIRMS it: 128+ bytes of mesh indices on the stack
...
Read as little-endian `uint16`, every one of the 64 halfwords is a plausible **triangle index**:
0x00eb (235) rising to 0x1fc4 (8132), gradually increasing with local variation — the exact
shape of an index buffer for a mesh of ~8k vertices. Nothing else in that window: no saved
registers, no pointers, no ASCII.
