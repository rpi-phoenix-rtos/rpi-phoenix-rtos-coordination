...
## 2b-bis. ⚠️ GLAMOR WORK IS BLOCKED: the X server cannot be rebuilt, and the cause is in the BUILD

The known-good glamor daemon renders the desktop at **0 faults**. Every relink against the current
GPU archives gives a server that starts, brings up all six clients, then shows a **grey or black
screen** with GPU faults. Three source suspects are now **cleared by experiment**, one per cycle:

| suspect | verdict |
|---|---|
| my DDX damage-rows change | ❌ innocent — clean `fbdev.c`, still grey→black (std **0.8**, 3 faults) |
| Mesa `aa916f2f060` (`u_vbuf` guard, your bug #3 fix) | ❌ innocent — neutralised the hunk, rebuilt, **still black**; the code reading agrees |
| the V3D winsys / uncommitted Mesa files | ❌ inert — add-then-revert pairs (net zero); the untracked files are not in the archive (`ar t`) |

So every source delta is accounted for and the difference is in the **build**. One narrowing came
free: **the same archives render all four Quakes and STK correctly** (every game capture in the reel
postdates them), so this is **glamor-specific** — 2D, many small draws, render-to-texture — not a
v3d regression.

⚠️ **Why it is stuck, and my mistake:** we saved the known-good *binary* but not the archives it was
linked against, and later rebuilds overwrote them in place — so there is nothing to diff or bisect.
Rule now written down (`artifacts/x11/known-good/README.md`): promote the archives **beside** the
binary. **Next:** capture a known-good archive set the next time one is proven, then bisect the
build against it. No more Pi cycles guessing at the source.
**✅ Demo unaffected — it runs the known-good daemon (sha `8e003ab45ef41009…`).**
