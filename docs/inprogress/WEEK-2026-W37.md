...
rebuilt `python3`. A **fresh 2-part image carrying all of it is building now**
(`--variant sd --with-showcase --with-tests`, currently in the ports stage; 0 errors so far). It is
not gated yet, so the pointer above deliberately still names the old one.
