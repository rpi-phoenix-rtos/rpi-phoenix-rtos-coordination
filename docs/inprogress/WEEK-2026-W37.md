...
**Per-component model REFUTED, and the real shape is per-SERVER.** `fileperf --depth`
stats the SAME file through paths differing only in component count (`./` padding — same
inode, same directory). Cost is **completely flat** from 2 to 10 components, so resolution
is not doing a round-trip per component. What it is instead:

| filesystem | stat (same file, 20 iterations) | vs devfs |
|---|---|---|
| **devfs** (`/dev/zero`) | **226 us** | 1x |
| dummyfs ramdisk (`/ramtmp/x/f`) | **17 717 us** | **78x** |
| nfs-fs (`/usr/share/.../stk_config.xml`) | **35 282 us** | **156x** |

**devfs reaches 226 us through the same VFS/IPC machinery**, so the message round-trip is
capable of being fast — the 17-35 ms is specific to those servers or to how requests reach
them. Note devfs and `/ramtmp` are *both* dummyfs instances (`dummyfs;-N;devfs;-D` and
`dummyfs-tmp;-m;/ramtmp;-D` in the syspage), yet differ by 78x, so it is not the dummyfs
code itself.

~17 ms also smells like a fixed wait rather than work — the same shape as the documented
`poll()` stall that cost ~100 ms per RPC (`nfs_set_poll_timeout(1)`): a server that waits
for a timer instead of waking on message arrival.

**Next:** stat `/ramtmp` (the mount point) and a 1-component file on the NFS root, to see
whether the cost is paid crossing the root mount or inside each server; then look for a
timed wait on the request path of nfs-fs / the mounted dummyfs versus whatever devfs does
differently.
