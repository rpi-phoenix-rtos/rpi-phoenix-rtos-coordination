...
⚠️ **Honest gap, my mistake:** cutting the image relinked **quakespasm, yquake2 and vkquake** too
(the `sd` variant forces the full stage list; I expected them to come back cached and they did
not). Their *source* is unchanged — I reverted the temporary V3D diagnostic, so the tree is the
gated tree plus STK's fix — but the binaries are **not byte-identical** to the previously gated
ones (the relink isn't reproducible), so their HW gate is evidence-expired. **Next turn: one
verification cycle each for quakespasm, yquake2, vkquake.** Quake III and the X desktop were not
relinked and keep their gate. Nothing is known-broken; the gate evidence is what lapsed.
