...
### ★★ DELIVERABLE: SIX-COMPONENT SD image (supersedes the 5-component one below)
...
**Verification plan:** the failure is ~1 in 3, so a pass needs a **rate**, not one clean boot —
bench the desktop and count `usb: New device` (must be **4**: root hub + hub + mouse + keyboard)
plus zero `open failed`. A single good boot proves nothing here, which is the mistake the
`--disable-dynamic-lights` episode already cost once.
