...
**★ SHOWCASE REEL v2 — NOW WITH REAL MOVEMENT (you asked, 2026-09-08).**
`artifacts/hdmi-video/20260908-154518-phoenix-rtos-rpi4-showcase.mp4` (132 s, 74 MB, 1080p30
H.264 High, limited-range/bt709, faststart). Rebuild: `./scripts/make-demo-reel.sh`.

Four of six segments are now genuine gameplay, from the demos the games **already ship**:
...
· X desktop (static by nature) · Quake III (static arena — see below)
Motion verified *through* the concat (4/4 distinct frames 1 s apart inside a segment), 0 faults on
every capture.

⚠️ **Quake III has no demo footage and can't easily get any.** Its shipped demos are the 1999
`.dm3` protocol; the engine only ever looks for `demos/<name>.dm_66/67/68/71` (log:
`Not found: demos/demo001.dm_66 …`), so they can never be found, and that protocol isn't
supported. Reading them is an engine/netcode project, not a showcase task. A bot match with
spectator-follow is the realistic route if you want Q3 moving — say the word.
