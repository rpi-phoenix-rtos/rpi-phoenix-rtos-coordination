...
**#1 — REAL and reproducible at a rate. My earlier read was WRONG.**
Last week I leaned toward "the corrupted capture was a mid-repaint sample". The proper
test says otherwise. `xresizer 0 drag` now does what a mouse does — ~40 `XResizeWindow`
calls a second for 3 s — then **stops and holds still for 12 s** so an HDMI tick samples a
quiescent frame. Four rounds, three sampled:

| settle round | result |
|---|---|
| 0 | ✅ correct at 700x480, all four bars, diagonal corner-to-corner |
| 1 | ✅ correct |
| 2/3 | ❌ **window ~395x290 while the client painted for 700x480** — blue and yellow bars missing, diagonal stops mid-window |

So roughly **1 in 3** settle rounds ends with the window's actual geometry and the client's
painted content disagreeing, and it **does not self-correct in 12 s**. That is exactly
"resizing doesn't work, strange visual artefacts". Being intermittent is also why single
screenshots kept misleading me — the same trap as #67 (grade a RATE, never one frame).

**Next:** make `xresizer` print `XGetGeometry` alongside its own `w`/`h` at each settle,
which says whether the CLIENT missed a ConfigureNotify or the SERVER/WM left the window at
an intermediate size; then grade over ~10 rounds for a real pass rate.
