...
⚠️ **This build is RED and must not be shipped: `bin/supertuxkart` FAILED TO LINK**
(undefined `deflate`/`deflateEnd`/`deflateInit2_` via libpng, and
`glProgramLocalParameter4fARB` via SDL2). The build still exits 0 because it is a soft step, so
an image cut from it would silently carry a **stale STK binary**. `libz.a` does define
`deflate`, and `libpng16.a` was rebuilt during this run, so it is a link-line/ordering
regression triggered by the xorg_server rebuild — **not** a missing library. Must be fixed
before any image is cut.
