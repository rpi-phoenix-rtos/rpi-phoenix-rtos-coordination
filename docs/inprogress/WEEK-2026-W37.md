...
- **✅ `std::chrono` FIXED and HW-VERIFIED — and it was ~30 features, not one.** (owner, 2026-09-09:
  ...
  ✅ **No regressions on the new toolchain:** `test-libc-{string,stdlib,stdio,math,pthread,time}` =
  **542 tests, 0 failures** on hardware.
  ✅ **`clock_getres` HW-validated:** the new `time_clock` group is **7/7, 0 failures** — including
  the test that a 20 ms `nanosleep` shows up as ≥20 ms and <1 s, which fails loudly on exactly the
  1-second regression. ⓘ `--scope core` does **not** build the tests; `--with-tests` is needed, and
  the old `test-libc-time` in the export had none of the new group.
  ⏭ **Deferred, needs its own window:** rebuilding the ports stage. C code is unaffected by this fix
  (it only touches libstdc++), so **SuperTuxKart is the only real C++ consumer** and its per-port
  clock patch already covers it — the ports rebuild is for consistency plus the CPython `config.site`
  fix, not for fps.
