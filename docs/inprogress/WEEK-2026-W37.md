...
**#1 — REAL, deterministic, and localised to WINDOWMAKER (not the server, not the driver).**
`xresizer 0 drag` does what a mouse does — ~40 `XResizeWindow` calls/second for 3 s — then
stops and holds still 12 s, and now prints the SERVER's geometry next to the client's
belief at each settle. Ten rounds, and the A/B is unambiguous:

| configuration | result |
|---|---|
| under **WindowMaker** | **10/10 MISMATCH** — e.g. `asked=700x480 server=633x415 client=700x480 frame=700x511` |
| **no WM** (same binary, same drag) | **server=700x480 every round**, 9/10 OK (round 0 is just the client not having drained its configures yet) |

So the X server and the V3D path apply the resize correctly every time; WindowMaker is what
loses it. The numbers name the mechanism precisely: `633 = 360 + (119*7 % 560)` and
`415 = 240 + (119*5 % 420)` — **exactly the last size of the drag loop**. The WM resizes its
frame to the requested `700x511` (= 700x480 + titlebar) and tells the client it got
`700x480`, while the client window itself stays at the previous drag size. Content and
geometry then disagree permanently — the owner's "resizing doesn't work, strange visual
artefacts".

⚠️ **Correction to my previous entry:** I had recorded this as "roughly 1 in 3, intermittent"
from grading settle frames by eye. With instrumentation it is **10/10** — a 633x415 client
inside a 700x511 frame simply *looks* plausible in a screenshot. Grading by eye understated
it.

**Candidate code, and what I can and cannot yet support:** `wClientConfigure`
(`src/client.c:147`) ends in `wWindowConfigure` (`src/window.c:2085`), whose resize
decision is `resize = (req_width != wwin->client.width || req_height != wwin->client.height)`
(`:2098`) — a comparison against the WM's **cached** client size, so any desync makes an
identical request early-out forever. I also examined the grow/shrink ordering at
`:2119-2134`, where `wFrameWindowConfigure` mutates `frame->core->width/height` *between*
the two guard conditions — but on analysis both branches still resize the client, so **that
is not the bug** and I am not claiming it. Nor can I yet explain how the cache would hold
`700x480` when the pair `(700,480)` never occurs during the burst (checked arithmetically).

**Next:** have the client poll `XGetGeometry` *during* the burst to capture the WM's applied
sequence, which distinguishes "the WM never applied the final request" from "it applied it
and something later reverted it".
