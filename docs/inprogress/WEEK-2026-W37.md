...
⏭ **Refreshed demo image is the next step** — the delivered `2940cc70` predates this fix. Three
follow-up devices commits (runtime-reset guard · cold-state probe off the racy FIFO · vcmbox lookup
budget) landed *after* the gate started, so they are **not** in the gated build and need a short
re-validation before any image is cut from them.
