...
**Next:** time the same LOOKUPs from the Linux host against the same export (host-side, no
Pi needed). If Linux sees ~0.1 ms where we see 5.5 ms, the stall is in our client, not the
server — that splits (1) from (2) cheaply.
