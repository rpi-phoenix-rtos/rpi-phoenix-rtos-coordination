...
**Next (one line, decisive):** vary `nfs_set_poll_timeout` (1 -> 5, and 0 for a busy-poll)
and re-measure. If per-component cost tracks the timeout, the cost is poll-bound and the fix
is the documented one — make the lwip socket poll wake on readiness, which speeds up every
poll/select consumer (X11, Redis, lighttpd, Dillo), not just NFS.
