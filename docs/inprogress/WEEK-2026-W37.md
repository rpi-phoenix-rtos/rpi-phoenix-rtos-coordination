...
⏳ **A 6-trial STK pass-rate bench is running** (`stkrate`, detached so it survives between
heartbeats) to see whether the guards actually change the crash rate. Baseline to beat: **2 of 5** runs
faulted before them. ⓘ Be honest about the statistics — 6 trials cannot separate "40% → 20%" from
noise; `0/6` would be suggestive and `3/6` would show no improvement, and nothing in between will
settle it. It is worth the Pi time only because it costs no attention.
