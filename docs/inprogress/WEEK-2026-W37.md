...
✅ **Added the missing TSD coverage** (tests, committed): per-thread isolation (threads spin
re-reading their own value so a shared slot is caught), a new thread starts unset while the creator's
value survives, destructor runs at thread exit, overwrite replaces rather than stacks. Built clean;
⏳ awaiting the Pi (bench in progress).
