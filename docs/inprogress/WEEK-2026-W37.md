...
**#67 vkQuake torches — 3/3 runs present, 32/32 at-reference frames lit** on the shipped build
(`check-torch-rois.py --label xgvq-T1 / xgvq-T2 / xgvqb-T1`, mae 3.4–3.8, both archway ROIs
93–120). Encouraging, but **3 runs is NOT a verdict** for a ~10–20% *per-boot* defect — the
standing rule for #67 is n>=8. Building that sample next; the 32/32 frame count is only strong
evidence if the defect is per-frame, and the evidence says it is per-load.
