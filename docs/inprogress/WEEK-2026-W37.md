...
So on shutdown the **DAMAGE extension looks up a private on a NULL object** — a teardown
*ordering* bug: the damage private is fetched after the owning object's privates have been
freed (or on a screen/drawable already gone). Fix belongs in the xorg_server port, most likely
a missing NULL guard on a damage callback reached from CloseScreen.
