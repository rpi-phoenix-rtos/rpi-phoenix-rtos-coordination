...
`allfixes-20260907`. The `dd` line below still named that deleted file until 2026-09-08 — fixed; image re-verified byte-intact the same day.
...
(`/dev/sda` is always the SD card on this host. Do **not** flash `rpi4b-sd.img` — that one is
FAT-boot-only, no rootfs.)
...
## 4e. X desktop-exit crash: 4 attempts failed, but the fault is now DECODED — OPEN

Reproduce on demand: `startx_gpu --quit-after 90 deskapps`. Fires every run. Shutdown-only; the
shipped image was never gated on it.

**Today's gain is the decode, not a fix.** Two readings that had steered every previous attempt
were wrong, and correcting them changes which object is suspect:

* `far/x0 = 0x60` is the **return value** of `dixGetPrivateAddr`, not its argument — `pc` is
  `+0x68` into damage.c's copy of `dixGetPrivate`, at `ldr x0,[x0]`. So `x1 = 0` is
  `pScreen->devPrivates`, and the key was **never** NULL (`[sp+16]` = `0x1b83450` =
  `damageScrPrivateKeyRec`; both key asserts passed). The old "NULL key is the strongest clue"
  note was a misread of a leftover register.
* `pDamage->pDrawable` is **stale in both runs** — run 1 `0x3ad9c8` sits below the image
  (`0x400000-0x1baac80`) and outside the heap with a self-pointer at +16; run 2 `0x2438e78` is
  only `0xF0` below its claimed `ScreenRec`, which is `0x3d8+` bytes, so they would overlap.
  Neither is a live drawable. **The broken lifetime is the damage's drawable**, which is why
  three private-focused fixes could not converge.
* Chain: `doFreeResource -> FreeGC -> glamor_destroy_gc -> glamor_invalidate_stipple ->
  DamageUnregister` — inside `FreeAllResources()`, well before main.c's screen loop frees
  `devPrivates`.

**Attempt 4 REFUTED and reverted** (`dbb322e09`): guarding `glamor_track_stipple` against
re-registering. Real latent defect, wrong bug.

**★ 2026-09-08, three instrumented cycles — the mechanism is now MEASURED, not hypothesised.**
Probes in `glamor_track_stipple` / `glamor_invalidate_stipple` / `damageDestroyPixmap` (the last
keyed on a global set to the exact registered pixmap), then reverted; tree byte-identical again
(sha `2da1b416…`). All runs agree:

* The stipple damage is registered **once**, on `gc->stipple` (`refcnt=2`, `type=PIXMAP`), and at
  crash time `pDamage`/`pDrawable` are **exactly that registered pair** — the damage is intact
  and still correctly registered.
* That pixmap has been **freed**: `pDrawable->pScreen` reads back a self-pointer (allocator
  bookkeeping), so `devPrivates` is garbage → the fault.
* `gc->stipple` != `gc_priv->stipple` (measured) → `glamor_destroy_pixmap`'s wrapper bypass is
  **ruled out**.
* **`damageDestroyPixmap` is never entered for that pixmap**, even though `FreeGC` (dix/gc.c:781)
  releases the stipple via `(*pScreen->DestroyPixmap)` *before* the line-783 `DestroyGC` that
  crashes — and a *later* print did flush, so the absence is real.

**→ damage.c's `damageDestroyPixmap` is not in this screen's `DestroyPixmap` chain, so no pixmap
free ever cleans up registered damages.** glamor's damage outlives its drawable.

**Next probe (small, should settle it):** print `pScreen->DestroyPixmap` right after
`DamageSetup`'s `wrap()` and again at `glamor_destroy_gc` entry, next to `damageDestroyPixmap`'s
address. If they differ, find who clobbered the chain (glamor's screen init wraps screen hooks
too — first candidate). `DamageSetup` demonstrably ran: `pScrPriv` is valid.

Full decode, the four failed attempts, and **three harness traps** (one invalidated a whole Pi
cycle: `test-cycle-psh-interact.sh` re-syncs `/bin` from `.buildroot/_fs` at cycle start, so a
daemon staged only to the NFS export is overwritten before boot):
**[docs/misc/2026-09-08-x-teardown-crash-open.md](../misc/2026-09-08-x-teardown-crash-open.md)**
