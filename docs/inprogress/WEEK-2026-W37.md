...
## 1b. ⏳ QUEUED (owner, 2026-09-08): full-project change study for upstream maintainers
...
current bug fixing effort." So it starts once the damage-teardown fix and the STK link
...
**⏸ OWNER ACTION — ★ FLASH THIS (superseded twice since; this is current):**
`artifacts/rpi4b/rpi4b-sd-2part.img`
SHA256 `0ddcee676f46ac57f607e7f7bd9f26b5e575fa0621863adc9b6428a0baaaa91e`,
manifest `manifests/2026-09-07-drivable-demo-image.md`.

**6/6 components gated x3 trials (21 boots, 0 faults) + reliable keyboard/mouse + correct
pixels on all six + torches 9/9.** Supersedes the six-component image, which superseded
`allfixes-20260907` (this section pointed at that stale one until 2026-09-08 — corrected).
...
## 4e. ★★★ desktop-teardown fault REPRODUCED and localised — X's DAMAGE extension

**`--quit-after` works and the fault is real.** With a valid repro the teardown ran genuinely —
`--quit-after fired -> SIGTERM to client[0] (WM) pid 28 -> client[0] exited (status=0xf00, i.e.
SIGTERM) -> session ended -> shutting down X` — and produced **2 faults + a stack dump**
(`-wmquit2`). The three earlier "0 faults" results were worthless, as suspected.

**It is not wmaker.** The faulting process is **`/bin/Xphoenix-glamor-daemon`** (PID 27,
thread 62): `Data Abort (EL0)`, `esr=0x92000007` (a **read**), **`far=0x60`** — a near-NULL
dereference at a struct offset. `pc` decodes to **`dixGetPrivate` in `damage.c`**.

**ROOT CAUSE, from the stack window's frame chain + the register dump:** callers decode to
**`DamageUnregister` -> `dixLookupPrivate` -> `dixGetPrivate`**, and **x0 = 0x60** is the
*privates address* handed to `dixGetPrivate` — i.e. `&((base)NULL)->devPrivates`, so the base
object was NULL/freed.

`damageCloseScreen` (`miext/damage/damage.c`) does `free(pScrPriv)` **and then** calls the
downstream `CloseScreen`. Layers wrapped *below* damage still call into it while they close —
the software cursor's `miSpriteCloseScreen` (`mi/misprite.c`) calls `DamageUnregister`, which
does `damageScrPriv(pScreen)` and dereferences that just-freed allocation. **A use-after-free
with an obvious ordering fix.**

**FIX: `phoenix-rtos-ports 4133eff`**, `xorg_server/patches/0001` — defer the `free` until the
downstream chain has unwound.

⚠️ **THE FIX DID NOT RESOLVE THE CRASH, and my premise was wrong. Do not ship this as
"xorg_server fixed".** On HW (`-dmgfix`, patched server confirmed staged) the teardown still
faults 2x. What changed is the *shape*: `Data Abort` reading NULL+0x60 became
**`PC alignment fault`, `pc = far = 0xaa1403e0aa1503e2`** — a value that is two AArch64
instruction words (`mov x0,x20` / `mov x2,x21`), i.e. a **return address overwritten with code
bytes**.

❌ **The caller is glamor, not misprite.** The stack window now shows one frame above
`DamageUnregister`: `0x5b2624` -> **`glamor_invalidate_stipple` (`glamor_core.c`)**. My patch's
comment asserts `miSpriteCloseScreen` calls back into damage — I inferred that from grepping
`DamageUnregister` callers and picking the plausible one, and **never evidenced it**. The
evidence says our own glamor teardown is the caller.

**ROOT CAUSE (both faults, one cause) — `ports 4569f79` supersedes the first cut.** The caller
is confirmed by symbol bounds, not adjacency: `0x5b2624` is **+68 into
`glamor_invalidate_stipple`** (0x5b25e0–0x5b2644), whose neighbours are
`glamor_stipple_damage_report`/`_destroy` — glamor registers its own Damage on stipple pixmaps.

`DamageUnregister` does `damageScrPriv()` and then an **indirect call** through
`pScrPriv->funcs.Unregister`. `damageCloseScreen` freed `pScrPriv` but left a **dangling
pointer in the private slot**, and glamor destroys its GCs during the downstream CloseScreen.
Hence both shapes: a bad *read* (NULL+0x60) first, then — once the free was merely deferred —
a **jump through the stale function pointer** (`0xaa1403e0aa1503e2`, two AArch64 instruction
words).

**Why deferring the free was not enough:** a lookup on a freed-but-still-populated slot returns
garbage, not NULL, so nothing downstream can tell the difference. The fix therefore (a) clears
the slot after freeing, converting a use-after-free into a checkable NULL, and (b) guards
`DamageUnregister` on that NULL before the indirect call — plus (c) a NULL-`pDrawable` guard,
since glamor reaches `DamageUnregister` **twice** on GC teardown (once from
`glamor_invalidate_stipple`, once from the `DamageDestroy` callback) and it clears `pDrawable`
on its way out.

**Two narrower fixes FAILED on HW. Recorded so they are not retried, and the third is
different in kind, not just bigger:**
1. *Defer the free past the downstream CloseScreen* (`-dmgfix`) — still faulted. It changed the
   symptom only: Data Abort at NULL+0x60 became a **jump through the stale function pointer**
   (PC alignment, `0xaa1403e0aa1503e2` = two AArch64 instruction words). The calls come from
   *within* that chain.
2. *Defer + clear the slot + guard `DamageUnregister`* (`-dmgfix2`) — fixed that caller and
   moved the identical NULL dereference to **`compDestroyWindow`** (+276, resolved by symbol
   bounds), i.e. the **Composite** extension. Guarding callers does not converge.

**FIX (rebuilding): `ports cf235b4` — do not free the screen private at all.** Composite,
glamor and misprite all call into damage *after* `damageCloseScreen`, and every damage entry
point begins with `damageScrPriv()`, several then making an indirect call through
`pScrPriv->funcs.*`. **No placement of the free is safe, because damage does not own the
teardown order of the layers above it.** So the allocation outlives them: one struct per screen,
reclaimed when the process exits moments later — a bounded one-shot leak traded for a whole
class of shutdown crash.

❌ **RETRACTED: there is no STK link regression and the build is not red.** I read
`bin/supertuxkart Error 1` as a breakage. It is **by design**: the recipe lets CMake's link
fail (its line lacks `-lz` and the GL ARB symbols), swallows it with `+ true`, and then performs
its **own manual link** against `libSDL2.a` + `libGL-phoenix.a` + `libv3d-phoenix.a`. Evidence
it works: the same `Error 1` appears in **every** build back to `build-stkfix`, each followed by
`[supertuxkart] M3 complete: /usr/bin/supertuxkart linked + installed`, and the installed binary
is freshly dated per build (23:58 for `-damage2`).

That also **re-validates the STK verification** I briefly doubted: the binary tested at 3/3 was
the patched one, not a stale carry-over.

⚠️ Worth fixing properly anyway (cosmetic, low priority): the CMake link line should carry `-lz`
so the build does not print a scary handled failure that reads exactly like a real one.

**Impact unchanged and still narrow:** fires only on desktop-session exit, after a recording
would end; the box stays up and psh resumes. None of the 21 gated boots are affected.

**Re-gate scope for this fix is desktop-only, and that is checked rather than assumed:** the
change lives in `xorg-server`'s `damage` module, which exists solely in
`Xphoenix`/`Xphoenix-glamor-daemon`. Grepping the shipped binaries, all five games —
supertuxkart, quake3e, quakespasm, vkquake, quake2 — contain **0** references to
`XOpenDisplay`, `/tmp/.X11-unix`, `DamageUnregister` or `damage`, against **57** in the glamor
server. They render standalone on `/dev/fb0` via SDL, so an xorg_server patch cannot reach
them. Desktop x3 + teardown runs is therefore the proportionate gate, not another 18 boots. Nothing else in the function touches the private after the
unwraps, so the change is confined to that allocation's lifetime. Generated as a real diff
against the pristine tarball, and it is the port's first patch (the hook was already a no-op
placeholder). Needs a rebuild + a `--quit-after` re-run to confirm.

⚠️ Impact is narrow: it fires **only when the desktop session is closed**, after the recording
would be over, and the box stays up (psh resumes). It does not affect any of the 21 gated boots.

### (superseded) The three invalid attempts, kept as a caution

Last risk to a *recording*: the untriaged EL0 fault on desktop teardown (the owner's "exit
Window Maker -> Data Abort"). Someone recording will close windows or quit the desktop.

**Three attempts, all invalid. 0 faults in each, and none of them is evidence:**
1. `wmexit2` — ran the repro as a *second* psh command after `startx_gpu`. `xlaunch` blocks in
   a `waitpid` loop (`pl_phoenix_xlaunch.c:711`), so it held the console and **the script never
   executed**. The `wmaker` in that log came from `startx_gpu` itself.
2. `wmexit3` — self-contained script, but `pkill -x` is **not present** on the image, so
   **SIGTERM was never delivered**. (`pkill -f` was avoided on purpose: it matches the script's
   own argv, a footgun already hit here.)
3. `wmexit4` — signalled `xlaunch` by the PID the script holds; `kill` reported success and the
   desktop was fully up (glamor x2, 5 clients), but **no teardown output followed**, so there is
   no evidence xlaunch acted on it.

Also learned: **`ps` is unavailable** on the image, so the process table cannot be inspected
from a test script — which is why finding wmaker's PID kept failing.

**DONE:** `pl_phoenix_xlaunch --quit-after <secs>`. It is our own launcher, used
only for demos and tests, and it is the only way to drive the teardown path deterministically
without a mouse or a process table. Until that exists this item stays **open and untested** —
it is *not* "0 faults, looks fine".
