...
**Reading, offered as hypothesis not fact:** deferring the free may have fixed one genuine
use-after-free (the NULL+0x60 privates read is gone) and exposed a second, different teardown
bug (a clobbered return address). Equally it may be irrelevant to both. Next: decide whether to
keep the patch with a corrected comment or revert it, and chase the corrupted return address
from `glamor_invalidate_stipple`'s frame.
