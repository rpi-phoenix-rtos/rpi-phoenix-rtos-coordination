...
re-registering an already-registered damage. Real latent defect, wrong bug — a bounded `ErrorF`
on the suppressed branch **never printed**, so the path isn't taken here; `far` only moved
because the extra struct field repainted the heap. `libglamor.a` + staged daemon are back to the
pre-attempt build byte-for-byte.

**Next attempt starts here:** `damageDestroyPixmap` skips damage cleanup entirely unless
`pPixmap->refcnt == 1` (damage.c:1493). `gc->stipple` is referenced by the GC, so when the
client's pixmap resource is freed during `FreeAllResources()` the refcnt is 2, cleanup is
skipped, and the pixmap survives with the damage registered — the open question is whether the
final release routes back through the wrapped `DestroyPixmap`, i.e. **`FreeGC` ordering**.
Cheapest probe: one `ErrorF` on that `refcnt != 1` branch (pixmap, refcnt, damage-list
non-empty) — it fires regardless of the GC path. **Instrument, don't patch.** (The
`glamor_destroy_pixmap` wrapper bypass is ruled OUT: it runs on `gc_priv->stipple`, always a
fresh pixmap, never the `gc->stipple` the damage is on.)
