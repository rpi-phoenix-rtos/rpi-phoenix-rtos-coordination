...
`1ca1dbc1e5df5ca0497a676e3dbef7febc9e4004277c62f483365495ae1695c3` · manifest
`manifests/2026-09-09-x11-damage-fixed-demo-image.md`.
```
...
**In progress:** a host harness compiling the real `malloc_dl.c` against host `mmap`/`munmap` — so a
use-after-unmap genuinely faults — to reproduce #36 with **no Pi cycles**, asserting "a fully-free
heap holds one chunk" and "no binned chunk references an unmapped range". Doc §27.
