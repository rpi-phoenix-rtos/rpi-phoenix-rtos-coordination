...
⚠️ **HOST hazard, not a Pi or build problem — worth knowing for future runs.** Two benches
were killed mid-flight by the host's low-memory watchdog. Cause: NFS reads plus build
artifacts had filled **21 GB of page cache**, and the watchdog reads *free* rather than
*available* memory (26 GB was available the whole time). `sync; sysctl -w vm.drop_caches=1`
returns ~24 GB free and the bench then runs. **Drop caches before a long bench.** The Pi
powered off cleanly through its exit trap both times; no cycle was left holding the UART.
...
⚠️ **THE RAMDISK ROUTE IS IMPOSSIBLE AS DESIGNED — `/ramtmp` caps at 32 MiB.**
`DUMMYFS_SIZE_MAX` is `(32 * 1024 * 1024)` (`dummyfs_internal.h:29`), so a 194 MB extraction
could never fit. The tar stopped after **1 entry**, STK rejected the empty asset dirs
("Falling back to use '.'") and silently used `/usr/share/supertuxkart` on NFS instead — so
`-125201`/`-1258*-stkram2` tested NFS again, not the ramdisk. **The owner's original
suggestion was defeated by the same 32 MiB cap** (the earlier `-085655-stkzip` run also never
printed its EXTRACT line — it failed the same way and that went unnoticed because `df -h`,
which the ported `df` does not support, had eaten the space check).

Two things follow: the cap is `#ifndef`-guarded so it can be raised with a `-D` at build time
(worth doing — `/tmp` is dummyfs too and STK writes its config there), and **STK has now
crashed 2 for 2 on this build** (`rc=139`, SIGSEGV), so its failure is close to deterministic
rather than rare.

`tests/pi-repro/stkram.sh` staged and corrected (uncompressed tar, no `df -h`) but it needs
the raised cap to actually run.
...
- **quake3e MERGED (1 commit)** — upstream `f694bbbc` "improved AVI pipe format validation"
  ...
  published). `phoenix-rtos-ports b36c396`, fork `3cccffa4`. Rebuilding, then Q3 re-verify
  on HW **before** any push.
