...
**Shipped artifact verified.** The probe-free `Xphoenix-glamor-daemon` now staged to both roots was
re-run on HW: **0 faults, 108.1 ms/frame, mirror check MAD 74.98 (fixed)**, desktop upright, GoL
xterm clean, all six clients render. That is the binary in `/srv/phoenix-rpi4-nfs-gcc16/bin` and in
`.buildroot/_fs/.../root/bin`.
