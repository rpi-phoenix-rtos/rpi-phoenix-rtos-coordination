...
**Next instrument:** libphoenix exports `malloc_test()` (`malloc_dl.c:680`), a full bin+heap
consistency walk that is currently dead code. Calling it periodically around the shader-compile
stage would catch the overflow near its source instead of at the next allocator touch. It needs
an STK (or libphoenix) rebuild, so it is the next build-cycle job — the gated image is
