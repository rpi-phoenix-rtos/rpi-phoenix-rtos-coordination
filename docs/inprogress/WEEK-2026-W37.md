...
never eliminated it.
**Prediction:** 0018 should print `getSuccessor(0) out of range (0 successors)`. A large `i` with a
sane `n` would instead mean the index arrives corrupt; an absurd `n` would mean the vector header is
corrupt, tying it to the font crash. Each outcome points somewhere different.
