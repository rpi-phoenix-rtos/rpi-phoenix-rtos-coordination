...
**★★★ #3 — WORKED AROUND with full visual quality, and root-caused to the driver.**
Quake III's world program is `(tex0.rgb * gl_Color.rgb) * tex1.rgb`, built by Mesa's
fixed-function texenv lowering for the two-unit `GL_MODULATE` world stage. The decisive
experiment: **`r_ext_multitexture 0`** (two-pass rendering — one texture unit at a time,
lightmaps kept) renders the **whole world correctly during a full bot match**, where the
two-unit path is black.

**Shipped:** multitexture now defaults **off on Phoenix** (fork `4c2e9d83`, ports
`ac77bfa`). Verified end-to-end: with no flag passed the engine reports
`multitexture: disabled` and a bot match renders courtyard, statues, arches and ornaments
with lightmaps, 0 faults. Two-pass costs a second pass over world surfaces; the frames look
normal but I have **not measured the frame-rate cost**.

**Why this is a driver bug, established by measurement not argument:** instrumented
readback (`PHXDIAG`) showed the GL state is **identical** in the working and failing cases
— unit 1 bound to the lightmap, both units enabled, `GL_TEXTURE_ENV_MODE` read back as
`GL_MODULATE` on both, one pass, arrays set once. What multitexture changes is that it adds
a **second texcoord vertex element**; Mesa's `u_vbuf` translate fallback **silently skips
`pipe->draw_vbo()`** when it cannot build a translation (the failure path only frees
buffers), which drops the surface entirely — and we already have HW evidence of that path
misbehaving here (our own `f342ce50282`: "quake3 crash mid-combat, u_vbuf_translate_begin").
On aarch64 there is no `translate_sse`, so the generic path is always in play, and
`debug_warn_once` is inert in release builds, which is why it failed silently.

**Real fix, still open:** make the `u_vbuf` failure path loud and non-lossy (unconditional
log; roll back `fallback_vbs`/`real_vertex_buffer` on a failed begin instead of leaking
them; upload rather than skip). Then revert this default and re-verify.

Eliminated by experiment along the way, so nobody repeats them: dynamic lights, the
`glLockArraysEXT` violation, the shader disk cache (cold, still black),
`should_tile`/UIF layout, a 3-byte `GL_RGB` upload, `r_texturebits`, texture memory
pressure (`r_picmip 3`).

⚠️ **TD:** `PHXDIAG` stays in the fork for now — it is inert with multitexture off (it
lives in `DrawMultitextured`, which is no longer reached) and it is the instrument that
will verify the eventual `u_vbuf` fix. Remove it with that fix, together with
`gl3_forceunderwater`.
