...
**#3 — REPRODUCED and isolated to the LIGHTMAP path.**
A static spawn view is clean, so the missing ingredient was gameplay. Bots supply it
without input: `quake3 +devmap q3dm1 +addbot sarge 4 +addbot grunt 4 +addbot major 4`
runs a real frag match (scoreboard, bot chatter, 0 faults) — and after a respawn into the
indoor areas **most world surfaces render BLACK**, with vertical stripe artefacts in
places. That matches the owner's "strange visual glitches early on after gameplay
started".

The discriminating observation: a bot's **player model renders fully lit and correct** in
the middle of the black world. Player models use entity lighting, world surfaces use
lightmaps — so the two paths are separable, and only one is broken.

Two A/Bs, one cycle each:
- `r_vertexLight 1` → **world renders fully** (walls, floor, ornaments, all textured).
  So geometry, textures and the BSP are fine; base×lightmap is where it dies, i.e. the
  lightmap texture is reading as black.
- `r_mergeLightmaps 0` → **still black.** The merged-lightmap atlas is NOT the mechanism,
  which is the one hypothesis the project's prior q3dm7 lightmap-black work would have
  suggested first.

Third A/B settles it: **`r_lightmap 1` (lightmap only) renders BRIGHT and CORRECT** —
smooth light gradients on the arch, steps and walls. So the lightmap texture data is fine
too. Putting the three together:

| mode | textures used | result |
|---|---|---|
| `r_vertexLight 1` | base only, 1 unit | ✅ renders |
| `r_lightmap 1` | lightmap only, 1 unit | ✅ renders |
| default | base **x** lightmap, 2 units | ❌ **black** |

Both single-texture paths are good; **only the two-texture combine fails.** That points at
the second texture unit / the MODULATE combine (or, if quake3e uses its own GLES shaders
here, at the lightmap sampler-uniform binding — an unset sampler defaults to unit 0, an
unbound unit samples black). It also explains why bot models stay correct: entity
lighting, single texture.

**But it is not simply "the combine is broken", and that is the key refinement:**
- the **no-bots** run rendered the world correctly with default settings for its whole
  ~3 minutes;
- in the bot match, the world is **still lit at the first death** (score 1-0-0-0) and only
  goes black **later** in the same match.

So the two-texture combine works, then **degrades during play** — a progressive or
triggered state problem, not a static one. That fits the owner's wording exactly ("early
on **after gameplay started**"). Candidates to separate next: effect/decal rendering
leaving texture-unit state behind, versus lightmap pages evicted or re-uploaded during
play. `+god` did NOT take effect from the command line (the player was still fragged), so
pinning the player in a known-good area needs `+wait` before the cheat.

Code analysis of the quake3e render path is still running; the next A/B is whichever
multitexture cvar our fork actually has — I am not guessing the name.
