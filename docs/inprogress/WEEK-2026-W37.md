...
**Quake III gameplay — `+devmap q3dm1` + bots 3/3** (`demoq3` bench): map loaded, god mode
applied (so the bots actually fight), **0 faults, 0 dropped draws** every trial, and the
frame checked on pixels — a fully lit world, which is the thing #3 used to break.

**QuakeSpasm — `+map start` 3/3** (`demoqs` bench): pak found and `Host_Init` completed
every trial, **0 faults, 0 dropped draws**, frame checked on pixels.

So three independent 3/3 benches on the exact build that is in the SD image. **Quake II and
vkQuake are still 1/1** (verified once each on that build, not yet benched) — next
heartbeat.
