...
Both allocator sites are now guarded (libphoenix `2d55b1e` + `5ece71d`): free-bin links are
value-checked before the unlink dereferences them, and on garbage the bin is **abandoned** rather than
followed — the same remedy as the kernel scheduler fix earlier today, which was the identical defect
class one layer down. HW-verified no false positives (stdlib 92/0, unix-socket 27/0, clean desktop
exit). ⚠ **STK does not benefit yet:** its port keeps no reusable objects, so picking this up needs a
full port rebuild plus a re-gate — worth doing when the demo is not live, since it could convert 2 of
4 crash sites from a fault into a survivable report.
