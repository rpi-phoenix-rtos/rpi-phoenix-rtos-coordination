...
**Next attempt starts here:** `glamor_destroy_pixmap()` (`glamor.c:266`) calls `fbDestroyPixmap()`
directly instead of `(*pScreen->DestroyPixmap)()`, bypassing damage.c's `damageDestroyPixmap` —
the only thing that destroys a dying pixmap's registered damages. Resolve first that the damage
is registered on `gc->stipple` while that destroy runs on `gc_priv->stipple`. Instrument, don't
patch.
