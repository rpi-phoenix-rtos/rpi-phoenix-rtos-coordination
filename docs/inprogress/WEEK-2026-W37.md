...
| vkQuake | T1 clean, 2 more running | Vulkan + `Host_Init`, 0 faults |
...
the corruption mechanism, so the remaining candidate is timing. Better use of Pi time: **STK's
crash rate on this build**, the one component that actually fails and has no data at all.
