...
## 2b-bis. ⚠️ CORRECTED AGAIN: current Mesa is **not** usable for the X server

Last turn I said the X server can be relinked onto current Mesa because the isolation build did not
SIGILL. Right about the SIGILL (that was a partial `libglamor` rebuild, now fixed), but too general
a conclusion — I judged it on one signal without looking at the screen.

| daemon | Mesa | faults | display |
|---|---|---|---|
| known-good | `e4be116324` | **0** | renders + animates |
| current-Mesa | `aa916f2f06` | **4** MMU-VIO + wedge-reset, every run | black in one run, vertical stripes in another |

It does not crash, but it does not render either — so **glamor work is still blocked**, and "not
blocked" is withdrawn. Caveat kept: both corrupt-display runs also had the (now reverted) DDX change
compiled in, so those two runs alone cannot separate them; the MMU-VIO faults appear with and
without it, which is why the suspicion sits on Mesa. Settling it needs one current-Mesa run with no
DDX change and the **display** checked, not just the log.
**✅ Pi left on the known-good binary, re-verified this turn: 0 faults, desktop rendering.**
