...
- **✅ `std::chrono` FIXED and HW-VERIFIED — and it was ~30 features, not one.** (owner, 2026-09-09:
  ...
  ⏳ **Promotion RUNNING** — rebuilding straight into `.toolchain` (a GCC install embeds its prefix,
  so it is rebuilt rather than renamed; the whole build is ~10 min here). The old toolchain is parked
  at `.toolchain-prefeatures/`, and the promotion script gates on 12 macros in the installed header
  plus a C and a C++ link, **rolling itself back** on any failure.
  ⏭ Then: `--scope core` rebuild → rebuild SuperTuxKart (the only significant C++ consumer) → boot
  test → STK bench against the `8/9/9` baseline in §4c → 6/6 showcase gate.
  ↩ **Rollback at any point:** `rm -rf .toolchain && mv .toolchain-prefeatures .toolchain`. The
  running image is untouched by any of this.
