...
**Boot-scoped hunt for the reopened `premain-hang`: 12 cold launches, 11 clean, 1 INCONCLUSIVE.**
One cold quakespasm launch per boot (the original failing scenario) on a **traced** build. T12
reproduced the *symptom* exactly — command echoed, **no output at all**, prompt never returned, and
**no libc-init markers**, i.e. it never finished libc startup. ⚠ **But it is not a confirmed
reproduction:** my window was `--idle-secs 60`, and a large cold exec over NFS is a documented
~**68 s** class here (the read-ahead fix in `vm/object.c`). A slow *data* load would still have
printed the markers and `main() entered`; their absence points at **ELF paging**, which the 68 s class
covers. Re-running at `--idle-secs 240` to settle it. The harness flagged it `VOID (capture
truncated)` and said "re-run with a longer window before bisecting" — correct both times.
⚠ The export carries a **diagnostic** (traced) build again; the delivered SD image is untouched.
