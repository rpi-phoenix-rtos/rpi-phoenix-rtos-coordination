...
**Scope measured:** ~**1 540 commits**, ~**890 files**, ~**150 000 insertions** across 14 repos
vs `origin/master`. Largest surfaces: ports +55k, devices +37k, libphoenix +33k, kernel +5.8k.
...
## 4e. X desktop-exit crash: REPRODUCIBLE, 3 fixes FAILED, model was wrong — OPEN

Reproduce on demand: `startx_gpu --quit-after 90 deskapps` (that flag exists only for this; the
image ships no `ps`/`pkill` and `xlaunch` blocks in `waitpid`). Fires every run.
`Xphoenix-glamor-daemon` takes a read fault in **`dixGetPrivate`** with the *privates* pointer
= **0x60** (so the base object was NULL) and the private **key = NULL**. Callers, resolved by nm
bounds: `DamageUnregister` (from **`glamor_invalidate_stipple`**) and **`compDestroyWindow`**.

**All three fixes failed on HW, and the third disproves the model:** (1) defer the free — became
a jump through a stale function pointer; (2) defer + clear the slot + guard `DamageUnregister` —
moved the same fault to `compDestroyWindow`; (3) **never free the private at all — fault
unchanged**, so a freed screen private was never the cause. **Reverted all three**
(`ports 21bd0ec`) rather than ship an unproven change that also leaked.

Full evidence, the failed attempts, and why `x1=0` (NULL key) never fitted the model:
**[docs/misc/2026-09-08-x-teardown-crash-open.md](../misc/2026-09-08-x-teardown-crash-open.md)**.
Next attempt should use the kernel's existing **hardware watchpoint** (prints `pc=writer` +
backtrace) instead of another hypothesis-driven patch.

**Stopping here deliberately.** Impact is shutdown-only, after a recording ends, the box stays
up, and the shipped image was never gated on it — while each iteration costs a ~30 min build
plus a Pi cycle and I have no evidence-grounded hypothesis left. The owner's queued
fork-vs-upstream study (§1b) is the higher-value work and starts now.
