...
## 2h. 🐞 ROOT-CAUSED: HDMI only updates **2.3×/s** — the damage path is DEAD under glamor

This is very likely why X still *feels* unpleasant even after today's 3.7× speedup, and it is now
...
**`damage calls = 0`.** `fbdevShadowUpdate` is never invoked, so 100% of presents come from the
300 ms *idle safety net* timer, always the whole 1080-row screen. 320 presents / 140 s = **2.3
presents per second**. So `gl-x11`'s 8.9 fps is what the client achieves — **the screen shows 2.3
new frames a second**, whatever the clients do.

**Where a present's 77 ms goes:** `glReadPixels` of the 8.3 MB screen 60.8 ms (137 MB/s) +
`write()` to `/dev/fb0` 16.2 ms (512 MB/s).

**One hypothesis tested and half-refuted:** the readback asks `GL_BGRA` from an RGBA8 texture, a
per-pixel CPU shuffle over 2.07 M pixels. `GL_RGBA` (no conversion) ran at **41.4 ms** — so **the
swizzle is 19.4 ms of every present** — but it made colours wrong ((108,76,77) mauve instead of
(79,81,109)), so `GL_BGRA` is correct and is reverted. Recovering those 19.4 ms means making
glamor's *render* path RGBA-consistent; the `glamor-rgba-upload` patch only fixed CPU transfers.

**The rate knob is zero-sum, so I did NOT ship it:**

| `FBDEV_FLUSH_MS` | presents/s | client fps |
|---|---|---|
| **300 (shipped)** | 2.3 | **8.88** |
| 16 | **12.8** | 3.10 |

A present blocks the single dispatch thread for 77 ms, so at 16 ms the server presents 78% of the
time and **every client slows 3×**. Better cursor latency, worse everything else — not a call to
make on my judgment. Restored and re-verified: 112.6 ms/frame, colours correct, mirror still fixed,
0 faults.

**➡ Next (clear, and the only real fix):** both knobs are dead ends while a present costs 77 ms for
all 1080 rows. Fixing damage fixes both at once — presents become *partial* (a 500-row band ≈26 ms,
an xclock tick ≈2 ms) **and** on-demand. Step 1 is one cheap probe inside `shadowBlockHandler` to
tell "block handler never runs" from "damage region always empty". Step 2 is presenting per Y-band
instead of per bounding box (`RegionExtents` is useless — six clients spanning y=0..1080 make the
box full-height every time). Doc §24.
