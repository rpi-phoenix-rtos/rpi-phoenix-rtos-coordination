...
Not all lightmaps fail — the spawn courtyard is correctly lit — so it is per-page /
per-area, and the stripes point at the same tiling/UIF family as the earlier
`should_tile` finding. **Next:** `r_lightmap 1` (draw lightmap only) to confirm the
texture itself reads black, then the V3D lightmap upload/tiling path.
