...
- **★ USB input re-gated 8/8 after the core rebuild.** The rebuild included
  ...
  worked.
- ⚠️ **Harness bug found: multi-trial `test-cycle-bench.sh` is broken on the BOOT-ONLY path.**
  `test-cycle-bench.sh N <label> --capture-secs S` (no `-- <cmd>`) is **terminated by SIGTERM
  after trial 1** (`rc=143`, reproduced twice, ~10 s after the first capture watchdog fires) and
  leaves the Pi powered on. The `-- "<cmd>"` psh-interact path is fine — it ran 2- and 3-trial
  benches repeatedly today. **Workaround: always give the bench a command**, e.g.
  `-- "/bin/mem"`, which boots to psh and captures the same enumeration evidence. I could not
  find the signal source (no group kills in the scripts; the netboot trap is a plain
  `EXIT INT TERM HUP`), so this is recorded rather than guessed at.
