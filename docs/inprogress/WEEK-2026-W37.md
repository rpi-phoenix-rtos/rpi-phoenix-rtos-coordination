...
🔄 **GATE STATUS — image re-cut `ce9ae9f0` (2026-09-08), HW re-gating IN PROGRESS.**
Why re-cut: the previous image could not reproduce two of the reel's segments — it had no
`browse` mode and no vkQuake demo hook (ports `6f08c26` vs the reel's `d7de9aa`). The new one is
current: ports `d7de9aa`, libphoenix `a595099` (upstream `execve("")`), `browse` present,
coordination recorded **clean** (no `+dirty`). Passed: contents gate (**safe to flash**) and the
QEMU structural boot check. HW gates for the six components are being re-run now — see below for
