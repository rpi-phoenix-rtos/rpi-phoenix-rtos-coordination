...
## 2c. ✅ X desktop responsiveness — root-caused and **2.5× faster**; it was our own patch

I withdrew my earlier explanation last turn. The real one: tracing what writes `/dev/fb0` showed
`fbdevFlushRegion` is the only writer of desktop pixels **and it is never called** — so
render-to-scanout genuinely works and the DDX present path is dead code here. The cost was never a
CPU copy; it was in the **upload** path.

Our own screen-flip patch did the Y-mirror the simplest way: **one `glTexSubImage2D` per row.**
Correct — and pathological here, because `Xphoenix-glamor-daemon` reaches the GPU through
`/dev/v3d-srv`, so **every GL call is an RPC**. A 640×480 `XPutImage` cost **480 round trips per
frame**. Now it flips into a packed buffer and does **one** bulk upload.

Measured on hardware, 0 faults, desktop correct with all six clients:

| | before | after |
|---|---|---|
| GL window | 1.0 updates/s | **2.5** |
| Game of Life | 12.2 gen/s | **17.0** |
| `rpi4-v3d` CPU (`top`) | 76.9 % | **14.1 %** |

The CPU figure is what confirms the *mechanism* rather than just the outcome — the daemon was
burning three quarters of a core servicing per-row RPCs. ⚠️ Honest scope: **2.5×, not the ~30×** that
would close the gap to full-screen in-process GL (35 fps), so the row loop was a real cost but not
the only one. Analysis doc §12–§17 for the wrong turns; the patch header carries the arithmetic.
