...
⚠️ **Owner call, and the bigger item:** libphoenix's 4 KiB default thread stack with no guard
page turns *any* port's large stack buffer into silent cross-thread corruption. Options: a
guard page by default, and/or a real default stacksize on MMU targets only (`PTHREAD_STACK_MIN`
is a POSIX floor, not a default). Not changed unilaterally — libphoenix also serves MCU targets.
