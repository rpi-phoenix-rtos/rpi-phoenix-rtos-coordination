...
> ⚠️ This image **predates the STK 1-fps clock fix** (`ports 6f08c26`, see §4) — SuperTuxKart on
> it still runs at 1 fps. The other four games and the X desktop are unaffected. Say the word and
> I'll re-cut; I have not, because the SD boot path can't be re-gated without the card reader.
