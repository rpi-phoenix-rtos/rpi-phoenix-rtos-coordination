...
⚠️ **HOST hazard, not a Pi or build problem — FIXED.** Three benches were killed mid-flight by
the host's low-memory watchdog, which reads *free* rather than *available* memory. Dropping
caches first was not enough: each bench trial re-syncs the rootfs tree, and the spike outran
the kernel's tiny default free floor (`vm.min_free_kbytes` = **66 MB**). Raising it to **2 GB**
(`sysctl -w vm.min_free_kbytes=2097152`) makes the kernel hold a real reserve and the benches
now run to completion. Still worth `drop_caches` before a long one. The Pi powered off cleanly
through its exit trap every time; no cycle was left holding the UART.
...
- **Re-bench on this exact image, in progress:** desktop ✅ **3/3** (0 faults, glamor up,
  5 clients in every trial — `imgdesk-T1` + `imgdesk2-T1/T2`); QuakeSpasm running; Q2 and
  vkQuake to follow. Until those land this is "gate passed on the parent build, two components
  re-verified" — not yet a fresh 5/5.
