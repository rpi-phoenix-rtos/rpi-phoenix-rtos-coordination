...
## 2c. ⚠️ WITHDRAWN: my explanation of the X desktop's ~1 fps was wrong

I told you the desktop was capped by a 300 ms full-screen presentation timer. **That is wrong** and I
am retracting it. Re-measured on a correctly built server: `fbdevFlushRegion` has exactly three call
sites and **all three are silent** — damage non-empty 0, damage empty 0, timer 0, and the timer's
one-shot "armed" banner never appears — while the desktop renders at 0 faults.

The silence was verified, not trusted (that failure mode has bitten this twice): all four probe
strings are in the binary that ran, alongside a control string known to appear in the log, and the
staged sha matches the build tree.

**So the DDX presents nothing**, and the ~8 MB read + 8 MB write per frame I blamed does not happen —
`glamor_phx_screen_readback` has only that one caller. Likely explanation: render-to-scanout is
actually working (the screen BO does take the scanout branch). But that contradicts
`v3d_gpu.c:22`/`:638`, which say the daemon leaves `W.scanout_pa` unset so "the default DRAM path
runs" — both cannot be true, and which is wrong is the next thing to find.

**Honest state: the ~1 update/s is measured; the cause is unknown.** Next = find what actually writes
`/dev/fb0` in the glamor + daemon-client configuration. No number from the old account is reusable.
Analysis doc §17.
