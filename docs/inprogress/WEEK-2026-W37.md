...
## 2b. ✅ YOUR BUG #5 ROOT-CAUSED — and it is **the same defect as bug #2**

Three probes, three refutations, then the cause. All eliminated by their own instrumentation, not by
argument: `glamor_spans` (0 hits on the screen pixmap), `glamor_copy` (**one** copy in a whole X
session, neither side the screen pixmap), and the presentation readback (**every** call is
`y0=0 rows=1080 H=1080` — a uniform whole-screen flip, so it cannot mirror one region).

**The cause is a size heuristic standing in for scanout identity**, in two independent places:

| | test |
|---|---|
| `st_atom_framebuffer.c:137` | `fb->Width >= 1024 && fb->Height >= 768` → force `Y_0_TOP` |
| `v3d_resource.c:143` | `width0 >= 1024 && height0 >= 768` → `V3D_CREATE_BO_SCANOUT` |

"Is this the scanout?" is answered by "is it ≥1024×768?". Right for the 1920×1080 screen pixmap,
wrong for any other FBO that large. The **BO** side turned out safe — the winsys refuses a second
claim, so no two pixmaps share the framebuffer's pages (that was my first mechanism, and reading the
guard killed it). The **orientation** side is unguarded: nothing ties the `Y_0_TOP` forcing to
whether the FBO actually claimed scanout, so a large *non*-scanout FBO renders inverted — and read
back through a uniform whole-screen flip, that is exactly a mirror.

**So #5 is not a separate bug from your #2**, and #2's entry already names the fix. The right
discriminator already exists and is already used in the same file: **`PIPE_BIND_SCANOUT`** — key both
tests off the resource's bind flags instead of its dimensions.
⏳ One measurement owed first (log every resource taking that branch during an X session, and whether
it claimed scanout — one refutes the mechanism, more than one confirms it). ⚠️ Then the fix is
**owner-gated**: it expires the 6-app gate and needs a re-verification pass. Analysis doc §10.
