...
**ROOT CAUSE: every path component resolved through nfs-fs costs ~5.5-7 ms, and paths are
resolved ONE COMPONENT AT A TIME.** `fileperf --stat` on paths of known depth:

| path | components via nfs-fs | stat avg |
|---|---|---|
| `/dev`, `/tmp` | 0 (re-bound mounts, resolved locally) | **118 / 169 us** |
| `/ramtmp`, `/stk-assets.tar.gz`, `/usr` | 1 | **~5 500 us** |
| `/ramtmp/x` | 2 | 10 902 us |
| `/ramtmp/x/f` | 3 | 17 742 us |
| `/usr/share/supertuxkart/data/stk_config.xml` | 5 | 35 953 us |

Dead linear, ~5.5-7 ms per component. `/dev` and `/tmp` are fast because the takeover
**re-bound** them (`nfs-fs: re-bound /dev (devfs port=3)`, `/tmp (tmpfs port=5)`) so they
resolve without an nfs-fs round-trip; `/ramtmp` was NOT re-bound, so even reaching that
ramdisk pays an nfs-fs lookup first — which is why the "ramdisk" looked slower than NFS
earlier.

STK: 5127 files x ~5 components x ~6 ms ~= **2.5 min of pure lookups**, before reading a
byte. That is the load time, and it taxes every deep-path workload in the system.

⚠️ **My previous entry said the per-component model was refuted — that was wrong, and the
fault was my test.** `fileperf --depth` padded paths with `./` segments; those are
normalised away before any round-trip, so the test could not detect depth by construction.
The flat result measured nothing. (Also retracted earlier this session: `cat`/`ls` loops
timing fork+exec, `cat`-through-bash understating throughput 10x, and a non-recursive tool
pointed at a tree.)

**Two independent things to fix, either of which would help:**
1. **~5.5 ms per LOOKUP RPC is itself far too slow** — local IPC does 0.12 ms (`/dev`), and a
   gigabit RPC round-trip should be well under 1 ms. Smells like the documented per-RPC
   stall class again.
2. **No lookup caching** — resolving a 5-component path with 5 network round-trips, every
   time, for every file.

**The cost is entirely in OUR CLIENT — the server is 110x faster.** Mounted the same export
on the Linux host with `actimeo=0,lookupcache=none` (so every stat is a real LOOKUP) and ran
the same tool:

| path | Linux client | Phoenix |
|---|---|---|
| `/usr` (1 component) | **49 us** | 5 500 us |
| 5-component path | **74 us** | 35 953 us |

The server answers a 5-component path in **74 us**. So the export, nfsd and the network are
fine; the ~5.5 ms/component is ours. (Caveat: the host talks to its own nfsd over the local
interface, so its RTT is ~0; a real Pi->host gigabit RTT is ~0.2-0.3 ms — still an order of
magnitude under what we pay.)

**Mechanism, with the code named.** `nfs/srv.c:398-404` already documents the underlying bug
and its workaround: *"the Phoenix socket poll() does NOT wake on data-ready — it blocks the
FULL timeout — so every RPC stalls ~100 ms... This is a targeted workaround; the general fix
is to make the lwip-port socket poll wake on readiness (benefits all poll/select apps)."*
It clamps `nfs_set_poll_timeout(nfs, 1)`. The lwip port does have a readiness path
(`port/sockets.c:835-855`: for a SINGLE `ftInetSocket` the kernel packs a block timeout so
the socket thread blocks in `lwip_select` and wakes on the netconn callback), and the kernel
fallback spin is `POLL_INTERVAL 20000` us (`posix/posix.c:49`), capped by the caller's
timeout. So libnfs ends up stepping in ~1 ms poll increments instead of waking on arrival,
which is the shape of a few ms per RPC.

⚠️ I have **not** yet attributed the exact 5.5 ms, so I am not calling this root-caused.

**POLL HYPOTHESIS IS DEAD.** Rebuilt with `nfs_set_poll_timeout(nfs, 5)` instead of 1: cost
went *down* slightly (1 comp 5500 -> 4665 us; 5 comps 35953 -> 30110 us), not up 5x. So poll
is not burning its timeout here — it wakes on readiness. Experiment reverted, core rebuilt,
source clean.

**Where the time actually is (new probe `tools/udprtt`):**

| measurement | value |
|---|---|
| raw UDP round-trip Pi -> host, 50 samples | **873 us** (best 802, worst 1024) |
| NFS stat, 1 component | 5 577 us |
| NFS stat, 5 components | 36 025 us (~7.2 ms/component) |

Each lookup costs **6-8x a bare UDP round-trip**, and the Linux client answers the whole
5-component path in **74 us** — one NFSv4 COMPOUND. Two INDEPENDENT gaps:

1. **Round-trips per path.** Linux packs the path into a single COMPOUND; our cost is dead
   linear in component count, i.e. ~one RPC per component — 5x the round-trips before any
   latency question.
2. **~5-7 ms per RPC against an 873 us wire floor** — ~5 ms of client-side overhead that is
   neither the network nor poll.

Even fixing (2) perfectly leaves a 5-component path at ~4.4 ms vs Linux's 74 us, so (1) is
the bigger win.

**TCP confound now closed — it is neither the protocol nor Nagle:**

| probe | avg |
|---|---|
| UDP ping-pong | 873 us |
| TCP with `TCP_NODELAY` | 922 us |
| TCP with Nagle left on | 926 us |
| NFS stat, 1 component | 5 595 us |

TCP ~= UDP ~= **~900 us** and Nagle changes nothing. So the wire floor is ~900 us on either
protocol, and a 1-component lookup carries **~4.7 ms of client-side overhead** that is not
the server (74 us), not the network, not TCP/Nagle, and not poll.

Secondary observation worth its own look: **~900 us for a 1-byte ping-pong is itself high** —
a Linux client on gigabit would be ~100-200 us — so our lwip path adds ~700 us before NFS is
even involved.

**Next:** read libnfs's lookup path and nfs-fs's resolve/lookup to confirm whether a path is
resolved component-by-component with one COMPOUND each, and whether any lookup/attr cache
exists. That decides between "batch the COMPOUND" and "add a lookup cache".
