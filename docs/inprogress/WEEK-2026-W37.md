...
⚠️ **`fileperf --stat` is now a timing-dependent measurement** — a hit or miss depends on
what touched the prefix in the last 100 ms, so a fast number proves nothing on its own. The
verdict is **STK load wall-clock** (thousands of asset opens under one warm
`/usr/share/supertuxkart/data/` prefix) plus the 5-game + desktop bench. This invalidates the
gated 15-boot-clean SD image: re-bench before cutting a new one.
