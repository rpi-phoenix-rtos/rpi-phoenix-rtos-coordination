...
⚠️ **THIS IS NOW THE HEADLINE ITEM, and it is an owner call.** libphoenix's 4 KiB default thread
stack with `guardsize = 0` does not just hide one bug — **fixing one caller simply exposes the
next**, which is exactly what just happened. Every port that puts a sizeable buffer on a thread
stack gets silent cross-thread corruption instead of a fault, and each instance costs a full
root-cause hunt. Recommended, in order:
1. **Guard page by default** (`guardsize = PAGE_SIZE`) — converts the entire class into an
   immediate fault at the point of overflow. This is the change that stops the bleeding.
2. **A real default stacksize on MMU targets only** (aarch64 etc.) — `PTHREAD_STACK_MIN` (256)
   is a POSIX *floor*; glibc defaults to 8 MiB.

Not applied unilaterally because libphoenix also serves MCU targets (armv7m/armv8m) where both
cost real memory — but the case is now much stronger than when it was one bug in one port.
