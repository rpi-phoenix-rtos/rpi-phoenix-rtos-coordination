...
⏭ **NEXT (picks up cleanly):** a refreshed demo image — the delivered `2940cc70` predates this fix.
Three follow-up devices commits (runtime-reset guard · cold-state probe off the racy FIFO · vcmbox
lookup budget) landed *after* the gate started, so they are **not** in the gated build. A rebuild
carrying them is running; the remaining steps are:
1. verify by content (`trace=0`, `refusing to read V3D MMIO`, `TRUE reset FAILED` present),
2. short re-validation — a `quakespasm -loadbench` storm plus one rendering game (`/usr/bin/quake2`,
   **not** the bare `yquake2`),
3. then `--variant sd`, gate the image, and deliver. ⚠ `--variant sd` overwrites the TFTP
   `loader.disk`, so do it **last** — netboot testing stops working until the next nfsroot build.
