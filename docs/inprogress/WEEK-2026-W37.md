...
## 4c. ★★★ ~100 ms PER open() — a general bug, not STK, and not NFS

Owner challenged my "STK is NFS-load-bound" story ("NFS now runs over fast ethernet, SD
will NOT be much faster — look at the asset loading code path"). He was right, and the
measurements now say something much bigger.

Spawn-free, one process (`tar`), same 243 files (`data/gui`, 3 MB):

| source | time | per file |
|---|---|---|
| NFS | 32 s | 132 ms |
| **RAM disk** | **26 s** | **107 ms** |

**Reading from a ramdisk costs ~107 ms per file.** NFS adds only ~25 ms on top of a
baseline that exists regardless of filesystem, so the cost is in the **per-file open path**
(libc / posixsrv / VFS), not in NFS. ~100 ms is the same signature as the documented
`poll()` stall that once cost ~100 ms per RPC (`nfs_set_poll_timeout(1)`).

That explains STK completely — 5127 files x ~110 ms ~= 9 minutes — and it silently taxes
**everything** that opens many files.

Healthy for comparison: `dd bs=1M` of a 50 MB pak = **2 s (~25 MB/s)**, matching the
documented 29.9 MB/s. Bulk I/O is fine; it is purely per-open.

⚠️ **Two of my own measurements were invalid and I retract them:** the first per-file
figures came from `cat`/`ls -l` in a shell loop, which **fork+exec once per file**, so they
timed process spawn; and a "2.8 MB/s NFS throughput" figure came from `cat` through bash
rather than `dd bs=1M`. The owner's pushback is what forced the valid measurement.

**Next:** find the ~100 ms in the open path. It is filesystem-independent, so NFS is out —
look at libphoenix `open()`, posixsrv's open/lookup handling and the VFS message round-trip
for a poll/select with a ~100 ms timeout. Scope caveat: the measurement isolates NFS out but
uses busybox `tar`, so it bounds the cost to the tar+libc+posixsrv+VFS path rather than
naming the exact frame yet.
