...
**Deliberately NOT started this turn:** these are core changes that would invalidate the
gated, 15-boot-clean SD image. Next heartbeat: implement #1+#2, rebuild `--scope core`,
re-measure with `fileperf`, re-bench the demo set, then cut a new image.
