...
| Quake II | T1 clean, running | |
| vkQuake | pending | |

**ntpclient/malloc-fault rate so far: 1 of 11 post-change boots** (vs 0 of 18 pre-change).
Under a true 1/11 rate, seeing 0 in 18 has probability ~18% — **still not significant**, so
...
memory corruption in the process, not a missing init. Static reading cannot get further;
the A/B is now the decisive step.
