...
⚠️ **GATE STATUS — corrected, I had this wrong twice.** I said "exactly 4 artifacts changed" and
later "quake3 + the X desktop keep their gate". Both false: the `--variant sd` cut (12:55–12:59)
rebuilt **essentially all of userland** — busybox, coreutils, the X apps, wmaker, python3, all
five games, `Xphoenix`. Source was unchanged for all of them (only STK's clock patch and the
reverted diagnostic differed), so nothing is *suspected* broken; what lapsed is the evidence.

Verified **on the image's own binaries** (mtime ≤ 12:59, tested after): **Quake III** q3dm1 today
0 faults + arena/lightmaps/HUD checked · **X desktop** 3 lifecycles 0 faults (glamor daemon never
rebuilt, X apps are the image's).
**NOT yet run on the image's binaries:** QuakeSpasm · Quake II · vkQuake · SuperTuxKart — each was
verified at ~11:1x–11:2x, *before* the 12:58–12:59 relink. Re-gating in progress; see below.
...
`artifacts/hdmi-video/20260908-135047-phoenix-rtos-rpi4-showcase.mp4` (126 s, 9.9 MB, 1080p30
H.264 High). Six titled segments: X11 desktop (Window Maker + xterm/xclock/xcalc on
glamor-accelerated X) · QuakeSpasm · Quake II · Quake III · vkQuake · the driven 4-kart
SuperTuxKart race. Titles show 4 s then clear, so nothing covers the HUD. Rebuild with
`./scripts/make-demo-reel.sh`. **Validated, not assumed:** full decode 0 errors, 3780 frames /
126.000 s, faststart OK, every segment frame-checked. Colour now standards-tagged (limited range
+ bt709) — it was deprecated full-range `yuvj420p` with an **SD** matrix on HD content, which some
players would render with wrong levels; A/B'd a frame first, visually identical.
⚠️ **Silent by design** — HDMI capture carries no Pi audio (PWM output goes to the 3.5 mm jack)
and that path is owner-gated. **I have NOT published it — your call.**
