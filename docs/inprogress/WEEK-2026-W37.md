...
**Next, cheapest first:** `r_ignoreGLErrors 0` (live) to un-silence `GL_CheckErrors`;
`imagelist` to confirm the atlas empirically; then instrument the TexEnv/active-unit state
cache — `GL_SetDefaultState` (`tr_init.c:1483`) does call `qglActiveTextureARB` directly
without updating `glState.currenttmu`, so cache and GL genuinely disagree after init, but I
found no site that re-desyncs them mid-gameplay, so that is a lead and not a conclusion.
