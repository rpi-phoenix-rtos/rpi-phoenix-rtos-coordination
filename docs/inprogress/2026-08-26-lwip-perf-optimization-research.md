...
### C. Shrink the credit‑RTT (the denominator of the socket model) — MEDIUM, cheap knobs
B widens the pipe; C makes each credit round‑trip cheaper/rarer.
* **C1 — `TCP_WND_UPDATE_THRESHOLD` (currently 5840).** Governs how much window must free
  before lwIP proactively emits a window‑update ACK, *independently* of the consumer's
  deferred `netconn_tcp_recvd`. Lowering it → window re‑advertised sooner → fewer sender
  stalls (at the cost of more ACKs); raising it → fewer credit round‑trips per MB. It sits
  directly on the critical path and is a one‑line stock knob. **Test both directions** — the
  interaction with `NETCONN_NOAUTORCVD` is subtle and undocumented; measure, don't assume.
  Cite: opt.h:1476‑1481.
* **C2 — bigger application reads (NFS `rsize`, socket recv buffer).** Each `recv()` credits
  the window once via `netconn_tcp_recvd`; larger reads credit more bytes per round‑trip →
  fewer round‑trips per MB → lower effective credit‑RTT amortized. App/NFS‑side, not lwIP,
  but it attacks the same term. (Ties to the memory note "PROFILE wakeup/IPC next.")
* **C3 — `LWIP_SO_RCVBUF`** is off; enabling changes recv accounting but is unlikely to help
  throughput. Low priority; mention only for completeness.
...
1. **§3‑A lightweight `SYS_ARCH_PROTECT`** — removes ~27 µs/frame → the biggest single jump;
   no external dependency. *(port change)*
2. **§3‑A2 static memp pools** — removes per‑frame malloc locking; complements A1.
3. **§3‑B/C window scaling + credit‑RTT knobs** — widen and speed the socket credit pipe.
4. **§3‑D HW checksum** — ≤10–20% once the GENET RX‑checksum bug is fixed.
