...
5. **P5 — strerror POSIX descriptions** (A2.4): ✅ **WIRED+COMMITTED (2026-08-21)** — analysis done (conformance fix, low-risk), errno.desc (78 entries, 1:1 with header) joined into string/Makefile errno.list gen, name-fallback, gaierr unaffected; libphoenix `e71331d` + test `56b1f7d` (string_errsign.strerror_posix_text). Generation logic verified on-host (offsets exact, blob 1833 B). ⏳ PENDING HW: `--scope core` rebuild + Pi test-libc-string run — BATCHED after the X11 ports build finishes (no concurrent .buildroot builds).
6. **P6 — lwip TCP gateway bug** (C3): 🔧 candidate (b) CLEARED + diagnostic prepped (2026-08-21). Source-read cleared the Phoenix glue: filter.c defaults to pfpass w/ no rules; the outbound SYN getting out proves LWIP_HOOK_IP4_ROUTE/ETHARP_GET_GW route off-subnet dests fine. Enabled lwip stats + `/dev/ipstats` on rpi4b (lwip `2323efd`, `-DLWIP_STATS=1`) → next Pi cycle localizes via snapshot→connect→snapshot diff (link.recv/ip.rterr/tcp.xmit deltas). Rides the batched core-rebuild+netboot cycle.
