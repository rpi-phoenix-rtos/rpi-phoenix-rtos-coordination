...
1. ✅/🔧 **quake2 (yQuake2)** — LAUNCHER LANDED (`quake2` = yquake2 -datadir /usr/share/quake2): now finds baseq2/pak0.pak (1106 files), loads ref_gl1 on V3D, fully initializes (no more crash-to-shell). VISUAL RENDER of the auto-demo still unconfirmed (black at 45s, likely mid NFS map-load — needs longer capture or a render-path check). WAS: does not run — — starts, but game DATA not found: it searches CWD (`/usr/bin/baseq2`) and dies at `GetPCXPalette: Couldn't load pics/colormap.pcx` → back to shell (no render). Data IS staged at `/usr/share/quake2/baseq2/pak0.pak`. ROOT CAUSE: basedir not set. FIX: launcher/basedir → `/usr/share/quake2` (yQuake2 `-datadir`). [in progress]
