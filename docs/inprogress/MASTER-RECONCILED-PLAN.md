...
3. **P3 — X11 ports migration** (C1) — **SIMPLIFIED by E4** (desktops paused → no glib2/pango yet): validate `xorg-libs` in-framework → **`xorg-fonts` glib-free tier DONE (7/7 built+staged, validated standalone)** → `xorg-server`/Xphoenix → rewire `xterm`/`windowmaker` off `/tmp` → app ports.
