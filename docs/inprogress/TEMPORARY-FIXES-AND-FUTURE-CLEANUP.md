...
- **Stage tag** (added 2026-05-04): each TD that maps to the strategic
  roadmap (`docs/done/roadmap-cache-ram-smp.md`) carries a `Stage:` field
...
- **Resolution requirements:**
  ...
  - Establish a rule that every marker added to test a hypothesis is
    ...
    [code-quality-and-upstreaming.md](../knowledge/code-quality-and-upstreaming.md)).
...
- **2026-05-29 progress + finding:**
  ...
  - Unmasking before fixing that abort regresses the boot (the handler
    ...
    wall. See `docs/done/2026-05-29-usb-reanalysis.md` and memory
...
- **Resolution requirements:**
  ...
  - Set firmware `total_mem` / `gpu_mem` config to the right
    values for our use, document in `docs/knowledge/host-macos-apple-silicon.md`.
...
- **2026-06-02 update — observability half RESOLVED (P2/P3).** Two TD-14
  ...
  `docs/done/2026-06-02-p1p2p3-postfix-10boot.md`):
...
- **External OS comparison (2026-05-03):**
  ...
  - Circle's Pi-focused bare-metal code also treats MMU/cache enable as
    ...
    and local mirror paths listed in `docs/knowledge/source-artifacts.md`.
...
`docs/done/roadmap-cache-ram-smp.md`. Stage order is
