...
2026-08-25 (session ~265 — wmaker THEMING fix: port now ships data + PNG (build running)):
 Decided to DO the wmaker theming completion (deferred last turn) — it's the clearest owner-valued tractable item. Confirmed libpng.a+png.h+libz available + WINGs Images.xpm in make-install. **Port edits (windowmaker/port.def.sh, UNCOMMITTED pending verify):** (1) `--prefix=/usr --sysconfdir=/etc` so wmaker's compiled DATADIR/SYSCONFDIR are TARGET paths (were build paths → data never found on target); (2) `--enable-png` + `-lpng16` in the link (swback/logo are PNG); (3) `make install DESTDIR=<stage>` + stage /usr/share/{WindowMaker,WINGs} + /etc/WindowMaker into PREFIX_FS/root (port previously shipped ONLY the binary → wmaker fell back to bare compiled defaults). Reverted the dbg-watchdog instrumentation from the workdir first (clean wmaker). GOTCHA: first attempt with `--prefix=/usr` FAILED — configure added `-I/usr/include` (the HOST glibc headers) to wrlib → `bits/wordsize.h` fatal in the cross-build. FIX: keep `--prefix="${PREFIX_PORT_INSTALL}"` (build path → includedir stays the cross one) and use `--datarootdir=/usr/share --sysconfdir=/etc` for the TARGET data paths only. Rebuild v2 running (monitor bpy70cs0q; make clean'd first; font stack cached).
 **DONE (ports 5b13622 PUSHED + HW-verified):** rebuild v2 clean; staged themed wmaker + data to -gcc16; `startx` runs, renders menus, and the "could not load widget images" warning is GONE (WINGs Images.xpm now found via DATADIR=/usr/share/WINGs). configure confirms "Supported graphic format libraries: XPM PNG builtin-PPM" (PNG codec built + linked). ⇒ port-completeness fix: the image now ships wmaker's config/data + PNG support (was binary-only). **HONEST OUTCOME:** the VISIBLE desktop is unchanged from the core-fix state (root menus render, but black root + no dock visible) — default-WindowMaker plainness + a dock/WMState config detail, NOT a hang. Residual decorative warnings: swback.png "could not find" (SwitchPanelImages search-path detail — the file IS at /usr/share/WindowMaker/Pixmaps) + logo "format not supported" (likely a TIFF logo; TIFF stays disabled). These are decorative config quirks — DEFERRED (diminishing returns; core wmaker works). README unchanged (already says wmaker works, no hang caveat — accurate).
 **NEXT:** (if owner wants full theming) chase the dock/WMState + swback search-path + a default Style/background — decorative. Otherwise: Quake2 mip-vs-lightmap (owner gl_texturemode); g34 release boot-verify.

2026-08-25 (session ~263 — ✅✅✅ WMAKER HANG FIXED + HW-VERIFIED via dbg-watchdog backtrace (no guessing)):
 Root-caused + FIXED the WindowMaker startup hang the owner reported. **Method (owner's ask — use the debugging facility):** built wmaker w/ `dbg_arm_watchdog(20)` (tools/dbg-probe/dbg.c, #include'd into main.c) + full clean rebuild w/ `-fno-omit-frame-pointer`; the SIGALRM watchdog dumped a walkable x29 backtrace of the hung process; `aarch64-phoenix-addr2line` symbolized it EXACTLY:
   `wScreenInit → WMCreateFont → XftInit → FcConfigEnsure → FcConfigBuildFonts → FcFileScanFontConfig → FcFreeTypeQueryAll → FT_New_Face → FT_Stream_New → sys_open` (blocked).
 **ROOT CAUSE:** wmaker's first Xft font load makes fontconfig build its cache by scanning `--with-default-fonts=/usr/share/fonts`, which holds **412 X11 core PCF bitmap fonts** (staged for the X server -fp path) — fontconfig FT_New_Face-opens EVERY one, glacial over NFS (>>45s = looked like a hang; 2 watchdog ticks had different PCs = grinding through files, not deadlocked). (dillo worked because it hit a warmed/simpler fc path.) **FIX:** point fontconfig at the SCALABLE dir only — image: xorg_fonts/port.def.sh `--with-default-fonts=/usr/share/fonts/truetype` (ports **de63acf** PUSHED); runtime -gcc16: fonts.conf `<dir>`→truetype. **HW-VERIFIED:** clean wmaker (no watchdog) now reaches its event loop (sys_poll in WMNextEvent) and **RENDERS the WindowMaker root menu + Applications submenu** on HDMI (both startx and startx_gpu). Non-fatal residue: missing widget/logo PNGs (swback.png/logo — /usr/share/WindowMaker absent on the drifted -gcc16 root; the image ships them; wmaker renders fine without).
 - Also confirmed wmaker hangs identically on BOTH `startx` (fbdev) + `startx_gpu` (glamor GPU) pre-fix ⇒ WM-internal (server-independent), and Xft/freetype/fontconfig themselves work (dillo renders antialiased text). README updated: removed the wmaker-hang caveat, bare startx/startx_gpu → WindowMaker (working default).
 **REUSABLE LESSON:** for a HANG, don't guess — `dbg_arm_watchdog` + `-fno-omit-frame-pointer` full rebuild + addr2line gives the exact stack in one shot. [[project_libdbg_facility]] [[project_x11_lib_port]]
 **NEXT:** (polish) Quake2 mip-vs-lightmap discriminator (owner-runnable gl_texturemode); g34 release boot-verify.
 - **wmaker THEMING follow-up (deferred — decorative; core wmaker WORKS via GNUstep config):** the windowmaker port ships ONLY the binary + configures `--prefix=$PREFIX_PORT_INSTALL` (a BUILD path) so wmaker's compiled DATADIR/SYSCONFDIR point at build paths absent on target → no /usr/share/WindowMaker data (Styles/Pixmaps/Icons/Themes) on the image; wmaker falls back to compiled defaults + the GNUstep root-menu. `make install DESTDIR=<stage>` DOES produce the full data (310 files: /etc/WindowMaker menus+defaults, /share/WindowMaker Themes/Styles/Pixmaps/Icons/Backgrounds, /share/WINGs widget pixmaps). PROPER FIX (when desired): configure `--prefix=/usr --sysconfdir=/etc`, add `make install DESTDIR=<stage>` + stage stage/usr/share/{WindowMaker,WINGs} + stage/etc/WindowMaker into PREFIX_FS/root, rebuild+stage. NOTE: `--disable-png/jpeg/tiff` means the switch-panel bg (swback.png) + logo (PNG) still won't load (would need libpng linked + --enable-png); XPM icons/styles WILL load (-lXpm is in the closure). So even the full data gives XPM theming, not the PNG switch-panel/logo.

2026-08-25 (session ~262 — wmaker hang = WMAKER-INTERNAL (both servers); Xft RULED OUT (dillo renders); instrumenting w/ dbg watchdog):
 Owner asked to test wmaker on GPU + non-GPU X and (correctly) to STOP guessing — use the debugging facility / instrument. Results:
 - **wmaker hangs on BOTH `startx` (fbdev) AND `startx_gpu` (glamor GPU)** — both black+cursor, mouse works, X server + glamor fully up (glamor initialised, GPU root, CL submits). ⇒ the hang is **wmaker-INTERNAL, server-independent** (not fbdev, not GPU).
 - **Xft/fontconfig/freetype RULED OUT:** dillo (FLTK→Xft) renders its full Quickstart page with crisp ANTIALIASED text under Xphoenix → the base font stack WORKS. So wmaker's hang is a WINGs/libwraster step specific to WindowMaker, NOT plain Xft. (My earlier Xft-hang lean was wrong — good that the owner pushed to instrument not guess.) Localized region: `wScreenInit()` (screen.c ~707 WMCreateScreenWithRContext / ~781 WMBoldSystemFontOfSize / RContext / dock), between config-read (startup.c:588) and manageAllWindows (663).
 - **Debugging facility = `dbg_arm_watchdog(secs)`** (tools/dbg-probe/dbg.c — SIGALRM→backtrace of the hung proc; vkQuake used it w/ -fno-omit-frame-pointer). PLAN: build the font stack + wmaker (running: build-port.sh windowmaker, monitor b0quidqw3), then inject `dbg_arm_watchdog(15)` into wmaker main.c + compile/link dbg.c + -fno-omit-frame-pointer, re-make, run → the watchdog dumps the EXACT hang location (addr2line) → fix. NOTE: wmaker config already corrected (phxfile→Sans, /var/cache/fontconfig created) — not the fix, but correct.
 - README caveat KEPT (wmaker still hangs, per owner's if-hangs-keep instruction). startx_gpu (GPU X) DONE + HW-verified last session; twm sessions (startx term/desktop, startx_gpu term) all render fine.
 **NEXT (monitor bsahvrxp9 fires on wmaker build done) — TURNKEY instrumentation recipe:**
   1. WORKDIR = `.buildroot/_build/aarch64a72-generic-rpi4b/port-sources/windowmaker-0.95.9/WindowMaker-0.95.9`
   2. `cp tools/dbg-probe/dbg.c $WORKDIR/src/wmdbg.c` (dbg.c is #include-safe: no main; funcs walk_fp/dbg_backtrace/watchdog_handler/install are static or unique; externs `_dbg_signal_ctx`/`_dbg_signal_pc` come from libphoenix's patched signal trampoline — vkQuake linked it fine).
   3. In `$WORKDIR/src/main.c`: after the includes add `#include "wmdbg.c"`; in `real_main()` (line 604) just before `StartUp(!multiHead)` (line 798) add `dbg_arm_watchdog(20);`.
   4. Re-make ONLY wmaker (font stack already built): `make -C $WORKDIR CFLAGS="<the port's cf> -fno-omit-frame-pointer"` (grep the port.def.sh `cf=` / the build log for the exact CFLAGS; MUST add -fno-omit-frame-pointer for the x29 walk).
   5. `cp $WORKDIR/src/wmaker /srv/phoenix-rpi4-nfs-gcc16/bin/wmaker` (stage to the netboot root).
   6. `test-cycle-psh-interact --label wmaker-bt --idle-secs 45 -- "startx"` → at ~20s the watchdog prints `dbg: *** WATCHDOG *** ... #NN 0x<addr>` backtrace of the HUNG point.
   7. Symbolize: `aarch64-phoenix-addr2line -f -e $WORKDIR/src/wmaker <addrs>` → exact function/line of the hang in wScreenInit → FIX that.
 (dillo already proved Xft/freetype/fontconfig work, so expect the hang in libwraster/RContext/dock or a WINGs non-font step — the backtrace will name it.)

2026-08-25 (session ~260 — ✅ startx_gpu (GPU-accelerated X) DONE + HW-verified; Quake2 autonomous repro not tractable):
 Owner asked for a `startx_gpu` mirroring `startx` but launching the experimental GPU-backed X. **DONE + HW-PROVEN (launcher ca16eec):** added argv[0]-keyed `startx_gpu` to pl_phoenix_xlaunch — it (1) starts the rpi4-v3d GPU daemon if /dev/v3d-srv absent + waits for it, (2) uses Xphoenix-glamor-daemon (V3D glamor 2D accel) instead of software fbdev Xphoenix; all session modes identical to startx. build-xlaunch.sh stages the binary under 3 names (pl_phoenix_xlaunch/startx/startx_gpu). **`startx_gpu term` HW-verified:** rpi4-v3d registered /dev/v3d-srv, glamor-phx GL up (Mesa 26.2.0/V3D 4.2.14.0), GPU-texture-backed root, twm+xterm rendered via CL submits rc=0, HDMI-confirmed (olive twm titlebar + ash shell). Staged to -gcc16. README updated (startx_gpu + recommend `startx term`/`desktop`; bare startx→wmaker hangs).
 - **Quake2 texture bug: autonomous repro NOT tractable.** Tried to drive quake2 to an in-game view (`+toggleconsole`) — console stays down, and the harness has NO input path into the running game (can't navigate to the viewpos or toggle gl_texturemode). quake2 RUNS (Outer Base/demo1 loads). ⇒ the mip-vs-lightmap discriminator (`gl_texturemode GL_LINEAR` at the viewpos) is owner-runnable in seconds; I can't. Left for owner OR a future demo-playback-capture setup.
 - **wmaker hang NARROWED to the Xft font-open path (session ~260 cont.):** ruled OUT the phxfile scheme (the staged .PREV-cruft config used `phxfile:...ttf` which the current wmaker binary doesn't grok — its built-in fonts are Xft names "Sans:pixelsize=12"; converted the -gcc16 config phxfile→Sans, STILL hangs) and ruled OUT a missing/slow fontconfig cache (created /var/cache/fontconfig + 90s idle, STILL hangs — hard hang, not slow). Decisive isolation: **xterm is built WITHOUT Xft** (`bad command line option "-fa"`) → it + twm use CORE X fonts (why they render); **wmaker is essentially the ONLY Xft client** and it hangs exactly where it opens Xft fonts. ⇒ leading root cause = Xft/fontconfig/freetype `XftFontOpenName("Sans")` hangs on Phoenix (untested until wmaker exercised it). CONFIRM via a minimal Xft test prog (XOpenDisplay + XftFontOpenName + draw) OR wmaker instrumentation; then fix the Xft/fontconfig/freetype gap. (-gcc16 wmaker config left improved: Sans fonts + fontconfig cache dir.) Default X = software fbdev (not GPU) — answered.
 **NEXT:** owner runs the gl_texturemode discriminator → fix Quake2 mip or lightmap path in external/mesa v3d + rebuild libv3d (+ gate the false winsys diagnostic). Debug wmaker font-load hang (WINGs instrumentation). g34 release boot-verify queued.

2026-08-25 (session ~258 — Quake2 tiling-hypothesis REFUTED (false-positive diagnostic); wmaker hang = wmaker-specific; twm/xterm render great):
 Used the freed Pi for diagnostics (owner couldn't test). THREE findings:
 - **Quake2 texture bug — my 64-wide LINEAR tiling lead was WRONG (subagent-proven false positive).** The winsys `vcheck VERTICAL-MISMATCH`/`TILING=LINEAR!` lines come from the probe running on IN-PLACE mipmap-gen TFU jobs (src==dst pointer → the "LINEAR" verdict is structurally guaranteed, comparing a word to itself; v3d_phoenix_winsys.c:1476-1548). At the driver level all these textures are correctly UIF_NO_XOR and the TFU dest-tiling + TMU sampler descriptor AGREE (v3d_resource.c:704-752, v3dx_tfu.c, v3dx_state.c:986-997). ⇒ NO tiling mismatch. The real bug (malformed ground rectangles) is either (a) bad TFU-generated MIP levels or (b) the lightmap glTexSubImage2D partial-upload path — the probe reads level-0 only + there are ZERO raster-source TFU jobs in the log, so it never tested the base uploads. **DISCRIMINATOR (owner-runnable, trivial): at the viewpos set yquake2 `gl_texturemode GL_LINEAR` — rectangles vanish → mip path (a); persist → lightmap path (b).** I can't run it myself (no input path to the running game; quake2 comes up at the console + I can't drive it to the view). quake2 itself RUNS fine (Outer Base/demo1 loads). The misleading winsys diagnostic should be gated on a raster source (subagent gave the exact 2-line fix) — do with the eventual real fix's libv3d rebuild.
 - **wmaker black screen = WMAKER-SPECIFIC (discriminator done).** `startx term` renders PERFECTLY (xterm + twm titlebar + ash shell, HDMI-confirmed) and `startx` modes now find the server (my /usr/bin/Xphoenix fix works). So the fbdev X stack + twm + core-font clients are fine; wmaker alone hangs after config-read, before painting (no dock). wmaker uses Xft/`phxfile:` TTF fonts (WindowTitleFont=phxfile:.../DejaVuSans-Bold.ttf) while twm/xterm use core X fonts — the hang is almost certainly WINGs' font load (phxfile/freetype). Needs a backtrace of the hung process to pin (wmaker isn't libdbg-linked → rebuild-with-prints or a signal backtrace). ⇒ **USE `startx term` / `startx desktop` for a working X desktop now**; bare `startx` (wmaker) hangs.
 - **Default X11 is SOFTWARE, not GPU** (owner asked): Xphoenix = kdrive fbdev DDX built `--disable-glamor --disable-dri --disable-glx` (CPU shadow-blit to /dev/fb0). GPU-accel = the opt-in experimental `Xphoenix-glamor` build.
 **NEXT:** owner runs the gl_texturemode discriminator (or I set up a demo-playback capture to reach an in-game view) → fix mip or lightmap path in external/mesa v3d + rebuild libv3d + gate the false diagnostic. wmaker: get a hung-process backtrace (rebuild WINGs w/ font-load prints). g34 release boot-verify still queued.

2026-08-25 (session ~256 — OWNER live-tested: bash WORKS ✅, Quake2 WORKS ✅ (minor texture bug → digging); startx→wmaker shortcut + README done):
 Owner (on the Pi) confirmed **interactive bash works** and **Quake2 runs smoothly**, with one bug: some GROUND textures render as malformed rectangles (baseq2 demo1, viewpos `-141 974 -81` angles `67 133 0`; artifacts/rpi4b-uart/20260825-135418-live-test.log). **ROOT-CAUSE LEAD (strong):** the V3D TFU texture upload lays out **64x128 & 64x64 RGBA8 textures as `TILING=LINEAR!`** with `vcheck VERTICAL-MISMATCH xor=0 match=2/6`, while 32x32 come out `UIF-VERIFIED match=5/5` — i.e. certain small-but-64-wide textures get the wrong tiling vs what the TMU sampler expects (same CLASS as the q3dm7 UIF_XOR bug [[project_quake3_lightmap_uif_xor]], different size regime). Dispatched a subagent to pin the exact tiling-decision path (v3d_resource.c v3d_setup_slices thresholds) vs the sampler-descriptor tiling (v3dx_state.c) — is LINEAR legit for these levels (sampler agrees) or a real mismatch? Fix will be in external/mesa v3d + libv3d rebuild + HW re-test at that viewpos. ACTIVE DIG.
 - **startx→wmaker shortcut DONE + a real bug fixed (launcher d61a1fb):** bare `startx` now launches Window Maker; `startx desktop/term/deskapps` = twm sessions. Fixed the server-path bug the subagent flagged — `startx <mode>` hardcoded `/bin/Xphoenix` but the framework image stages it at `/usr/bin/Xphoenix`, so EVERY startx mode failed to find the server; now stat()s /usr/bin first. Staged live to -gcc16 + provisioned the drifted root (fonts, /etc/fonts fontconfig, WindowMaker config incl. WMWindowAttributes). ⚠️ **UNRESOLVED:** my HDMI *capture* shows black for wmaker AND twm (every run logs `HDMI1:EDID error`) — can't tell render-regression vs capture failure from here; asked owner what the real monitor shows.
 - **README DONE (6e140fa):** added "Running the showcase apps" (rpi4-quake, startx desktop, mc/nano w/ TERM=vt100) + fixed the stale bash caveat (interactive bash now works). FLAGGED to owner: Quake II/III engines are NOT built by the default --with-showcase flow (only their data is fetched; the engines exist only on the hand-maintained netboot export) — offered to wire them in.
 **NEXT:** on subagent return → advisor on fix approach → patch external/mesa v3d tiling → rebuild libv3d → HW re-test Quake2 at the viewpos. Then (owner q's) wire Q2/Q3 into the default build; confirm startx render on the real monitor.

2026-08-25 (session ~254 — interactive bash RIGOROUSLY re-verified: pipes/loops/vars/conditionals/cmd-subst all work):
 No owner feedback; owner idle ~1h+ (back in away-mode); g34 still building; Pi free. Did the thorough interactive verification I should have done originally (over-claimed twice before). test-cycle bash-rigorous — all features execute INSIDE bash at the `bash-5.2#` prompt, 0 faults: **pipe** `echo a b c | cat`→`a b c`; **loop+var** `for i in 1 2 3; do echo row-$i; done`→row-1/2/3; **var** `V=phoenix; echo greet-$V`→greet-phoenix; **conditional** `true && echo OK`→OK; **cmd-subst+pipe** `echo $(echo inner | wc -c)`→6; then `exit`→psh, psh still works. ⇒ interactive bash is a FULLY FUNCTIONAL shell on Phoenix (pipes notable — psh itself has none). The owner's #1 long-standing bug is genuinely + comprehensively resolved. g34 release rebuild (Monitor brigbpp1o) still in the ports/showcase stage; will carry the same bash+libphoenix.
 **NEXT:** g34 export → interactive-verify the release image's bash (same binary/source, confirms the artifact) → G5 public "released" declaration (held for owner touch / full-image SD-boot residual). Deferred: sync-netboot-tree export-target fix.

2026-08-25 (session ~253 — interactive-bash fix captured in memory; g34 release rebuild carrying it; staying off the Pi for owner re-test):
 No owner feedback; owner hasn't re-tested yet (newest UART log is my ~252 interactive verify). Interactive bash is FIXED + HW-verified (~252) and the fresh bash is staged on the -gcc16 netboot root with the server up — owner can re-test anytime. This turn (owner may grab the Pi to re-test → I stayed OFF the Pi + netboot infra): captured the two hard lessons durably in memory [[project_bash_port]] + MEMORY.md — (1) verify interactive shells by running commands INSIDE the shell, never `bash /script` (bypasses readline's select path, PASSES on broken interactive); (2) `--scope core` after ANY committed libphoenix change (the select fix sat stale in the 08-22 buildroot for 3 days → shipped broken to the owner). g34 release rebuild (with bash in ports.yaml + fresh libphoenix) advancing through the ports stage; Monitor brigbpp1o armed → when it exports I'll extract its bash + interactive-verify the release image carries the fix.
 **NEXT:** on g34 export → stage its bash to netboot + interactive-verify → then G5 public "released" declaration (still held for owner touch / the full-image SD-boot residual). Follow-up (deferred, netboot-infra, do when Pi idle): fix sync-netboot-tree targeting the default export vs the -gcc16 the Pi mounts (root cause of the drift that forced hand-staging bash).

2026-08-25 (session ~252 — ⚠️ OWNER-REPORTED: interactive bash STILL broken; root cause = STALE libphoenix.a in gcc-16 buildroot; fixing):
 Owner live-tested over netboot (artifacts/rpi4b-uart/20260825-132244-live-test.log) and interactive bash still EOF-exits: `bash` → `bash-5.2#` prompt → instant `exit` (bash auto-prints "exit" on EOF; the owner typed nothing between). Same for `bash -i`. **I OVER-CLAIMED** — my session-~251 "HW-verified" ran bash NON-interactively (`bash /script`), which never touches readline's `select(NULL)` path, so it masked this. **Root cause (confirmed, not a code bug):** the select() fix (libphoenix 033ee1f) was committed **2026-08-25 11:14**, but the gcc-16 buildroot's `libphoenix.a`/`select.o` are from **2026-08-22 16:45** — 3 days OLDER. The committed core change never got the mandatory `--scope core` rebuild into the gcc-16 buildroot (the exact stale-core hazard CLAUDE.md warns about), so bash linked the OLD select (returns 0 immediately → readline sees EOF → exit). ash/psh work because they don't use the select(NULL) readline path. **✅ FIXED + INTERACTIVELY HW-VERIFIED:** `--scope core` regenerated libphoenix.a/select.o (now 2026-08-25 13:29, with the fix) → rebuilt bash against it → staged to the -gcc16 netboot root → **interactive test PASSED** (test-cycle-psh-interact bash-interactive): sent `bash`, then `echo HELLO_FROM_BASH_INTERACTIVE`→printed, `pwd`→`/`, `echo count=7`→`count=7` (all executed INSIDE bash, prompt stayed `bash-5.2#`), then `exit`→back to `(psh)%`, `echo BACK_IN_PSH`→ran in psh. bash stays at its prompt, runs interactive commands, exits only on explicit exit — the owner's bug is genuinely resolved. The select fix was correct all along; it just never got compiled into the gcc-16 buildroot (stale-core hazard). LESSON: interactive-shell fixes MUST be verified with an actual interactive send-command-inside-shell test, never `bash /script` (which bypasses readline's select path). The g34 Docker image (fresh libphoenix from master) will carry the working bash. Owner can re-test over netboot now (fresh bash staged; server up).

2026-08-25 (session ~251 — ✅ bash NOW SHIPS in the image + builds under gcc-16; rebuilding the release artifact with it):
 No owner feedback; Pi free. Closed the top release follow-up — bash was in the headline but not the image. **Root-caused the gcc-16 bash build break:** `build-port.sh bash` failed at unwind_prot.c ("too many arguments to function 'function'" + incompatible-pointer-type on without_interrupts) — gcc-16 defaults to **C23**, where a K&R `type function();` decl means `(void)` not "unspecified args", so bash's cleanup-fn typedefs error. The framework `-std=gnu17` pin wasn't reaching bash's target CFLAGS (only host build-tools got -std=gnu89). **FIX:** pinned `-std=gnu17` in bash/port.def.sh's own CFLAGS (ports **751dda7**, pushed) — clean rebuild produced /bin/bash (1.35MB aarch64 static ELF). **Wired bash into the image:** `- name: bash` in rpi4b ports.yaml (project **1ed20f0**, pushed, default if:true like coreutils/busybox). **HW-verified on the Pi** (staged to the -gcc16 NFS root + netboot): bash 5.2.21 runs a script — arithmetic (calc=42), for-loop, `$BASH_VERSION`, and `bash --version` (aarch64-unknown-phoenix) — 0 faults. Updated the release-notes bash bullet (follow-up resolved).
 - **Release artifact rebuild kicked off:** the G3.2 image (sha 29e20cd8) predates the bash entry, so it lacks bash. Relaunched the Docker release build (g34) to mint the final with-bash reproducible artifact + validate bash in the FULL release build (not just standalone). Monitor armed; verify next turn (bash present + G3.3 spot-check → new sha).
 **NEXT:** verify g34 (bash in the image + G3.3 userspace spot-check on the new sha) → then G5 remaining = the public "released" README flip + artifact publish (still held for the full-image SD-boot residual / owner touch). Other follow-up: sync-netboot-tree vs -gcc16 export mismatch.

2026-08-25 (session ~250 — ✅✅✅ G3.2 GREEN: the clean gcc-16 reproducible Docker build SUCCEEDED; now at G3.3 boot-verify):
 No owner feedback; Pi free. **G3.2 PASSED** — the `--no-cache` gcc-16 Docker release build (build-sd-in-docker.sh g33) completed end-to-end and exported `docker-out/rpi4b-sd-2part.img` (Aug 25 12:48, 835 MiB, **sha256 29e20cd84677f5eb9e536a89c5e0e1f264665b98f55ad6ad6131288223c48945**). All the reproducibility fixes held — the build cleared the freetype step that killed g32 (fetched via the SourceForge mirror), then built the full X11/glib2 showcase + games. ⇒ **the gcc-16 + binutils-2.47 image builds reproducibly from a clean container against my committed sibling state.**
 - **G3.3 boot-verify — path DECIDED by inspecting the image:** loop-mounted p1 (FAT boot). config.txt loads `loader.disk` as initramfs at 0x08000000 (generic), BUT root selection is baked INSIDE loader.disk, and this is a `--variant sd` build ⇒ loader.disk mounts the ext2 SD root (mmcblk0p2). So the image genuinely wants **SD boot** — netbooting it card-less would fail root-mount. Authoritative G3.3 = self-flash the image to the Pi's 64GB card (present) via the netboot-Linux path, then SD-boot + verify. This is a delicate destructive Pi op → evaluating the safest unattended automation (advisor consult in progress) before executing. Boot artifacts confirmed present + gcc-16-dated: kernel8.img, loader.disk (21.7MB), bcm2711-rpi-4-b.dtb, armstub, start4.elf/fixup4.dat, overlays — all Aug 25 12:34.
 - **G3.3 userspace HW-verified (advisor-guided, card-absent branch):** the advisor flagged my card-presence fact as STALE — confirmed via a netboot log `sdcard: no card present in slot 0`, so self-flash SD-boot is impossible unattended (the 2026-08-06 "no card" memory was right, not the 2026-07-02 doc). Sanity-checked the image (loop-mounted p2: complete 213M root — rpi4-quake/Xphoenix/wmaker/dillo/coreutils/redis/sqlite/lua/curl + quake1/2/3 data; **but NO /bin/bash** — bash was never wired into ports.yaml, built ad-hoc in the fix arc). Then did the non-destructive userspace verify: staged 4 docker-built binaries onto the gcc-16 NFS root (fsid=0 = /srv/phoenix-rpi4-nfs-gcc16, NOT the default — first attempt staged to the wrong dir) and ran them on HW — **`cksum`→231799749/202168 (no Data Abort), `od`→correct ELF magic, `sha256sum`→bfd32571… BIT-EXACT vs host, `lua -v`→Lua 5.4.7; 0 faults.** ⇒ the reproducible artifact's binaries run correctly on real HW + the ustack Data-Abort fix is present in the release image. Cleaned up staged files. Full-image SD-boot of the exact artifact BANKED (no card) — mitigated by the same-source gcc-16 kernel/loader already HW-boot-verified 2026-08-22.
 - Snapshotted the release integration manifest (manifests/2026-08-25-gcc16-release.md) + finalized the release-notes draft (gates checked, honest verified-vs-banked G3.3, bash bullet reworded: the select() fix is in base libc → benefits shipped ash/readline apps; bash-in-image is a follow-up ports.yaml entry).
 - **G5 rollback anchors DONE:** tagged + pushed `known-good/gcc16-release-2026-08-25` across libphoenix (033ee1f), phoenix-rtos-ports (a761b2d), phoenix-rtos-build (643293e), phoenix-rtos-tests (ca616da), coord (6b6ec63). The release state is anchored for deterministic rollback.
 **NEXT (G5 remaining = the PUBLIC "released" declaration, deliberately HELD):** org README flip to "gcc-16.2.0 released" + publish the image artifact (sha256 29e20cd8…). Held because the release-notes' own gate said "publish after BOTH gates pass" and G3.3's full-image SD-boot is the one banked residual (no card) — flipping the public README to "released" is a strong one-way declaration best made once the full-image boot closes OR with a light owner touch. Everything up to it is done + pushed (G3.2 green, G3.3 userspace-verified, manifest, notes, rollback tags). Follow-ups: wire bash into ports.yaml so the image ships the shell the headline names; fix the sync-netboot-tree vs -gcc16 export mismatch (sync writes /srv/phoenix-rpi4-nfs, Pi mounts -gcc16). Both build Monitors fired + ended.

2026-08-25 (session ~247 — G3.2 Docker build FAILED on a freetype CDN outage → mirror-fallback fix + relaunched):
 No new git owner feedback; Pi free. The session-~246 G3.2 Docker build (docker-release-g32.log) **FAILED**, not hung: `#10 freetype-2.13.2: fetch attempt 1/3..3/3 failed` → `rebuild-rpi4b-fast.sh ... exit code: 1`. Root cause = download.savannah.gnu.org (freetype's canonical host) down for hours (curl timeout; its nongnu mirror 502'd), and `tools/x11-port/build-x11-phoenix.sh`'s `fetch_extract` was **single-URL** (unlike the xorg_* port.def.sh, which I gave mirror+cache earlier this arc). The `cp: ... are the same file` line I first suspected was a red herring — build-libffi.sh runs `set -u` not `set -e`, so that self-cp is harmless.
 - **Fix (coord c2fcfa5):** `fetch_extract` now takes a whitespace-separated **mirror list** (tries each twice) + consults/populates a persistent **distfiles cache** ($PHOENIX_DISTFILES, default ~/.phoenix-distfiles/x11) — same reproducible-build hardening already on the xorg ports. freetype now leads with two verified-reachable mirrors (SourceForge, mirror.kumi.systems, both served the correct 3.87 MB tarball) and keeps savannah last. Pre-cached freetype into the host cache too. `bash -n` clean.
 - **Relaunched** build-sd-in-docker.sh detached (pid 412966, log docker-release-g33.log) — it re-serves committed HEAD so it picks up the fix. Verified all pre-freetype fetches (zlib/libpng/jpeg/xcb-util/x.org libs) succeeded last run, so freetype was the only down host; if x.org/xcb go down mid-build next, the mirror infra is now in place to extend.
 - **G5 de-risk (while the build bootstraps the toolchain):** verified every release tag-target repo is fully pushed to `publish` — libphoenix/tests/build already clean; **phoenix-rtos-ports had 4 unpushed, verified bash-port commits** (config.cache env-cache-strip + siginterrupt=yes + gcc-16 -Wno-error/--allow-multiple-definition build flags; job-control change nets out via its own revert; bash-only, clean fast-forward) → **pushed** (ports 53e19ab..a761b2d). So the draft's "tag ports' published HEAD" is now correct and G5 is turnkey. Also confirmed the "uncommitted" siblings (coord/devices/lwip/ports working-tree) are all benign build artifacts + the gcc-14 rollback dir + untracked WiFi fw blobs — the release build's committed state is clean.
 - **Reproducibility-hardening batch (audit subagent returned):** the audit confirmed framework `port.def.sh` ports are already dual-source (phoesys mirror via port.subr) — NOT gaps; the real risks are the hand-rolled `tools/` builders (58 single-URL fetches). Closed the highest-leverage ones, all mirror URLs verified reachable (200) before wiring:
   • **x11 auto-mirror (coord 7bc0ae0):** `_mirror_urls` in build-x11-phoenix.sh transparently expands www.x.org / xorg.freedesktop.org/archive → artfiles.org, and xcb.freedesktop.org/dist → the x.org+freedesktop individual/xcb trees — closes 28 of 58 gaps (ranks 1-3) with zero call-site churn.
   • **toolchain multi-mirror (phoenix-rtos-build 643293e, PUSHED):** `fetch_gnu` tries ftp.gnu.org → mirrors.kernel.org → mirrorservice.org for binutils/gcc. This was the audit's #1 risk — the FIRST fetch of any clean build, previously a single http mirror with no fallback (total-outage = build dies at step one). set -e-safe; aborts cleanly if all mirrors fail.
   • **glib2 mirror fallback (coord aa1e280, session ~248):** verified via build-showcase-apps.sh that the live showcase invokes libiconv/libffi/ncurses/build-x11-phoenix/**glib2**/nano/mc (NOT cairo/pango/wmaker/xserver-core — desktops paused, so those would be dead-code to touch). glib2 is the next real fetch after the freetype step, on download.gnome.org (flaky); added the ftp.gnome.org mirror (verified) + curl --retry 2 + -f. Sweep now proportionate: freetype+x11(28)+toolchain+glib2 cover every invoked outage-prone fetch; libiconv/libffi/ncurses/nano/mc sit on reliable hosts (gnu.org/github) — noted, not blocking.
   Build g33 progressing healthily through the GPU phase (quakespasm LINK OK, bundling rpi4-quake into loader.disk) as of session ~248; the X11/freetype/glib2 STAGE phase (where g32 died) runs later after build.sh — fixes will be exercised then. Monitor armed for terminal state.
 - **Session ~249: g33 CLEARED the freetype failure point** — loader.disk created (867s), now in the STAGE phase building `X11: Xphoenix` (freetype+libX11 fetched OK via the mirror ⇒ freetype fix VALIDATED). No terminal marker yet; Monitor re-armed (bh2izzocr, 60min). Note: `docker-out/rpi4b-sd-2part.img` dated **Aug 1** is a STALE leftover — g33 exports its image (current timestamp) only after the build finishes; don't mistake the stale one for success.
 - **G3.3 boot-verify — SCOPED + turnkey (session ~249):** the docker image is a 2-part SD image (64MB FAT boot p1 + 768MB ext2 root p2). There IS a 64GB card in the Pi (per docs/inprogress/2026-07-02-sd-fullsystem-boot-verification.md — "card stayed in the Pi throughout"). AUTHORITATIVE path = self-flash: netboot Pi into Linux (artifacts/linux-netboot/ exported) → `dd` the g33 image to the Pi's /dev/mmcblk0 → `./scripts/test-cycle-netboot.sh --sd-boot` → verify psh + SD/NFS root + a game + `bash -i` round-trip, 0 faults. SIMPLER fallback (draft says "netboot/SD" so acceptable) = loop-mount p1, TFTP-boot the docker-built loader.disk/kernel8/dtb over netboot with the existing gcc-16 NFS root → validates the docker-built boot chain unattended without SD flashing. Decide when the image lands. Then G5: manifest + tag known-good/gcc16-release-2026-08-25 (libphoenix/ports/build/tests all pushed, HEADs current) + org README flip + publish artifact.
 **NEXT (check each heartbeat):** poll docker-release-g33.log for BUILD_RC=0 + docker-out/rpi4b-sd-2part.img (Monitor armed for success/failure/crash). If clean → G3.3 boot-verify → G5 (manifest + tag known-good/gcc16-release-2026-08-25 + org README flip + publish). If it fails again, diagnose + relaunch. Docker build = HOST CPU not API tokens. Follow-up backlog: harden ranks 5-10 standalone-builder fetches; WiFi data-plane (banked at fw wall); ffmpeg-HW.
...
2026-08-25 (session ~241 — HONEST CORRECTION to ~240: bash EOF NARROWED (not yet root-caused); need HW instrumentation):
 ...
 - libtty console read = `libtty_read`→`read_raw(st=NULL)` = **blocking condWait** (VMIN=1 never returns 0). pts read = `libtty_read_nonblock`→`read_raw(st!=NULL)`; posixsrv `_pts_read` (pty.c:240) on timeout_ms==0 does LIST_ADD + r=NULL = **suspends the client (no 0 delivered)**. So BOTH paths look like they block on VMIN=1 — which CONTRADICTS the observed EOF. ⇒ a subtle bug remains unpinned (candidates: TCSETS not applying c_cc[VMIN] on the pts so VMIN stays 0 → `vmin==0&&vtime==0` poll → returns len=0 → posixsrv responds 0 bytes → EOF; OR readline's poll/select returning readable spuriously then read 0; OR a resume-race returning 0 to the client; OR bash fd0 is neither pts nor pl011 as assumed).
 **NEXT (decisive, needs BUDGET for --scope core + 1 Pi cycle):** instrument on HW — in libtty_ioctl(TCSETS) log the applied c_cc[VMIN]/[VTIME]; in libttydisc_read_raw log (vmin,vtime,st?,len,return); in posixsrv _pts_read log (timeout_ms, responded-bytes); AND log bash's fd0 identity (fstat/isatty/ttyname). Launch `/bin/bash -i` over the ash-harness, capture: what VMIN is set after readline, whether the read returns 0 and from which branch, and whether fd0 is /dev/pts/N or /dev/console(pl011). That single cycle pins the exact 0-return → then the one-line fix (most likely: honor VMIN on pts TCSETS, or map VMIN>0/VTIME=0 first_char_timeout to an infinite-block sentinel instead of 0). Cheap pre-check possible: read posixsrv pty.c TCSETS handler + ttydefchars CMIN to see if pts VMIN defaults to 0 and whether TCSETS applies it.
 - Banked/solid: gcc-16 build+boot DONE; libphoenix siginterrupt PUSHED; JC-off detour reverted. (siginterrupt + the read-path narrowing are the real, kept results of the bash arc.)


2026-08-25 (session ~240 — ★★★ bash interactive-exit ROOT-CAUSED at source (libtty pts VMIN>0 raw-read returns 0=EOF)):
 No new git owner feedback. (Org spend-limit from ~239 appears lifted enough for cheap source work; avoided builds/subagents to stay economical.) **NAILED the bash bug at the source level** — the owner's "fundamental thing" is a libtty non-canonical-read bug on the pts path:
 - readline (rltty.c:531) sets raw mode **VMIN=1, VTIME=0**; rl_getc (input.c:846-848) does select()+read() and treats **read()==0 as EOF** → bash exits at its prompt.
 - `libttydisc_read_raw` (sources/phoenix-rtos-devices/tty/libtty/libtty_disc.c:487) on an EMPTY rx fifo:
   • **st==NULL (BLOCKING, pl011 console):** condWait → blocks until data; VMIN=1 never returns 0 → **ash on the console works.**
   • **st!=NULL (STATEFUL/non-blocking, posixsrv PTS):** `first_char_timeout=(vmin==0)?vtime:0` = 0 for vmin=1, so it does `st->timeout_ms=0; return 0;` and the resume path hits `if (we_wanted_to_sleep_ms==0) return 0;` → **returns 0 (EOF-like) immediately/forever.**
 ⇒ When bash's fd0 is a **pts** (posixsrv pty layer, per pty-run's note that psh's interactive input comes from the pty layer), readline's VMIN=1 raw read returns 0 → EOF → bash quits. ash uses a plain blocking read (no select, and/or the console blocking path) so it stays. **Root cause = the stateful pts raw-read path cannot express "VMIN>0/VTIME=0 = block indefinitely for the first char"; it conflates timeout_ms==0 with "expired, return 0."**
 - **FIX (proposed, libtty_disc.c):** in the stateful branch, distinguish VMIN>0/VTIME=0 (indefinite block) from VMIN==0/VTIME==0 (poll → 0) and VTIME>0 (timed). For VMIN>0 & VTIME==0 with an empty fifo, DON'T set timeout_ms=0/return-0-as-EOF — return the "no data yet, keep blocking / re-arm on rx waitq" sentinel that posixsrv already uses to suspend+resume the client (like libtty_read_nonblock's resume path) so the client read() SUSPENDS instead of getting 0. Verify no regression to VMIN=0 poll (busybox editing) + VTIME timeouts. This is a CORE change → needs `--scope core` rebuild + the ash-harness A/B (bash-nojc/bash reach prompt + accept a typed command, don't EOF) on HW. NEEDS BUDGET (build+Pi cycle).
 - **CONFIRM (cheap, next turn):** that psh/bash fd0 is actually a pts (not /dev/console) + that posixsrv delivers the read-0 to the client — 1-2 source reads of posixsrv pty.c + psh_ttyopen's ttydev. If fd0 is the console instead, re-examine (but the st!=NULL 0-return is the only source path that yields read()==0 on a live tty, so pts is strongly implicated).
 - Banked wins stand: gcc-16 build+boot DONE; libphoenix siginterrupt PUSHED; JC-off detour reverted.


2026-08-25 (session ~239 — ORG MONTHLY SPEND LIMIT hit; recording state + ending cleanly):
 **BLOCKER (external): the org hit its monthly API spend limit** — the readline-vs-ash subagent (a2dcc6e73d) failed mid-run with "hit your org's monthly spend limit". Be economical until the limit resets / owner raises it. No new git owner feedback (many duplicate heartbeats queued while owner away).
 - **bash readline dig — partial finding banked** (from the subagent before it died): readline `rl_getc` (lib/readline/input.c ~848-849) does `select()` then `read()`, and treats **`read()==0` as EOF** → bash exits. Combined with the HW A/B (ash STAYS, bash EOFs on the SAME psh fd 0), the narrowed question is: **why does Phoenix's fd 0 return 0 from `read()` after `select()`/FIONREAD reports it readable, when ash's plain blocking read on the same fd does not?** Prime suspects (per the earlier libtty audit): psh's fd 0 is a posixsrv **pts** whose empty read can return 0 (the "resume-sentinel"), OR readline's raw-mode termios (VMIN) not honored → poll-mode read returns 0. NEXT (when budget returns): finish reading rltty.c prep (VMIN/VTIME/O_NONBLOCK readline sets) + confirm whether psh fd0 is pts vs pl011 console + whether that read returns 0 on empty → then fix in the Phoenix tty/pts read (don't return 0 to a blocking userspace reader) OR readline config. This is the true, well-scoped root-cause path; job control was a refuted detour (siginterrupt fix + JC-off-build capability shipped/banked regardless).
 - State: gcc-16 release build+boot-verify DONE (image exported, 0 faults); libphoenix siginterrupt PUSHED; bash JC-off flip REVERTED (local). All prior commits pushed except the local bash-port robustness commits (env-cache/CFLAGS/LDFLAGS — sound but held; push when budget returns after a JC-on rebuild check).


2026-08-23 (session ~238 — ★★ gcc-16 build+BOOT-VERIFY DONE; bash root-cause CORRECTED (real, narrowed) + siginterrupt shipped):
 No new git owner feedback. Net results:
 - **★★ gcc-16 RELEASE: full build SUCCEEDED + BOOT-VERIFIED on HW (fault-clean).** The fresh all-gcc-16 build (core w/ siginterrupt via --scope core + framework ports + X + GPU + games + glib2) exported an image (Verification OK), and netbooted on the Pi 0 FAULTS: all drivers (thermal/hwrng/usb-kbd+mouse/lwip-genet-link/nfs-takeover), psh, NFS root, busybox ash interactive. E10 gcc-16 now build✓ + boot✓. REMAINING: G3.2 Docker --no-cache reproducible build (launch-ready; buildx installed) → G5 publish.
 - **★ bash interactive-exit: root cause CORRECTED to a REAL bash-specific readline read-path bug (owner vindicated).** HW A/B: JC-on and JC-off bash BOTH reach PS1 then EOF-exit identically (JC hypothesis REFUTED). Decisive ash-vs-bash cycle: **busybox ash STAYS interactive on the same psh fd 0 (reads subsequent input) but bash's readline hits immediate EOF right after PS1** → real bash-specific bug, NOT automation, NOT job control. Reverted the JC-off default flip (ports, local). SHIPPED the sound win: libphoenix **siginterrupt** (was declared-unimplemented) + test/libc/signal — PUSHED (libphoenix ad494b5, tests 1f6e9e9). Subagent a2dcc6e73d pinpointing the readline read path vs ash + Phoenix pts/console read semantics (VMIN / read-returns-0 / O_NONBLOCK) → the specific fix.
 **NEXT:** (1) act on the readline subagent's pinpoint → fix (likely Phoenix tty/pts read honoring VMIN or not returning 0, or a readline/psh-fd-0 fix) → re-test bash interactive over the ash harness. (2) gcc-16 G3.2 Docker --no-cache release build (watch the early git-serve/BuildKit-network phase — build-sd-in-docker.sh relies on --network=host; may need DOCKER_BUILDKIT=0 legacy if BuildKit isolates) → BUILD_RC=0 → G5 publish + manifest/tag known-good gcc16. (3) other backlog (quake residuals, etc.).

2026-08-23 (session ~237 — bash FIX implemented: libphoenix siginterrupt + JC-off config; --scope core building):
 No new git owner feedback. Executing the bash Fix A (build bash without job control, the correct config for a no-JC OS). Steps done this turn:
 - **Found + fixed the JC-off build blocker.** Building bash JC-off (`bash_cv_job_control_missing=missing`) compiles nojobs.c, which failed: `siginterrupt` was **declared in libphoenix signal.h but NEVER implemented** → nojobs.c both clashed (static-vs-nonstatic when ac_cv_func_siginterrupt=no) and would fail to link. **Implemented siginterrupt()** in libphoenix signal/signal.c (standard sigaction round-trip toggling SA_RESTART) + a test/libc/signal round-trip test (owner directive: always add a libphoenix test). libphoenix ad494b5, tests 1f6e9e9 (LOCAL — push after boot-verify).
 - Also fixed a latent bash-port bug: the committed config.cache pinned stale `ac_cv_env_{CFLAGS,LDFLAGS}_value` ('-g -O2 -fcommon' from the original scratch build) → autoconf env-change guard aborted framework rebuilds. Stripped those + set `ac_cv_func_siginterrupt=yes` + `bash_cv_job_control_missing=missing`. ports 83dd096 (LOCAL).
 - **--scope core rebuild RUNNING** (pid 3861110, RPI4B_BUILDROOT=.buildroot-gcc16, log gcc16-scope-core-siginterrupt.log) to propagate siginterrupt into the sysroot libphoenix.a (required before JC-off bash can link). Waiter b4zuowijj armed (also checks siginterrupt lands in libphoenix.a).
 **UPDATE (same session, continued):** --scope core DONE (siginterrupt confirmed `T siginterrupt` in .buildroot-gcc16 sysroot libphoenix.a; fresh loader.disk). **★ JC-off bash BUILT** (2.4MB aarch64 static ELF, siginterrupt resolved) after clearing 3 JC-off-path blockers, each committed LOCAL to the bash port: (a) siginterrupt impl [libphoenix ad494b5]; (b) `-Wno-error=implicit-function-declaration/implicit-int` target CFLAGS (nojobs.c parse.y count_all_jobs implicit-decl under gcc-16 C23) [ports d4342a8]; (c) `-Wl,--allow-multiple-definition` (bash lib/sh strtoimax vs libphoenix strtoull.o — the original build relied on this via the stale env-cache LDFLAGS I'd stripped) [ports 5b233df]; plus JC-off + siginterrupt=yes + env-cache-strip [ports 83dd096]. Staged JC-off bash → /srv/phoenix-rpi4-nfs-gcc16/bin/bash-nojc (kept JC-on /bin/bash for contrast). Refreshed artifacts/gcc16-netboot/bootfs with the fresh gcc-16 kernel+loader (kept RPi firmware). **Pi cycle RUNNING** (task bt49mez1z): netboot, `/bin/bash --version` → `/bin/bash -i` (JC-on, expect exit-at-startup) → `/bin/bash-nojc -i` (JC-off, expect PS1 prompt). Ordered JC-on before JC-off so the harness resyncs to psh between them.
 **★ Pi RESULT (HONEST — JC hypothesis REFUTED as the cause):** BOTH reached their prompt then exited identically — `(psh)% /bin/bash -i → bash-5.2# exit → (psh)%` and `/bin/bash-nojc -i → bash-nojc-5.2# exit → (psh)%`. So there is NO job-control startup exit; bash (both configs) reaches its PS1 prompt then hits **immediate EOF on fd 0** and prints "exit". JC is NOT the differentiator. Positive results that STAND: JC-off bash builds+runs+prompts (distinct `bash-nojc-5.2#`); `siginterrupt` is a real libphoenix gap-fill. But the interactive-exit is the pty-run "immediate EOF after prompt", = the original P2 automation limitation, NOT a JC bug.
 - **★★ DECISIVE RESULT (ash-vs-bash-eof cycle): ash STAYS, bash EOFs — a REAL bash-specific bug (owner vindicated).** UART: `/bin/ash` → `~ #` prompt → received the next line `/bin/bash-nojc -i` (ash STAYED + ran it) → bash prompted `bash-nojc-5.2#` → EOF-exit → back to `~ #` (ash still alive). So on the SAME psh-handed fd 0, ash's plain read blocks/stays but **bash's readline read returns immediate EOF right after printing PS1**. NOT automation, NOT job control. Narrowed to the readline read path.
 **NEXT:** read the ash cycle → conclude. Commit disposition: KEEP libphoenix siginterrupt (ad494b5) + test (1f6e9e9) + the bash-port env-cache/CFLAGS/LDFLAGS robustness fixes (real); but the **JC-off default flip is now unjustified as an interactive fix — REVERT `bash_cv_job_control_missing` to present** (keep siginterrupt=yes since it's harmless+correct) unless the ash test shows JC matters. Push the sound commits after deciding. Then PIVOT off bash (likely owner-attended per P2): gcc-16 boot-verify (full build SUCCEEDED, image exported) → manifest/tag → G3.2 Docker release (launch-ready) → G5.

2026-08-23 (session ~236 — ★★ gcc-16 FULL BUILD SUCCEEDED (glib2 fix worked); bash root-cause CONFIRMED via Phoenix audit):
 No new git owner feedback. Two big results:
 - **★★ gcc-16 full build COMPLETE + image exported, Verification OK.** The glib2 `-std=gnu17` fix + clean-tree + tarball-precache worked: log shows `glib2 core OK (libglib-2.0.a)` then full image export (rpi4b-sd.img, sha 71f0b477…, Verification OK). So the COMPLETE all-gcc-16 system (core + framework ports + X + GPU + games + glib2/mc) builds clean end-to-end. Bootfs at .buildroot-gcc16/_boot/aarch64a72-generic-rpi4b/rpi4b/{loader.disk,kernel8.img}. NEXT = boot-verify over netboot.
 - **★ bash interactive-exit root cause CONFIRMED + refined by a full Phoenix pgrp/tty audit (subagent).** My first sketch (tcsetpgrp stub → background-read EOF) was REFUTED: tcsetpgrp is real, and psh's runfile.c `setpgid+tcsetpgrp` makes bash the tty foreground group BEFORE exec, so bash's foreground check PASSES and it ENTERS full job-control mode. Real finding: **Phoenix has NO job control** — tty->pgrp never gates reads, no SIGTTIN/SIGTTOU, stop signals default to _signal_ignore, no kernel stop-state, setsid is a stub. FIONREAD already fixed (b247643, "not the sole cause"). ⇒ bash-built-WITH-job-control is a capability mismatch on a no-JC OS; ash (JC compiled out) is the correct model. **Fix A = build bash without JC (bash_cv_job_control_missing=missing)** is the CORRECT config, not a workaround. Fix B (real JC) is large (4 components: sender-pid plumbing, libtty read/write gating, kernel stop-state, libphoenix signal dispositions). Doc: docs/inprogress/2026-08-23-bash-interactive-exit-analysis.md (corrected).
 **NEXT:** (1) build a JC-off bash + stage as /bin/bash-nojc in the gcc16 NFS export (keep JC-on /bin/bash for A/B). (2) sync fresh gcc-16 rootfs → export. (3) ONE Pi cycle: boot-verify the all-gcc-16 system (psh/drivers/NFS/0 faults) + launch `/bin/bash -i` (expect exit) vs `/bin/bash-nojc -i` (expect PS1 prompt). If JC-off reaches the prompt → make it the bash-port default + fix stale KNOWN-ISSUES/pty-run text. → then gcc-16 manifest/tag → G3.2 Docker release → G5.

2026-08-23 (session ~235 — gcc-16 build past X-stack, fixed glib2 C23; bash interactive ROOT-CAUSED to job control):
 No new git owner feedback (owner messaged in-session: keep working the whole backlog autonomously till morning, self-prioritize). Two threads:
 - **gcc-16 build:** the ~233 fontconfig/cairo cache+fallback fix WORKED — build sailed through the entire framework X stack + showcase GPU, then b_died in the ad-hoc `tools/ports/build-glib2.sh` on a gcc-16 **C23** issue (glib 2.56.4 `goption.c: gboolean bool;` — `bool` is a C23 reserved keyword). **FIXED: -std=gnu17 in build-glib2.sh** (coord 07ff47d, pushed). libiconv/libffi/ncurses build before glib2 and already passed under gcc-16; nano/mc are run_step_soft (non-blocking). Restarted — but glib2 FAILED AGAIN (same error): build-glib2.sh skips `./configure` if `config.status` exists (line 101), so the stale glib tree from the failed run kept its old (no-gnu17) Makefiles. FIX: `rm -rf tools/ports/src/glib-2.56.4` forces re-extract+reconfigure with -std=gnu17; also PRE-CACHED glib-2.56.4.tar.xz (7MB, gnome.org up, xz-verified) into tools/ports/src/ so the re-extract doesn't depend on a live CDN. (The Docker release is unaffected — it starts fresh, no stale tree.) Re-restarted (pid 3725569, log gcc16-promoted-full-5.log); waiter b02hrhfuv armed. loader.disk NOT yet built (showcase must finish first).
 - **★ bash interactive-exit ROOT-CAUSED (owner asked) = JOB CONTROL, not fd-0 EOF.** Owner's discriminator (ash/xterm/quake all interactive → fd 0 delivers bytes) kills the stale "fd 0 EOF" theory. Smoking gun: **busybox ash = job control OFF (`CONFIG_ASH_JOB_CONTROL` unset) + raw-editing ON → works**; our bash = **job control ON** (`bash_cv_job_control_missing=present`). bash's initialize_job_control does setpgid + tcsetpgrp to become the tty foreground group; on Phoenix (suspected tcsetpgrp stub / no foreground-pgrp) bash becomes a BACKGROUND reader, tty read returns 0 instead of SIGTTIN → readline EOF → bash exits at first prompt. ash never does this (JC off). Analysis doc: docs/inprogress/2026-08-23-bash-interactive-exit-analysis.md (coord ad6ef1e). Subagent a9ba732938606a210 auditing Phoenix pgrp/tty/background-read to confirm at source level + assess the real-JC fix.
 **NEXT — bash experiment (fold into the gcc-16 boot-verify cycle, one Pi cycle):** build a JC-OFF bash (`bash_cv_job_control_missing=missing`) as `/bin/bash-nojc`, stage into the gcc-16 NFS export, then over psh-interact launch `/bin/bash -i` (JC-on, expect exit-at-startup) vs `/bin/bash-nojc -i` (JC-off, expect PS1 prompt + wait). Discriminator observable under automation = does it reach/print PS1 before any input. If confirmed → make JC-off the port default + fix stale KNOWN-ISSUES/pty-run text; scope real foreground-pgrp+SIGTTIN (Fix B) for full JC. (Advisor: likely QEMU-reproducible, not Pi-locked — faster loop if QEMU brings up a console.) THEN gcc-16 boot-verify (recipe in prior entry) → manifest/tag → G3.2 Docker release (launch-ready) → G5.

2026-08-23 (session ~234 — build recovered past fontconfig; G3 buildx blocker CLEARED + Docker launch-ready):
 No new git owner feedback. **The ~233 cache+MacPorts-fallback fix WORKED end-to-end:** the restarted gcc-16 build (pid 3462017) hit `xorg-fonts: fontconfig-2.14.2 from distfiles cache` (the exact fetch that b_died last turn) and sailed through cairo (cache) + libXft → xorg_server (artfiles) → now building windowmaker + xorg_apps (xclock [OK]). Build is in the final X-apps stage heading to image assembly; healthy.
 - **G3 buildx blocker CLEARED:** installed `docker-buildx` 0.30.1 (`sudo apt-get install docker-buildx` — the Ubuntu-26.04 package is `docker-buildx`, NOT `docker-buildx-plugin`; sudo is passwordless; build-sd-in-docker.sh already uses `sudo docker` so the socket-perm side is handled). **Validated the release Dockerfile under BuildKit:** `sudo docker buildx build --check -f Dockerfile .` → "Check complete, no warnings found" (my Q2/Q3 ARGs + 7zip + fetch-quake-data calls parse; ubuntu:26.04 pulls). ⇒ **G3.2 Docker release build is LAUNCH-READY** — run AFTER the gcc-16 build + boot-verify (don't compete for CPU/RAM now). Coord 000cbd7 (+ ports/coord from ~233).
 - README now documents the BuildKit/buildx prerequisite + that the Docker build fetches all 3 Quakes' data at build time (PAK0Q2_URL/PAK0Q3_URL build-args).
 **NEXT:** monitor pid 3462017 to completion (image assembly next) → **boot-verify recipe (recon done this turn; NFS export already = /srv/phoenix-rpi4-nfs-gcc16 fsid=0, TFTP driven by PHOENIX_BUILDROOT/_boot):**
```
PHOENIX_BUILDROOT=/home/houp/phoenix-rpi/.buildroot-gcc16 RPI4B_NFS_EXPORT=/srv/phoenix-rpi4-nfs-gcc16 ./scripts/sync-netboot-tree.sh   # fresh rootfs -> gcc16 export (no --delete)
PHOENIX_BUILDROOT=/home/houp/phoenix-rpi/.buildroot-gcc16 ./scripts/test-cycle-netboot.sh --capture-secs 240   # TFTP serves gcc16 bootfs; card OUT; Pi-lock
./scripts/uart-summary.sh <label>   # confirm psh + drivers + nfs mount + 0 faults
```
 → snapshot manifest + tag known-good gcc16 → **G3.2 Docker --no-cache release build (now launch-ready)** → G3.3 boot-verify the Docker image → G5 publish. Pending: wire fetch-quake-data.sh into SD/netboot flow; C++23 std-module fix.

2026-08-23 (session ~233 — flaky-CDN night: SECOND CDN (freedesktop) down; build b_died at fontconfig; recovering):
 No new git owner feedback. The restarted gcc-16 build (pid 3410156) got past the x.org fonts libs (artfiles mirror + cache fix from ~232 WORKED — log: "libXfont2 from distfiles cache") but then **b_died fetching fontconfig-2.14.2 from www.freedesktop.org — that CDN is now ALSO down** (connection timeout, all 3 retries failed). Same class as the x.org outage: xorg_fonts pulls fontconfig from freedesktop + cairo from cairographics.org (also down, curl 000) with no mirror, only the cache — and the cache wasn't pre-populated for those two. Build exited cleanly (b_die), machine clean.
 - **RESOLVED + build restarted.** Subagent recovered all 5 tarballs into the cache from the **MacPorts distfiles mirror** (mirrors.mit.edu/macports — reliably archives historical upstream releases; LFS/Gentoo/fossies-old all 404'd): fontconfig-2.14.2.tar.xz (sha256 dba695b…, xz-verified + configure present) + cairo-1.16.0.tar.xz (sha256 5e7b29b…) + freetype/libpng/jpeg. Cache ($HOME/.phoenix-distfiles/xorg) now has EVERY xorg_fonts tarball → fully offline-reproducible locally.
 - **DURABLE Docker-release fix (ports 53e19ab, pushed):** generalized xorg_fonts `_fetch_extract` to a multi-URL list (primary + fallback mirrors, tried in order) and added the MacPorts mirror as a fallback for fontconfig (freedesktop-down) + cairo (cairographics-down). A fresh Docker container (empty cache) now survives one CDN outage — the local cache alone can't help it. sha256-verified fallback tarballs.
 - **FIXED a Docker bug found en route (coord 2deef51, pushed):** `p7zip-full` has NO apt candidate on Ubuntu 26.04 (the new Docker base) — the `7z` binary the Q2 .exe extraction needs is now in the `7zip` package. Dockerfile apt line p7zip-full→7zip (verified on the 26.04 host).
 - **Build restarted** (pid 3462017, log gcc16-promoted-full-3.log): GPU archives already present → skipped straight to ports (now on sqlite3, a single-TU amalgamation = expectedly 1-core), will reach xorg_fonts → fontconfig CACHE HIT → cairo (cache) → xorg_server (artfiles) → X + image.
 **NEXT:** monitor pid 3462017 to completion → stage + boot-verify the complete all-gcc-16 system → manifest/tag known-good gcc16 → G3.2 Docker --no-cache release build (host buildx still broken — install docker-buildx-plugin or DOCKER_BUILDKIT=0) with all-3 game data → G5 publish. Also pending: wire fetch-quake-data.sh into SD/netboot flow; C++23 std-module fix.

2026-08-23 (session ~232 — G3/G4 release items + FIXED x.org-down build breakage + owner Q2/Q3 default-data request):
 No new git owner feedback at turn start; a mid-turn owner message resolved the Q2/Q3 licensing question (distribute build scripts only, download data at build time → give Q2/Q3 default URLs like Q1). Work this turn (all coord/ports/pushed):
 - **G3/Docker game-data (owner request): DONE.** Shared `scripts/fetch-quake-data.sh` (q1|q2|q3 → correct overlay subdir + pak + extraction) used by SD/Docker/netboot; Dockerfile refactored to call it for all 3. Q2 default = Yamagi q2-314-demo-x86.exe (7z-extract baseq2/pak0.pak, sha256 cae2571…); Q3 default = GWDG linuxq3ademo-1.11-6.x86.gz.sh (makeself skip+gzip+tar → demoq3/pak0.pk3, sha256 e77abad…). Both VERIFIED end-to-end (subagent downloaded+extracted+hashed; my tests: correct pak+sha rc=0, mismatch rc=1, empty-skip rc=0). Added p7zip-full to Docker apt. Hardened cleanup trap (read-only makeself trees) — proven it can't mask a real failure. Commits 0e66aef/2829201.
 - **G4 docs: DONE.** README callout, bootstrap-linux-host.sh comments, BUILD.md step-5 now say gcc-16.2.0+binutils-2.47 DEFAULT (was "still gcc-14 during transition" — G2.2 promotion made it default+boot-verified); Ubuntu 26.04 noted validated. Commit 4bc8f5b.
 - **★ FIXED the running build's x.org breakage + distfiles-cache follow-up: DONE.** The full gcc-16 build (pid 3208131) was failing in xorg_fonts — x.org's CDN (xorg.freedesktop.org) was down/timing out, and xorg_fonts/xorg_server fetched from www.x.org with NO cache. Added the artfiles.org mirror + shared persistent distfiles cache ($HOME/.phoenix-distfiles/xorg) to xorg_fonts' _fetch_extract, and pointed xorg_server's source= at artfiles (ports c3f8a6f, pushed publish/master; mirrors the xorg_libs pattern). Pre-populated the cache with libfontenc/libXfont2/libXft. Killed the doomed build + restarted (pid 3410156, --scope auto, reuses built core+ports; log gcc16-promoted-full-2.log). **VALIDATED in-build:** the restart fetched the x.org fonts libs via artfiles + is populating the cache + progressed past them (now on fontconfig/expat) — the exact failure is fixed. Note: x.org is INTERMITTENT not fully dead, but artfiles is reliably up.
 - Build now runs FULLY multi-core (16 compilers) — confirms the session-~231 build-gl/build-v3d/games parallelization on a real gcc-16 build.
 **NEXT:** monitor pid 3410156 → GPU phase (done) → ports (in xorg_fonts, fixed) → xorg_server (artfiles) → X + image → stage + boot-verify the complete all-gcc-16 system → snapshot manifest + tag known-good gcc16 → G3.2 Docker --no-cache release build (needs host buildx — currently broken, note for G3) with all-3 game data → G5 publish. Also: wire fetch-quake-data.sh into the SD/netboot build flow (currently Docker-only); C++23 std-module fix (scheduled).

2026-08-23 (session ~231 — owner "only 1 core working, please check": diagnosed + finished ALL build parallelization):
 Owner reported the full gcc-16 build (pid 3208131) using only 1 core. **Diagnosed:** the port_manager env ALREADY has `MAKEFLAGS=--no-print-directory -j 16` (inherited from the build.sh:74 fix), so the 72 bare-`make` ports DO build with -j16 — sampled the running build at 7→13→10 active compilers on openssl. The 1-core moments the owner saw are (a) inherently single-TU/serial steps: `sqlite3` (single-amalgamation .c), autotools `configure`/`libtoolize`/link — unavoidable; and (b) **the custom-python GPU/game builders, which spawn aarch64-phoenix-gcc DIRECTLY and thus bypass MAKEFLAGS** — those needed explicit parallelization. **FIXED all remaining serial custom-python builders:** build-gl-phoenix.py (Mesa GL frontend ~200 C/C++ TUs — the biggest serial block, in --with-showcase) + flagship build-quakespasm-phoenix.py (da009cf); yquake2 + quake3e nested `for groups: for units:` loops flattened to one ThreadPool + build-port.sh -j9→-j$(nproc) (20d019e). All syntax-checked, pushed to publish/main. The running build will pick up the parallel build-gl + flagship-quakespasm when it reaches the showcase phase (invoked from coord tools/ by path). build-v3d/vkquake/quakespasm-sdl were already done earlier this session.
 **NEXT:** monitor pid 3208131 (still in ports: openssl/autotools) → showcase (now parallel) → stage + boot-verify the complete all-gcc-16 system → snapshot manifest + tag known-good gcc16 → G3 Docker 26.04 --no-cache + Q2/Q3 data → G4 docs → G5 publish. C++23 std-module fix still scheduled.

2026-08-23 (session ~230 — ★★★ G2.2 TOOLCHAIN PROMOTED to gcc-16.2.0+binutils-2.47; Mesa builds CLEAN under gcc-16; full everything-build launched):
 No git owner feedback. **G2.2 PROMOTE DONE:** default `.toolchain` is now gcc-16.2.0 + binutils-2.47 (atomic mv: .toolchain[gcc-14]→.toolchain-gcc14 rollback; .toolchain-gcc16-247→.toolchain). Verified default `aarch64-phoenix-gcc`=16.2.0, `ld`=2.47. So ALL path-based consumers (rebuild-rpi4b-fast.sh + every tools/* GPU/game script that hardcodes .toolchain) now use gcc-16 — no per-script edits needed. **★ Mesa/libv3d builds CLEAN under gcc-16.2.0** (build-v3d: driver 29/29, aux 43/43, core 333/333, FAIL=0, archive+link PASS) — the big G1.2 unknown de-risked (Mesa uses its own -std, dodges C23; no -Werror breakage). Also validated the build-v3d ThreadPool parallelization on a real build. Freed the superseded .toolchain-gcc16 (binutils-2.43, 8.6G; disk 78G free).
 - **LAUNCHED: full 'everything under gcc-16.2.0+binutils-2.47' build** (detached pid 3208131, log gcc16-promoted-full.log): `--variant nfsroot --scope full-clean --with-ports --with-showcase --with-vkquake` into .buildroot-gcc16 with the promoted default gcc-16 toolchain. This is the FIRST build with core+ports+X+GPU+GAMES ALL under gcc-16 (games were gcc-14 before). MONITOR next turn.
 **NEXT:** monitor pid 3208131 → if clean, that's the complete all-gcc-16 system (incl. gcc-16 games) → stage + boot-verify → snapshot manifest + tag known-good gcc16 → G3 Docker 26.04 --no-cache (BUILD_RC=0) + Q2/Q3 data → G4 docs → G5 publish release. If GPU/game issues surface under gcc-16, fix iteratively. ROLLBACK: .toolchain-gcc14 (mv back). Also pending: yquake2/quake3 build parallelization; C++23 std-module fix (scheduled).

2026-08-23 (session ~229 — COMPLETE gcc-16 rootfs BOOT-VERIFIED; G2.1 toolchain promotion STARTED (gcc-16.2.0 + binutils-2.47 building)):
 No git owner feedback. **Complete gcc-16 rootfs BOOT-VERIFIED over netboot** (log 20260822-221346-netboot-gcc16-complete): re-pointed TFTP→gcc16 bootfs (dnsmasq had reverted to the gcc-14 default), NFS fsid=0=gcc16 export. Boot: all gcc-16 drivers (vcmbox/thermal/fb0/gpio/hwrng/lwip/usb — PIXART mouse enumerated) + `nfs-fs: mounted 10.42.0.1:/ via v4` (complete gcc-16 export) + psh prompt + IP, 0 faults. ⇒ the complete gcc-16 rootfs (system+ports+X+games) boots + serves over NFS. (Same kernel/drivers as the core rootfs; now with the full X+ports userspace.)
 - **G2.1 STARTED — toolchain promotion:** edited sources/phoenix-rtos-build/toolchain/build-toolchain.sh:65-66 → BINUTILS=binutils-2.47, GCC=gcc-16.2.0 (LOCAL/uncommitted — commit after verify). Launched the from-scratch toolchain build (detached pid 2904762, log gcc16-247-toolchain.log) into a FRESH .toolchain-gcc16-247 (keeps .toolchain gcc-14 + .toolchain-gcc16 binutils-2.43 as fallbacks). Passed preconditions (no CC/CFLAGS env, aarch64-phoenix-gcc not in PATH), downloading binutils-2.47.tar.bz2 from the GNU mirror. Uses the staged patches (gcc-16.2.0-*.patch ×4, binutils-2.47-04-aarch64-phoenix.patch). Multi-hour; MONITOR next turns.
 - **★ BUILD PARALLELISM AUDIT (owner: use all 16 cores):** found THREE hardcoded/serial bottlenecks, all fixed host-adaptive: (1) build-toolchain.sh MAKEFLAGS -j9→-j$(nproc) (f90bc71, local); (2) **phoenix-rtos-build/build.sh:74 MAKEFLAGS "-j 9"→"-j $(nproc)"** — the ENTIRE image build (kernel+programs+ports via rebuild-rpi4b-fast.sh) was -j9, so even my gcc-16 builds under-used cores (7463000, local); (3) tools/v3d-driver-port/build-v3d-phoenix.py build_objs() compiled ~400 objects SERIALLY → now a cpu_count() ThreadPool, deterministic archive order kept (coord 4551563, pushed). Killed the old -j9 toolchain build + relaunched at -j$(nproc)=16 (pid 3041151; RAM fine — 20G avail + 8G swap after desktop apps idled). Game builds: **vkquake + quakespasm parallelized** (ThreadPool ex.map, deterministic order; coord c300ae8, syntax-checked, pending build-verify). FOLLOW-UP: yquake2 (~140) + quake3e (~169) have NESTED group loops (`for groups: for units:`, per-group flags) → flatten into one ThreadPool; do + build-verify ALL game builds (incl. the unverified vkquake/quakespasm/build-v3d parallel edits) once the toolchain build frees the CPU.
 - **✅ G2.1 DONE — gcc-16.2.0 + binutils-2.47 toolchain BUILT + VERIFIED + PUSHED.** .toolchain-gcc16-247 installed: `aarch64-phoenix-gcc (GCC) 16.2.0` + `GNU ld 2.47.20260726`; C and classic-C++ (std::vector, libstdc++.a) both compile → static aarch64 ELFs. Pushed the now-verified sibling commits (phoenix-rtos-build 8b0c29c..7463000: gcc-16.2.0+binutils-2.47 pins, binutils-2.47 patch, toolchain + build.sh -j$(nproc)). ONE non-fatal gap: the C++23 `std` MODULE (import std;) fails to build (gcm.cache/std.gcm ordering — maybe a -j race) — harmless (Phoenix uses classic C++), SCHEDULED as a follow-up per owner (gcc16-release-plan.md). Reclaimed 6.7G toolchain build-scratch (disk 33G free). .toolchain-gcc16 (binutils-2.43) now superseded, removable after promote.
 **NEXT:** G2.2 PROMOTE (backup .toolchain→.toolchain-gcc14, swap .toolchain-gcc16-247→.toolchain — deliberate, owner-authorized, keep gcc-14 rollback) → then a full gcc-16.2.0+binutils-2.47 build makes ALL scripts (incl. tools/GPU/games, which hardcode .toolchain) use gcc-16 = G1.2 (expect + fix Mesa-under-gcc-16 issues) → boot-verify → G3 Docker 26.04 --no-cache + Q2/Q3 data → G4 docs → G5 tag+publish. Also pending: yquake2/quake3 build parallelization + build-verify all parallelized builds. (.toolchain→gcc-16, tag gcc-14 rollback) → G2.3 verify all path-based consumers + G1.2 GPU/games under gcc-16 → G3 Docker --no-cache 26.04 + Q2/Q3 data → G4 docs → G5 ship. Per gcc16-release-plan.md.

2026-08-23 (session ~228 — ★★★ COMPLETE gcc-16 rootfs BUILT (system+ports+X); README advertises gcc-16/binutils; Docker→26.04; Q2/Q3-data scheduled):
 No git owner feedback (owner sent live asks this turn). **G1.1 DONE — the full-clean gcc-16 --with-ports build COMPLETED CLEAN** (pid 2624760): image exported (Verification OK), the ENTIRE X stack built under gcc-16 — Xphoenix (6MB), xterm, wmaker (00:04-00:05) — via the libXt-1.3.1 + gnu17 fixes. The 22 "error" grep-hits are all benign banners (OpenSSL "successfully configured", install progress, fontconfig/cairo cosmetic warnings), no real failures. RE-STAGED the complete gcc-16 rootfs: rsync'd _fs/root → /srv/phoenix-rpi4-nfs-gcc16 (now has X + all ports + games + gcc-16 nfs) + refreshed artifacts/gcc16-netboot/bootfs kernel/loader (00:08). ⇒ the owner-testable gcc-16 rootfs is now the COMPLETE one.
 - **Owner live asks actioned:** (a) README top-level callout advertising GCC 16.2.0 + binutils 2.47 modernization (coord 8a4d6a4; honest status). (b) Dockerfile UBUNTU_TAG 24.04→26.04 (coord b429f17 — 26.04 is the actual build host; corrects my earlier 24.04 assumption) + README default fixed. (c) SCHEDULED: Docker game-data fetch is Q1-only → add Q2/Q3 (baseq2/demoq3) redistributable demo-data fetch (gcc16-release-plan follow-up; licensing-clean; a shared fetch-quake-data.sh).
 **NEXT:** boot-verify the COMPLETE gcc-16 rootfs (netboot → psh + optionally X/games) → G2 promote toolchain (build-toolchain.sh:65-66 → gcc-16.2.0 + binutils-2.47, patches staged; rebuild + verify) → G3 Docker --no-cache 26.04 gcc-16 (BUILD_RC=0) + Q2/Q3 data → G4 docs → G5 manifest+tag+publish gcc-16 release. Per gcc16-release-plan.md.

2026-08-23 (session ~227 — gcc-16 build2 through the X stack (libXt-1.3.1 building clean); G3 Docker flow confirmed):
 No owner feedback. build2 (pid 2624760) RUNNING clean — reached + is building the X stack: `CCLD libXt.la` succeeded ⇒ libXt-1.3.1 + gnu17 fix VALIDATED under gcc-16 (past the earlier failure). Proactive gnu17 pins for xorg_fonts/server/xterm/windowmaker are synced into the buildroot so the rest of X should follow clean. Monitoring to image completion (log gcc16-full-build2.log).
 - **G3 prep (Docker toolchain chain confirmed):** Dockerfile:64 → scripts/bootstrap-linux-host.sh → build_toolchain() (line 475) → scripts/build-phoenix-toolchain-linux.sh → sources/phoenix-rtos-build/toolchain/build-toolchain.sh:65-66 (the BINUTILS/GCC pins). So the SINGLE G2.1 pin bump (gcc-16.2.0 + binutils-2.47) flows through the reproducible Docker build with no other edits. UBUNTU_TAG=24.04 is fine (gcc-16.2.0 already built on this 24.04-class host). Cosmetic: bootstrap-linux-host.sh comments (lines 175, 470-473) still say "gcc-14.2 + binutils-2.43" → update in G4/docs.
 **NEXT:** monitor build2 → complete gcc-16 rootfs (system+ports+X) → assemble/stage + boot-verify → G2 promote (build-toolchain.sh:65-66 → gcc-16.2.0 + binutils-2.47, patches staged; rebuild+verify) → G3 Docker --no-cache → G4 docs → G5 ship. Per gcc16-release-plan.md.

2026-08-23 (session ~226 — gcc-16 X-stack C23 fixes (proactive gnu17 across all X ports); full build re-running):
 No owner feedback. gcc-16 build2 (pid 2624760) re-running clean after the libXt fix (past core, through non-X ports). PROACTIVELY fixed the remaining X ports for gcc-16/C23 before the build reaches them: xorg_fonts (7 CFLAGS sites), xorg_server (1), xterm (2), windowmaker ($cf var) all set their own CFLAGS and so missed the Makefile.common gnu17 pin — added -std=gnu17 to each (ports 6893d36, pushed publish/master; no-op under gcc-14). Synced all 4 into .buildroot-gcc16 so the RUNNING build2 picks them up when it reaches the X stack → the whole X stack should build gcc-16-clean in this pass (no per-port failure→rerun grind). Combined with libXt-1.3.1 + xorg_libs gnu17 (3cddb65) + the distfiles cache/mirror (2cb8a62), all X ports are now gcc-16-ready.
 **NEXT:** monitor build2 to completion (log gcc16-full-build2.log) → if the full gcc-16 --with-ports image builds clean, that's the COMPLETE gcc-16 rootfs (system+ports+X); assemble/stage + boot-verify. Then G2 (promote toolchain: build-toolchain.sh → gcc-16.2.0 + binutils-2.47 + rebuild + verify) → G3 Docker --no-cache gcc-16 → G4 docs → G5 ship. Per docs/inprogress/gcc16-release-plan.md.

2026-08-23 (session ~225 — SCHEDULED the gcc-16 RELEASE plan (owner ask: align everything to gcc-16 + ship a release); full build still running):
 Owner directive: "align ALL build instructions + scripts to gcc-16; verify the Docker reproducible build against gcc-16; ship a gcc-16-based release to the org." This AUTHORIZES the gcc-16 promotion (previously held owner-attended). Wrote the durable schedule: **docs/inprogress/gcc16-release-plan.md** (phases G1 build-completeness → G2 promote toolchain+scripts → G3 Docker verify → G4 docs → G5 ship+tag). KEY INSIGHT recorded: all build scripts reference `.toolchain/` BY PATH, so gcc-16 alignment = a PROMOTION driven by ONE script (`scripts/build-phoenix-toolchain-linux.sh`, currently gcc-14.2.0) + the Dockerfile that invokes it; making `.toolchain` = gcc-16 aligns rebuild-rpi4b-fast.sh + all tools/* GPU/game scripts + Docker at once, no per-script edits. Docker build system = Dockerfile + scripts/build-sd-in-docker.sh + serve-repos-for-docker.sh + phoenix-rtos-project/docker-{build,devel}.sh.
 - Full-clean gcc-16 --with-ports build (pid 2457574) FAILED at xorg_libs (first gcc-16 port issue, G1.1): libXt-1.3.0 Shell.c `static Boolean true = True;` breaks under gcc-16's C23 default (`true` is a keyword). FIXED two ways (ports 3cddb65): (a) bumped libXt 1.3.0→1.3.1 (owner suggestion — upstream renamed the identifier, verified C23-clean; no Phoenix patches, clean drop-in, cached); (b) added -std=gnu17 to the xorg_libs compile CFLAGS (the port set its own CFLAGS so missed the Makefile.common gnu17 pin) → covers the other ~23 old X libs too. My distfiles cache + artfiles mirror WORKED in the failed build (libs pulled from cache). Re-launched --with-ports (detached pid 2624760) — resumes past xorg_libs. Monitor next turn; expect possibly more gcc-16 port issues to fix iteratively (G1.1 grind).
 - **binutils bump folded in (owner ask: "use the most recent binutils"):** current 2.43 → latest **2.47**. Version pins are in sources/phoenix-rtos-build/toolchain/build-toolchain.sh:65-66 (BINUTILS/GCC). Staged + verified the binutils-2.47 aarch64-phoenix patch (sibling 96f5697; the 2.43 aarch64 patch applies clean to 2.47, dry-run confirmed); gcc-16.2.0 patches already present. Both tarballs on the GNU mirror in the fetched formats. So G2.1 = a version-pin bump (gcc-16.2.0 + binutils-2.47) + from-scratch rebuild + verify.
 **NEXT (per gcc16-release-plan.md):** G1.1 finish/verify the running full build → G2.1 bump build-toolchain.sh:65-66 to gcc-16.2.0 + binutils-2.47 + rebuild + verify → G2.2 promote `.toolchain`→gcc-16 (tag gcc-14 rollback first) → G2.3/G1.2 verify all scripts + GPU/games under gcc-16 → G3 Docker --no-cache gcc-16 (BUILD_RC=0) + boot-verify → G4 docs → G5 manifest+tag+publish gcc-16 release. Interleave: open game-bug digs (Q1 tex, vkQuake fire/light, USB, Q2 slow-load) + deferred renderer features F1/F2.
 No owner feedback in git. **Tarball cache + mirror implemented (owner-requested):** xorg_libs/port.def.sh (ports 2cb8a62) now (1) checks a persistent distfiles cache ($HOME/.phoenix-distfiles/xorg, override PHOENIX_DISTFILES) before downloading + populates it on a miss → clean rebuilds reuse tarballs, no re-download; (2) defaults XBASE/XARCHIVE/XCBB to the reliable full mirror https://artfiles.org/x.org/pub/xorg/individual (owner-suggested, all 23 X tarballs HTTP-200). Pre-populated the cache: 23/23 xorg tarballs downloaded (14M) → mirror confirmed complete + cache ready.
 - **LAUNCHED: full-clean gcc-16 --with-ports build** (detached pid 2457574, log $CLAUDE_JOB_DIR/tmp/gcc16-full-build.log) — "clean host fs core ports project image" with the gcc-16 toolchain + PHOENIX_DISTFILES cache + artfiles mirror. This is the owner's "full clean rebuild of everything using gcc-16" (system + ports + X, now unblocked). First --with-ports gcc-16 build → may surface gcc-16 -Werror/-std issues in ports/X not seen in the core-only builds; iterate + fix (resilience). MONITOR next turn (log + pid + disk).
 **NEXT:** (1) monitor pid 2457574 → if clean, assemble/stage the COMPLETE gcc-16 rootfs (system+ports+X, + games) + boot-verify; if it errors, fix the gcc-16 port issue + resume. (2) gcc-16-games stretch (tools-script toolchain override + Mesa-under-gcc-16). (3) resume open game-bug digs (Q1 tex, vkQuake fire/light, USB warm-reboot, Q2 slow-load). (4) deferred features F1 (Q2 GLES3) / F2 (Q3 Vulkan). NOTE: netboot currently serves the gcc-16 CORE rootfs (session ~223) for owner testing; the full build writes .buildroot-gcc16 only (doesn't touch the live export until I re-stage).

2026-08-23 (session ~223 — ★★ gcc-16 ROOTFS BOOT-VERIFIED over NFS (owner-testable); working xorg mirror found):
 No owner feedback in git. **DELIVERED: a gcc-16 rootfs over NFS, boot-verified, ready for owner testing.** Assembled: (a) gcc-16 bootfs = gcc-14 RPi firmware (start4.elf/fixup4.dat/overlays) + gcc-16 Phoenix files (kernel8/loader.disk/dtb/config/armstub) at artifacts/gcc16-netboot/bootfs; (b) gcc-16 NFS export /srv/phoenix-rpi4-nfs-gcc16 = gcc-16 _fs/root base (incl. my gcc-16 nfs fix) + the hand-staged games/data/tools overlaid from the gcc-14 export (games are static → run on the gcc-16 kernel). Swapped NFS fsid=0 → gcc16 export (/etc/exports.d/phoenix-rpi4.exports; gcc-14 backup at .gcc14bak), TFTP → gcc16 bootfs (RPI4B_NETBOOT_TFTPROOT). HW BOOT-VERIFIED (netboot, log 20260822-212408-netboot-gcc16-rootfs): gcc-16 loader v1.21 + gcc-16 microkernel v3.3.1 + all gcc-16 drivers (vcmbox/thermal/fb0/gpio/hwrng/usb/lwip) + `nfs-fs: mounted 10.42.0.1:/ via v4` (the gcc16 export) + psh prompt + genet link + IP, ZERO faults. ⇒ **owner can netboot-test the gcc-16 system now** (card out; netboot currently serves gcc-16). To revert to gcc-14: restore exports.d from .gcc14bak + exportfs -ra + restart nfs-server, and drop the TFTP override.
 - **Owner's xorg mirror WORKS:** https://artfiles.org/x.org/pub/xorg/individual/ → HTTP 200 for libXdmcp-1.1.5, libX11-1.8.7, xorgproto (lib/*, proto/*). Unblocks the full gcc-16 --with-ports build (with xorg). Set XORG_XBASE/XARCHIVE=…/individual, XORG_XCBB=…/individual/lib.
 - **Tarball-cache gap (owner-reported):** xorg_libs/port.def.sh _fetch_extract downloads into the wiped-on-clean build tree ($SRC) → re-downloads every clean build. Other ports keep tarballs committed in-dir. FIX (next): add a persistent distfiles cache (env PHOENIX_DISTFILES, default repo .distfiles/ gitignored) that _fetch_extract checks first (copy if cached) + downloads from the artfiles mirror only if missing + populates the cache.
 **NEXT:** (1) implement the distfiles cache + artfiles mirror in the xorg fetch (owner's ask); (2) full clean gcc-16 --with-ports build (with xorg now unblocked) → a COMPLETE gcc-16 rootfs (system+ports+X); (3) gcc-16-games stretch (tools-script toolchain override + Mesa-under-gcc-16). Deferred features F1 (Q2 GLES3) / F2 (Q3 Vulkan). Game-bug digs (Q1 tex, vkQuake fire/light, USB, Q2 slow-load) interleave.

2026-08-23 (session ~222 — gcc-16 migration STARTED (assessment + nfsroot build launched); 2 renderer features scheduled by owner):
 No owner feedback in git. Owner (live) added TWO renderer feature requests, then said "schedule both for next sessions, you choose when": **F1 = yQuake2 GLES 3.0 renderer** (`+set vid_renderer gles`; V3D supports GLES 3.1 + some 3.2 → "modern look"); **F2 = quake3e Vulkan renderer** (parallels its GL1; V3DV works via vkQuake). Both leverage the existing V3D GL/V3DV stack — DEFERRED to future sessions (see Tier-2 features below).
 - **gcc-16 ASSESSMENT (per [[project_gcc16_rebase]]):** toolchain built (.toolchain-gcc16), core gcc-16-clean under -Werror (4 fixes pushed), -std=gnu17 pinned, BOOT-VERIFIED on HW (kernel+drivers+nfs+lwip→psh). BLOCKERS for a "full clean rebuild of EVERYTHING": (a) tools/ GPU+game build scripts HARDCODE .toolchain (gcc-14) — they don't honor PHOENIX_AARCH64_TOOLCHAIN, so GPU/games would need script overhaul + Mesa-under-gcc-16 verify to build under gcc-16; (b) xorg blocked by x.org CDN (infra, not gcc-16) → skip-xorg; (c) disk 30G free (tight); (d) prepare-buildroot SKIPPED for existing .buildroot-gcc16 → committed sibling fixes need syncing (or full-clean). Games are STATIC → gcc-14 games run fine on a gcc-16 kernel, so a gcc-16-core + gcc-14-games rootfs is a valid testable deliverable; gcc-16-games is a stretch (F-gcc16-games).
 - **STARTED: incremental gcc-16 nfsroot build** (detached pid 2450917, log $CLAUDE_JOB_DIR/tmp/gcc16-nfsroot-build.log): synced my committed nfs_ops.c fix into .buildroot-gcc16, launched `--variant nfsroot --scope core` with the gcc-16 toolchain override (reuses the built ports incl. libnfs → no xorg re-hit). Refreshes the boot-verified gcc-16 nfsroot with the nfs fix. MONITOR next turn (check log + pid).
 - **✅ gcc-16 nfsroot build COMPLETED CLEAN (2026-08-22 23:15):** no errors; gcc-16 kernel8.img + loader.disk + _fs/root refreshed; the built nfs binary has 0 "fh-cache evict" strings ⇒ my nfs fix compiled in under gcc-16. _boot/.../rpi4b/ has firmware (start4.elf/fixup4.dat/overlays) + gcc-16 Phoenix files (kernel8/loader.disk/dtb/config/armstub) — TFTP-ready. Rootfs = .buildroot-gcc16/_fs/aarch64a72-generic-rpi4b/root (served over NFS). NOTE: exported artifacts/rpi4b/rpi4b-sd.img is incidental; for netboot use _boot (TFTP) + _fs/root (NFS).
 **NEXT (assemble + stage the testable gcc-16 rootfs):** (1) the gcc-16 _fs/root has NO game binaries (games are hand-staged, not image-built) → stage the (gcc-14 static) game binaries + data + hand-staged tools into a gcc-16 NFS export (mirror what /srv/phoenix-rpi4-nfs has), OR rsync _fs/root → a gcc-16 export + overlay games; (2) netboot-serve it (RPI4B_NETBOOT_TFTPROOT=.buildroot-gcc16/_boot/.../rpi4b + point NFS at the gcc-16 rootfs) + one Pi boot-verify to psh; (3) tell owner it's ready. Then: full-clean --with-ports skip-xorg gcc-16 for completeness; gcc-16-games stretch (tools-script toolchain override + Mesa-under-gcc-16). Deferred features F1 (Q2 GLES3) / F2 (Q3 Vulkan). Game-bug digs (Q1 tex, vkQuake fire/light, USB, Q2 slow-load) interleave.

2026-08-22 (session ~221 — Q3 u_vbuf crash FIXED + 2 UX fixes; deployed to current build; gcc-16 next):
 No owner feedback. Worked the execution queue. **Q3 crash FIXED:** root = Mesa `u_vbuf_translate_buffers` used the translate object from translate_cache_find() unchecked; translate_generic_create() returns NULL on aarch64 for an unsupported combo (illegal int format, or CALLOC fail) — upstream never checks (x86 has translate_sse) → NULL `tr->set_buffer/run` deref = the SIGSEGV. Added a NULL check → PIPE_ERROR (caller skips the draw cleanly). external/mesa f342ce50282 (local). Rebuilt libv3d + relinked all 3 GL games. **2 owner UX fixes:** (a) ram-stage-play now prints a start banner + "preloading… N MiB" every 16 MiB + "DONE creating RAM disk" (coord 977e1be) — no more silent preload; (b) SDL2 phoenix video: no-op SetRelativeMouseMode (relative motion already native) + SetWindowGammaRamp → silences the two Q2 warnings (ports ca16da9). 
 - **DEPLOYED to current (gcc-14) build (23:05, Pi off):** quakespasm-sdl + yquake2 + quake3e relinked (u_vbuf crash-fix) + ram-stage-play rebuilt, all restaged to /srv/phoenix-rpi4-nfs. Current build now has: Q2 fb-recovery, vkQuake 8M stack, Q3 u_vbuf fix, ram-stage UX. PENDING deploy (fold into gcc-16): SDL2 mouse/gamma (needs libSDL2 rebuild), nfs print-removal (needs --scope core). SDL2 change ca16da9 NOT yet build-verified — the gcc-16/libSDL2 rebuild verifies it.
 **NEXT — gcc-16 migration (owner's overnight goal):** per [[project_gcc16_rebase]] the gcc-16 cross-toolchain is BUILT in .toolchain-gcc16/. Steps: (1) kick a gcc-16 build (RPI4B_BUILDROOT=.buildroot-gcc16 + PHOENIX_AARCH64_TOOLCHAIN=.toolchain-gcc16) to surface -Werror breakages; (2) fix them iteratively (gcc-16 defaults C23 → -std=gnu17 pin already committed 8b0c29c; expect stringop-truncation etc.); (3) full clean rebuild under gcc-16 (incorporates ALL committed fixes above); (4) build a gcc-16 NFS rootfs + stage for owner testing. Large multi-turn effort — start fresh, monitor. Remaining game-bug digs (Q1 textures, vkQuake 2a fire-pits/2b lighting, USB warm-reboot, Q2 slow-load) continue after/interleaved.

2026-08-22 (session ~220 — owner finished testing (Pi FREE); round-2/3 issues captured + 2 more fixes; owner set gcc-16 overnight goal):
 Owner tested all 4 games + reported round-3: Q2/Q3 playable; Q2 VIDEO-setting change FREEZES the screen (renderer restart doesn't re-acquire fb); Q2 loads much slower than Q1/Q3; Q3 CRASHED mid-combat. Then: "Pi is off, use it; fix all issues step-by-step; when done continue gcc-16 migration overnight → full clean rebuild under gcc-16 → gcc-16 NFS rootfs for testing." Full analysis + EXECUTION QUEUE in docs/inprogress/2026-08-22-owner-test-round2-issues.md.
 - **Q3 crash ROOT-CAUSED (no-Pi):** Mesa gallium `u_vbuf_translate_begin` NULL-deref — disasm shows `ldr x5,[x22,#928]; blr x5` with x22=NULL (indirect call through a NULL context/mgr at +0x3a0), hit when combat geometry needs vertex-format translation. libv3d/Mesa GL-driver bug, not a game bug. Needs the exact NULL hook + repro.
 - **Q2 fb-recovery FIXED (impl):** phxgl_init made idempotent — reuse existing context+scanout FBOs on vid_restart instead of building a 2nd context that can't re-claim the scanout (phoenix-rtos-ports 6f0481f). Pending Pi validation.
 - **Q2 slow-load:** captured (RAM-stage 50MB + per-texture GL1 upload; hypothesis texture-upload-bound). Low pri.
 - Plus from round-2 (all committed): vkQuake 2c crash = worker stack 512K→8M (coord 1606e5c); nfs noise print removed (fs b017513); vkQuake torches already fixed; Q1 box-texture + vkQuake 2a/2b + USB captured with repro plans.
 - **BUILT + DEPLOYED (2026-08-22 22:49, Pi off so restage safe):** yquake2 + quakespasm-sdl relinked with the fb-recovery glctx fix, vkquake rebuilt with the 8 MiB worker stack; all 3 restaged to /srv/phoenix-rpi4-nfs (compile+link clean). VALIDATION NOTE: both fixes are EVENT-triggered (Q2 fb-recovery needs an in-game vid_restart; vkQuake crash needs a playthrough) → NOT deterministically validatable by an automated psh-cycle (the trigger is in-game console/menu input, not psh). A boot-smoke cycle only exercises FIRST-init (the fb-recovery early-return is guarded on re-init) so it wouldn't test the fix. ⇒ these need the owner's interactive re-test OR a scripted in-game-stdin harness (TBD). Deployed + ready.
 **NEXT (Pi free):** (1) Q3 u_vbuf NULL-deref — no-Pi dig to pin the NULL hook at +0x3a0 in u_vbuf_translate_begin → libv3d fix (a real GL-driver bug, analyzable without in-game interaction); (2) Q1 box textures (frame-dump SSIM vs host — Pi, automatable); (3) USB warm-reboot (cold-vs-warm Pi cycle, automatable); (4) Q2 slow-load timing; (5) vkQuake 2a/2b. THEN gcc-16 migration → full clean rebuild (incorporates all committed sibling fixes) → gcc-16 NFS rootfs for owner testing. nfs print-removal (fs b017513) + flagship rpi4-quake/rpi4-vkquake all fold into the gcc-16 clean rebuild.

2026-08-22 (session ~219 — owner testing the 4 games; prepared flagship-binary finalization, held heavy work off the shared resources):
 Owner asked to manually test quakespasm/quake2/quake3/vkQuake; I gave them the launch rundown + the KEY caveat (run the fixed per-game binaries `/bin/quakespasm-sdl`, `/usr/bin/quake2`, `/usr/bin/quake3`, `/bin/vkquake` — NOT the stale flagship `rpi4-quake`/`rpi4-vkquake`, which still have the old bugs). Brought the netboot server back up (was down; dnsmasq pid 2440360, NFS exported, fixed binaries intact ≥21:33). No owner feedback in publish/main.
 - **FLAGSHIP FOOTGUN + FINDING:** `usr/bin/rpi4-quake` + `usr/bin/rpi4-vkquake` (both 16:50) predate the fixes → still broken. They are BAKED INTO `.buildroot/_fs` (base image), so hand-staging a fixed copy into the export REVERTS on the next `sync-netboot-tree` (export-drift trap [[project_netboot_export_bin_resync]]). ⇒ the ONLY persistent fix is a `--with-showcase --with-vkquake` image rebuild (writes `.buildroot/_fs`) then a sync. PREP DONE: rebuilt `libquakespasm.a` (22:15, links fixed libv3d) + `libvkquake.a` (22:00) so that rebuild will relink both flagship binaries fast. Deliberately did NOT run the heavy build now: it's a >10-min host compile that could degrade NFS-serving to the Pi during the owner's LIVE test, and the export sync would disturb their mounted root. Correctly a POST-TESTING op. `rebuild-rpi4b-fast.sh` confirmed to write only `.buildroot` (no export/sync refs) so it's safe once the Pi is free.
 - **OWNER TEST ROUND 2 (Q1 + vkQuake) — 4 issues captured (no-Pi analysis done, Pi-repro queued) in docs/inprogress/2026-08-22-owner-test-round2-issues.md:** (1) Q1 quakespasm "works very well" except broken textures on SOME health/ammo boxes — hypothesis = small/NPOT UIF tiling edge case (same family as q3dm7/quake2-speckle); (2a) vkQuake start-map FIRE-PIT flames missing (torches fixed, fire pits use a different particle/sprite path); (2b) vkQuake lighting wrong at spots (dark/unnatural/flicker; owner VIEWPOS (63 1696 104)) — lightmap/lightstyle path; (2c) vkQuake CRASH after play = memmove overrun in libphoenix stdio write_buffer (file.c:554) during Con_Printf of a centerprint (addr2line'd: CL_ParseServerMessage→Con_LogCenterPrint→Con_Printf→write_buffer→memmove→Data Abort far=0xfdef000; then signal-trampoline double-fault; threads 58+61 both faulted) — hypothesis = concurrent Con_Printf from vkQuake worker threads corrupting stream->bufpos OR worker stack overflow; ✅ ROOT-CAUSED + FIXED (pending Pi validation): NOT a stdio race (libphoenix write path IS locked) — a WORKER-THREAD STACK OVERFLOW. vkQuake workers spawn via the port SDL shim which gives each a fixed 512 KiB stack (PL_THREAD_STACKSZ, pl_phoenix_sdlcompat.c); the deep render/BSP/lightmap worker call chains overflow it → corrupt adjacent heap (the stdout FILE bufpos) → the memmove crash. Raised 512 KiB→8 MiB. Needs vkquake rebuild+restage+replay to confirm. (Separate latent footgun: libphoenix default pthread stack = 4 KiB, harmless for shim-based vkQuake but a hazard for future default-stack pthread ports.); (3) USB enumeration FAILED after reboot (xhci completion code 36→19, "Fail to get device descriptor", enum failed 3 attempts) — hypothesis = stale VL805/xHCI state after WARM reboot [[project_pi4_xhci_crcr_stale_after_hcrst]], full power-cycle clears it. Also removed a noisy per-eviction NFS debug print (owner-flagged; phoenix-rtos-filesystems b017513, needs --scope core build+deploy).
 **NEXT (post owner-test, Pi free):** (a) `./scripts/rebuild-rpi4b-fast.sh --with-showcase --with-vkquake` → verify flagship rpi4-quake/rpi4-vkquake carry the fixes → `netboot-server-up.sh` → one Pi cycle each; (b) also picks up the nfs b017513 noise removal if `--scope core`. Then work the round-2 issues in priority order: Q1 box textures (frame-dump SSIM vs host) → vkQuake 2c crash (libdbg/gdb repro of the stdio overrun) → USB-after-reboot (cold vs warm) → vkQuake 2a/2b (V3DV render digs). Act on any new owner test-result report first.

2026-08-22 (session ~218 — ★ Issue 2 (vkQuake torches) RESOLVED = stale binary; master-plan Tier-0 reconciled):
 No owner feedback (publish/main in sync). Continued the owner's Tier-0 GL/GPU bugs. **Issue 2 (vkQuake torches missing) RESOLVED + HW-verified:** root cause = STALE BINARY — external/vkquake HEAD already carries `d3e329c "present opaque models with alpha=1 (fixes invisible torches)"` (the flame is an opaque alias model that pre-fix presented at alpha=0 → invisible); the owner's tested binary was a P7 RELINK of pre-d3e329c objects. FIX = clean rebuild from HEAD (rm /tmp/vkqobj + recompile 83 TUs incl. r_alias.c, --link) vs current libv3dv/libv3d → staged /bin/vkquake (22:00). HW-verified (netboot +map start, 4920+ present frames, no hang): grab 20260822-200435-vkq-torchfix shows a flame RENDERING + ANIMATING (bright in tick, dim in final) on the right wall where the owner's 14:37 grab shows none — same camera → render-content difference. A few intermittent binner wedges (mmu_ill=0x800066a5) occurred but reset+drop recovered each (separate tracked thread-B issue, not the torch bug, not a hang). FOLLOW-UP: /usr/bin/rpi4-vkquake (flagship 12.8MB) still predates the rebuild → run /bin/vkquake or rebuild rpi4-vkquake --with-showcase. Also reconciled MASTER-RECONCILED-PLAN Tier-0: the stale "O2 striping = benign capture tearing" claim was half-wrong — render WAS fine (SSIM via tiling-aware GPU blit) but a REAL tiled-scanout display bug existed underneath (=Issue 1, now fixed).
 **NEXT:** Issue 3 (X11 GL Y-offset, lowest pri, needs live repro — client present path already cleared) OR rebuild rpi4-vkquake --with-showcase to carry the torch fix into the flagship binary. Then Tier-1 finalization (P8 remaining tools/→ports: v3d-driver-port→devices, ffmpeg, python, game ports) or Tier-2 greenlit goals.

2026-08-22 (session ~217 — ★ OWNER-LIVE GPU regressions: Issue 1 (GL scanline garbage) ROOT-CAUSED + FIXED):
 Owner manually testing (uses the HDMI grabber AS the monitor → broken HDMI = cannot see the app). Reported 3 GL/GPU regressions from recent GPU work; triaged in docs/inprogress/2026-08-22-gl-gpu-regressions-owner-reported.md.
 - **RECONCILIATION — line 550's "GLQuake HDMI scanline-tear = benign animated-content capture-tearing, not a render bug" is WRONG.** Owner: old GL grabs were clean, this is a RECENT regression, and it's the LIVE view not a capture artifact. Confirmed by root-cause below. Do not dismiss GL scanline-shred as capture-tearing again.
 - **Issue 1 (BLOCKER) ROOT-CAUSED by code inspection:** the fullscreen GL scanout RT is UIF-TILED; the HVS display scans only linear → reads tiled-as-linear → horizontal-shred garbage (all GL games). Chain: Mesa main/renderbuffer.c:276 stamps PIPE_BIND_SAMPLER_VIEW on EVERY renderbuffer → the scanout renderbuffer carries it → the q3dm7 force-RASTER gate's `!(bind & SAMPLER_VIEW)` exclusion (commit 4363822955b) now also skips the scanout RT → left tiled. Explains all controls (frame-dump SSIM 0.993 = GPU-aware blit; vkQuake clean = V3DV not gallium; X11-GL clean = DRAM-FBO readback). NOT the page-flip/pan (all pa/pitch/nbuf/virt_h verified consistent; no wedge).
 - **FIX implemented (pending HW grab):** added v3d_phoenix_peek_next_scanout() to the winsys (non-destructive read of the next_scanout one-shot set right before the scanout renderbuffer's glRenderbufferStorage); v3d_resource.c gate now forces RASTER when `peek_next_scanout() || !(bind & SAMPLER_VIEW)` — scanout RT → linear, sampled atlas → tiled (q3dm7 fix preserved). libv3d-phoenix.a rebuilt clean (link PASS). Files: tools/v3d-driver-port/v3d_phoenix_winsys.c, external/mesa/.../v3d_resource.c.
 - **✅ HW-VERIFIED + PUSHED:** quakespasm-sdl relinked vs fixed libv3d, netboot 1920x1080 +map start → CLEAN textured GLQuake frame (walls/armor/candle/viewmodel/HUD), zero scanline shred, 0 GPU faults (grab 20260822-193816-qs-sdl-v3d-tilefix-tick.png; UART triple-buffer scanout, no wedge). coord 571d57f pushed publish; external/mesa 34a448d6a29 local (mesa clone has no publish remote). Staged /srv/phoenix-rpi4-nfs/bin/quakespasm-sdl + wrote id1/config.cfg (1920x1080 fullscreen). Issue 1 CLOSED.
 - **Issue 3 (X11 GL Y-offset) — dst-y guess REFUTED, DEFERRED (lowest pri):** tools/x11-port/gl_x11_window.c present path is correct end-to-end (FBO 640x480, correct bottom-left→top-left flip, XPutImage full-window; draw_scene identity-projection centered fan → no offset). The reported "content low / black top band" is NOT in this client → server-side Xphoenix window geometry or a different/older GL client. Needs live repro; non-blocking.
 - **Issue 2 (vkQuake torches) — SCOPED, strong lead (NOT the #51 wedge):** vkq-semafix grab confirms vkQuake RENDERS the start map fully (viewmodel alias model fine) → flame-specific rendering bug, not the hang. external/vkquake HEAD already has d3e329c "fixes invisible torches" (Aug 4); staged rpi4-vkquake is Aug 22 yet owner's grab shows none → either stale binary OR a later V3DV Mesa-rebase change (0f598ef drop-DISABLE_TFU / alias-fullbright path) undid it. NEXT turn: clean-rebuild rpi4-vkquake from HEAD (non-INCR) vs current libv3dv/libv3d, restage, netboot+grab → present=stale-binary-DONE, missing=bisect d3e329c..HEAD + broadcom/vulkan rebase.
 - **ALL 3 GL games restaged with the fix + quake2 HW-reconfirmed:** yquake2 relinked (21:49, + launcher c411b87) + quake3e relinked (21:50, sudo-staged) against fixed libv3d. quake2 netboot grab 20260822-195420 = CLEAN console/Q2-logo/"Outer Base" map-load, no shred, 0 faults → owner's original "Quake2 no visible image" RESOLVED (3D demo view not captured only due to slow 50MB RAM-stage filling the 120s window; identical scanout path to the proven-clean quakespasm).
 **NEXT (fresh turn):** Issue 2 vkQuake torch rebuild-and-test (above). Also fixed quake2-launcher video args (c411b87) + nfs-fs readdir ./.. (ee32037) earlier this session. quake2/quake3 inherit the libv3d RASTER fix but need relink+restage to pick it up.

2026-08-22 (session ~216 — #51: TFU hypothesis RULED OUT (real fault regs mis-read); pristine export completeness fixed):
 Subagent delivered a strong #51 analysis that CORRECTED the prior dig: **`mmu_ill` in the TIMEOUT lines is the MMU_ILLEGAL_ADDR *scratch-config* reg (0x1230) = what the winsys WROTE, NOT the fault address** — the real fault is in VIO_ADDR(0x1234)/VIO_ID(0x122c), which the winsys never reads. So the prior "fault VA" leads were red herrings (different scratch-page placements per boot). Also `ovf_armed=0`/no INT_OUTOMEM ⇒ tile-alloc-pool-overflow REFUTED.
 - Proposed zero-rebuild experiment: `V3D_DEBUG=disable_tfu` (reverts V3DV off the HW-TFU path, mesa 0f598ef regression suspect). HW-TESTED (vkq-notfu2): **RULED OUT** — with disable_tfu vkQuake reaches "Quake Initialized" (slightly further) but the FIRST RENDER FRAME STILL WEDGES (BIN/RENDER TIMEOUT, watchdog). ⇒ NOT the TFU path; it's the GENERAL V3D binner wedge on the first render frame (same class as q3dm7/glamor).
 - **DEFINITIVE next step (subagent's instrumentation patch, ready in the ~215 transcript):** apply the env-gated `V3D_MMU_DBG=1` winsys patch (reads the REAL VIO_ADDR/VIO_ID + MMU_CTL fault bits + walks the BO table) → rebuild libv3d-phoenix.a + relink rpi4-vkquake → ONE HW cycle pins the real fault VA + faulting client (TFU/PTB/CLE/L2T…) + owning BO. THAT localizes the deep binner-wedge root cause (which eluded prior digs precisely because they read the scratch-config reg). Deferred to a fresh turn (delicate winsys patch + 2 rebuilds; not at the end of a mega-session).
 - **PRISTINE EXPORT completeness FIX:** the first pristine build dropped legit hand-staged tools not in the default build. Re-added to /srv/phoenix-rpi4-nfs: bash (P2), python3 (CPython)+/lib/python3.14 stdlib, wpa_supplicant, wpa_cli, pty-run; re-exported. Updated make-pristine-nfs-export.sh's preserve-set to include them (future pristine builds stay complete). Removed the temp vkq-notfu.sh experiment script. Export re-verified complete (games+X+CLI ports+bash+python3) + clean.
 **NEXT (fresh turn): the #51 instrumentation dig** — apply V3D_MMU_DBG patch, rebuild libv3d+vkquake, HW-test → real fault VA/client → root-cause the binner wedge (helps vkQuake + q3dm7 + glamor).

2026-08-22 (session ~215 — #51 vkQuake: dispatched the V3D binner-wedge root-cause dig):
 No owner feedback. #50/#52 done (rootfs + GLQuake). Highest priority = #51 vkQuake (owner wants it back). Dispatched subagent ae7cfac163c96d4e0 for the focused binner-wedge root-cause: decode mmu_ill=0x8002843e (VA in bytes + owning region), analyze the winsys bin/render + tile-alloc/BINOVF + VA-recycle/TLB path, propose ONE testable fix (candidates: enlarge/relocate BINOVF pool, V3D_VA_NO_RECYCLE=1, TLB barrier, re-enable DISABLE_TFU for V3DV) + an env-gated instrumentation patch to pin the faulting allocation in one HW cycle. Briefed to BUILD ON the prior thread-B / project_quake3_lightmap_uif_xor characterization (same mmu_ill wedge; consistent VA on bin+render; NOT the L2T-flush race; leads = stale-PTE/VA-recycle/tile-alloc-pool). Running background; I'll review + HW-test its proposal (multi-trial) when it reports.
 - Context: the wedge is a GENERAL V3D binner bug (q3dm7 GL + glamor-X M3a + vkQuake all hit it) — deep/HW-marginal/prior-stalled. vkQuake hits it PERSISTENTLY on the first V3DV frame → hang. Fixing it helps ALL three.
 **NEXT:** review the subagent's mmu_ill decode + proposed fix/instrumentation → HW-test on the fresh rootfs (rpi4-vkquake, multi-trial). Manifest 2026-08-22-clean-rebuild-pristine-rootfs snapshotted (clean-build state banked).

2026-08-22 (session ~214 — #51 vkQuake regression CHARACTERIZED on the fresh rootfs: persistent binner wedge):
 HW-tested rpi4-vkquake on the pristine rootfs. It INITIALIZES FULLY: V3DV device (1920x1080 fb0 scanout, no WSI), all render resources (renderpass/framebuffer/cmdpool), ALL shader modules compile (basic/world/alias vert+frag). Then on the FIRST FRAME render it WEDGES the GPU: `BIN TIMEOUT ... mmu_ill=0x8002843e` → "GPU wedged — true reset + drop this frame" repeating (drops=1,2,3) → `RENDER TIMEOUT ct1cs=0x118 errstat=0x1000` → `WATCHDOG tick #1 (possible hang)`. ⇒ **vkQuake never presents a frame → hangs. Root cause = the tracked q3dm7-class V3D BINNER WEDGE (mmu_ill), but here PERSISTENT (every frame) not intermittent** — which explains the owner's "was running almost perfectly, now fails" (the wedge went from occasional-recovered to always-on for the V3DV render path).
 - Leads for the #51 fix (deep V3D dig, owner-attended-adjacent): the wedge fault VA/opcode is consistent (mmu_ill=0x8002843e) across bin+render; connects to [[project_quake3_lightmap_uif_xor]] (q3dm7 wedge) + the M3a glamor-X first-CL wedge. Possible regression source = a recent V3DV/Mesa change (Mesa 26.2.0 rebase; external/mesa "drop Phoenix DISABLE_TFU force" 0f598ef) altering the V3DV render/BO/VA pattern → now consistently triggers the mmu_ill. NEXT (focused dig, likely a subagent): instrument the wedge (real fault VA in bytes + owning BO/CL block + tile-alloc/BINOVF extent) on the vkQuake first frame; compare V3DV render-path BO/VA layout vs the last-known-good vkquake 0d8dc54 state; bisect the Mesa-rebase/DISABLE_TFU change. Multi-trial (wedge may vary).
 **STATE for the owner's session:** clean rootfs READY (#50 done); Quakespasm/GLQuake WORKS (#52 done); vkQuake (#51) characterized = persistent binner wedge on first V3DV frame → hangs; fix = the deep binner-wedge root-cause (tracked, hard). Manifest of the clean-build state still to snapshot.

2026-08-22 (session ~213 — ★ pristine rootfs BOOT-VERIFIED + Quakespasm WORKS (#50, #52 DONE)):
 Boot-verified the pristine export on HW. GOTCHA: first netboot → NFS takeover FAILED ("takeover mount attempt 10 failed") because the pristine-export SWAP (mv) changed the export-dir inode → STALE nfsd. FIX (per [[project_nfs_erange_stale_server_state]]): `sudo exportfs -ra` + `sudo systemctl restart nfs-server`. After re-export: `nfs-fs: registered / (takeover)` → clean boot. **ANY future export dir-swap needs a re-export.**
 - **#52 Quakespasm/GLQuake WORKS on the fresh rootfs (owner deliverable):** `rpi4-quake` → "Initializing QuakeSpasm (Phoenix/V3D)", found /usr/share/quake/id1/pak0.pak (basedir=/usr/share/quake), "Quake Initialized", "Playing demo from demo1.dem/demo2.dem", 0 faults. HDMI grab is scanline-torn = the KNOWN animated-content capture-tearing (not a render bug; authoritative render-correctness = coherent frame-dump SSIM, previously q1/q2/q3 all correct). ⇒ GLQuake still works. #52 DONE.
 - **#50 clean rootfs DONE:** pristine export boots + NFS-mounts + runs a game, clean+complete. rpi4-quake auto-finds data at /usr/share/quake (searches /ramtmp,/tmp,/usr/share/quake,/opt,/). Old cruft still backed up at /srv/phoenix-rpi4-nfs.PREV-cruft.
 **NEXT: #51 vkQuake** — netboot rpi4-vkquake on the fresh rootfs, characterize current state (owner: "we had it running almost perfectly, now fails"; docs say hangs-after-menu). Then report both games for the owner's session + snapshot a manifest of the clean-build state.

2026-08-22 (session ~212 — ★ CLEAN REBUILD SUCCEEDED + PRISTINE NFS EXPORT BUILT):
 **RESUME4 EXIT=0** — the full clean rebuild completed with all 3 latent xorg bugs fixed. Fresh _fs/root complete: core, full X11 (Xphoenix+wmaker+xterm+xcalc+xclock+dillo), games (rpi4-quake=GLQuake, rpi4-vkquake), id1 Quake1 data, CLI ports.
 - **Built the PRISTINE NFS export** (scripts/make-pristine-nfs-export.sh, committed 4b0cf2d): safe SWAP — fresh _fs/root + overlaid ONLY legit hand-staged game/media content (Q2/Q3 engines+data 96MB, launchers quakespasm/vkquake/ram-stage-play, e4 video tools) → /srv/phoenix-rpi4-nfs; DROPPED all junk (GPU experiments, test litter, ML .bin, 60+ qdet-*.raw, 67MB nfsbench, broken `&1`/aaaa-symlink). Old cruft backed up at /srv/phoenix-rpi4-nfs.PREV-cruft (delete after owner confirms). Verified: root has only legit dirs (bin/usr/etc/... + linuxrc netboot-init), all games+data+X+core present, 457MB. The owner's #51/#52 targets (both Quake1) are fully in the fresh build (rpi4-vkquake/rpi4-quake + id1).
 - CLEAN-BUILD HARDENING banked (all real latent bugs the framework-migration never clean-verified): xorg_libs mirror-override (f450916); xorg_apps libiconv dep (db887a3); xorg_apps split-tree HTTPS fetch (6784881).
 **NEXT:** boot-verify the pristine rootfs via the game HW-tests → #52 Quakespasm (GLQuake) + #51 vkQuake on the fresh rootfs → report game state for the owner's session → mark #50 done + snapshot a manifest. (Note: a future netboot sync-netboot-tree rsync [no --delete] re-lays fresh _fs/root over the pristine export + preserves the game overlay → stays clean.)

2026-08-22 (session ~211 — clean-rootfs: fixed xorg_apps tarball-fetch bug (2nd latent); RESUME4 running):
 RESUME3 (libiconv fix) built xcalc OK but failed fetching xclock — ROOT CAUSE (2nd real latent xorg_apps bug): the demo-app tarballs are SPLIT across x.org trees (xlogo only /releases/, xedit only /archive/, xclock both, xcalc /archive/) and the port fetched all from a SINGLE http:// /archive/ base (unreliable) → couldn't cover the split. FIX (ports 6784881): source= → https /archive/ (xcalc, sha unchanged); _fetch_extract tries BOTH https bases (/releases/ then /archive/) per app; XORG_APPS_BASE override. Verified all 4 apps reachable via HTTPS across the two trees; xbill fetches from github (no x.org risk). Killed RESUME3, launched RESUME4 (--scope auto, x.org defaults). Monitor biojctahw watches FAILED/completion/export.
 - CLEAN-BUILD HARDENING so far this saga (all real latent bugs the framework-port migration never clean-verified, + resilience): xorg_libs XORG_XBASE/ARCHIVE/CBB mirror override (f450916); xorg_apps libiconv dep (db887a3); xorg_apps HTTPS+multi-base fetch (6784881). x.org outage recovered.
 **NEXT (RESUME4 completion):** verify _fs/root complete → pristine export → HW-test games (#52 quakespasm, #51 vkQuake) → report → manifest. Next possible blocker: xterm/windowmaker/dillo/showcase-stage/_user-games (fix if a latent issue surfaces).
...
2026-08-25 (session ~267 — g35 release rebuild kicked off to validate + bake in ALL recent fixes; wmaker dock deferred):
 No owner feedback. wmaker: investigated the missing dock — config is CORRECT (DisableDock=NO; WMState has the Logo.WMDock appicon at top-right "-64,0") so it's a WRaster tile/logo RENDERING issue (deep + decorative), NOT config → DEFERRED (core wmaker works; don't guess-chase decorative rendering). Backlog otherwise thin/blocked (Quake2 owner-gated + no .dm2 demos for autonomous repro; deep V3D q3dm7 wedge banked; G5 publish held). Concrete on-plan action instead: **kicked off g35 full Docker release rebuild** (build-sd-in-docker.sh, monitor bfljyjqqn) — a clean-container reproducible build serving committed state (ports 5b13622 wmaker data+PNG + de63acf fontconfig-scalable-dir, project 1ed20f0 bash, coord startx/startx_gpu launcher). Validates all recent port changes integrate in a FULL --with-showcase --with-ports build (build-validate discipline) + produces the current release artifact for the held G5. Docker = host CPU (safe under spend-limit); ~40-60min from scratch (toolchain bootstrap + all ports + showcase).
 **NEXT (monitor fires):** confirm g35 BUILD_RC=0 + docker-out/rpi4b-sd-2part.img (new sha) → the release artifact now carries all fixes. If a recent port change broke the full build, diagnose+fix (the isolated build-port.sh passed, but full-showcase integration is the real test).

2026-08-25 (session ~268 — cron refreshed; g35 release rebuild still running):
 No owner feedback. Cron age unknown + many days elapsed → refreshed the heartbeat cron to guarantee loop continuity: CronDelete bc450b68 + CronCreate **bf338bd7** (same 7,22,37,52 schedule + prompt; fresh 7-day clock). g35 full Docker release rebuild still building (monitor bfljyjqqn; at #8 ~717s = toolchain/core phase; ~40-60min total). Held off other heavy work to not compete with g35 for host CPU; backlog otherwise thin/blocked (wmaker core done + dock deferred as decorative rendering; Quake2 owner-gated; deep V3D digs banked). NEXT: g35 monitor fires → verify BUILD_RC=0 + new image sha (release artifact carries all recent fixes: gcc-16/fontconfig/wmaker-data+PNG/startx_gpu/bash).

2026-08-25 (session ~269 — ⚠️ DISK-FULL incident from un-pruned Docker builds → RECOVERED + root-fixed):
 The g35 rebuild filled `/` to 100% (450G/468G) — root cause: repeated `--no-cache` Docker release builds (g32→g35) accumulated **332GB Docker images + 69GB build cache**, never pruned. Fallout: my progress commit failed (git couldn't write), and g35 died mid-build (the `std.cc`/`sqrtl` "failed to read compiled module" errors were disk-full artifacts, NOT a real C++23 bug). **RECOVERY:** `docker system prune -af` reclaimed 137.6GB; trimmed old HDMI snapshots (19G→258M) + stale job-tmp scratch (vidcap 22G etc.) → disk now ~95G/468G ~21% (300G+ free). **ROOT FIX (coord):** build-sd-in-docker.sh now prunes its image + build cache after export (`docker image rm phoenix-rpi` + `builder prune -af` + `image prune -f`) so repeated release builds can't refill the disk. Cron already refreshed to bf338bd7 this session. NEXT: g35 was a false failure (disk) — re-run only if a current-artifact release build is wanted (now safe: it self-cleans); the recent fixes are already committed+pushed + isolated-build-verified. Lesson: monitor host disk when chaining Docker `--no-cache` builds; each is ~20GB.

2026-08-25 (session ~270 — g35b release rebuild re-launched (disk-safe now); validates all fixes end-to-end):
 No owner feedback. Disk healthy (351G free) + release script self-prunes now → re-launched the g35 build that disk-full killed last turn (g35b, monitor b0f4nu02y). It's the interrupted end-to-end validation: a full clean --no-cache Docker build serving committed state (ports 5b13622 wmaker-data+PNG + de63acf fontconfig, project 1ed20f0 bash, coord startx/startx_gpu + build-sd-in-docker self-prune 56541ef). Confirms all recent port changes integrate in a full --with-showcase build + produces the current release artifact for the held G5 + exercises the new self-prune fix. One build needs ~20-40GB << 351G free, so no refill risk. NEXT (monitor fires): verify BUILD_RC=0 + new image sha + that the script pruned its image/cache afterward (disk stays lean).

2026-08-25 (session ~274 — g35b CLOSED-verified; pivoted to owner #1 = Quake2 malformed textures; deep analysis banked):
 g35b DONE + verified (host==container sha 4a0735f5, self-prune reclaimed disk). Advisor priority-correction: O1 glamor is ALREADY HW-confirmed+closed (checked before spending a redundant Pi cycle), and the owner's most-recent EXPLICIT ask — the **Quake2 malformed-texture bug** (gated on "finish wmaker+readme", both done) — is the true #1, and it's NOT owner-gated after all (gl_texturemode is a LAUNCH cvar; the q2 frame-dump harness is autonomous). Analyzed owner log 20260825-135418: demo1 (Outer Base) viewpos -141 974 -81; winsys TFU diag shows 64×128/64×64 textures verdict **LINEAR!** + VERTICAL-MISMATCH while Mesa REQUESTED UIF_NO_XOR (ioa iofmt=6) — BUT those jobs have NON-raster src (icfg ifmt=0xE) so the vcheck's raster-src assumption may make it a false-positive on in-place MIP jobs (matches prior session). The 08-22 Pi timedemo was CLEAN (SSIM 0.993) ⇒ bug is view/state-specific or an 08-22→08-25 regression. Source analysis is inconclusive alone. Full analysis + exact next experiment: **docs/inprogress/2026-08-25-quake2-malformed-textures.md**. NEXT (1-2 Pi cycles): run yquake2 q2demo1 TCP-sink capture default vs `+set gl_texturemode GL_LINEAR` → malformation-gone-with-LINEAR ⇒ mip-path bug; persists ⇒ base-level raster→UIF. (Pi-side sink cvar spelling + whether the `quake2` ram-stage wrapper forwards argv still TBD — noted in the doc.)

2026-08-25 (session ~275 — Q2 texture bug: TFU mismatch CONFIRMED false-positive-on-mip-jobs; coherent-capture discriminator launched):
 No owner feedback. Ran Pi cycle q2tex-default (quake2 demo1) — reproduced the SAME 64×128/64×64 TFU VERTICAL-MISMATCH, but HDMI grabs only showed the Q2 console overlay + black lower half (page-flipped 3D doesn't reach HDMI — O2/O3 class). ★ SOURCE-DECODED the mismatch to a CONFIRMED FALSE POSITIVE: Mesa v3dx_tfu.c sets ICFG FORMAT (6-bit field 23:18) = UIF_NO_XOR (14) for these jobs ⇒ UIF-tiled SOURCE ⇒ they are MIPMAP-GENERATION jobs, and the winsys vcheck (raster-src assumption, + it masks only 4 bits at bit18) is INVALID for them. So the diag is NOT the real bug; the corruption is mip-level or base-level → resolved by the gl_texturemode GL_LINEAR fork. RESOLVED the capture plumbing: external/yquake2 gl1_sdl.c streams every-Nth 3D frame over TCP to scripts/quake-capture-sink.py; cvars scr_capture/_max/_host/_port; quake2 wrapper forwards argv. Started host sink (:5599 → /tmp/quake-pi-q2-default) + LAUNCHING the coherent default-capture Pi cycle (mirrors quake2-host-capture cvars for 1:1 pairing). Full recipe + decode: docs/inprogress/2026-08-25-quake2-malformed-textures.md. NEXT (cycle done): inspect default frames (repro? regression vs 08-22?), then GL_LINEAR cycle → mip vs base-level.

2026-08-25 (session ~271/272 — g35b still building (showcase-ports phase); re-armed completion watcher; SD-write-DMA reviewed = HW-gated no-card):
 No owner feedback (git log HEAD..publish/main empty). g35b ALIVE + healthy (disk 25%, 337G free): base image done (loader.disk created @ #10 ~1011s) → now in the --with-showcase ports phase (libffi→python/ffmpeg; Docker buffers RUN output so log has quiet gaps, process confirmed alive). The prior monitor (b0f4nu02y) had expired → re-armed a background completion watcher (bash bw0yh5cpt: fires on `== done:`/build-fail/process-death, 10-min budget). Kept this turn non-competing (g35b is host-CPU-heavy; won't start a 2nd heavy build). Reviewed the standing SD-driver directive (full-speed write path): sdcard.c:1625 gates SDMA to reads-only; write-DMA shows intermittent first-block silent corruption. Code review = ordering is CORRECT (uncached MAP_UNCACHED staging buf + dsb barriers at 622/656 before the CMD kick), so it's a genuine controller-level write-DMA quirk, NOT a software barrier bug → needs HW repro to fix, and **there is no SD card in the Pi** ⇒ HW-gated, do NOT blind-flip the gate. NEXT (watcher fires): verify g35b BUILD_RC=0 + new docker-out/rpi4b-sd-2part.img sha + self-prune; then (fresh binaries carry the O1 glamor vertical-flip fix PHX_READBACK_FLIP_Y=1, wmaker, bash, startx_gpu) a 1-cycle netboot O1 HDMI-grab check is the clean autonomous validation — the Pi is free and g35b doesn't touch it.

2026-08-25 (session ~273 — ✅✅ g35b RELEASE BUILD SUCCEEDED end-to-end + reproducible-export VERIFIED):
 No owner feedback. **g35b DONE (== done: @17:31): the full clean --no-cache --with-showcase Docker release build completed** serving committed state — integrates ALL recent fixes by construction (build log confirms: bash-5.2.21(U), port apps mc+nano, X11 Xphoenix-glamor-daemon; earlier fontconfig/wmaker-data+PNG/startx_gpu). Artifact: docker-out/rpi4b-sd-2part.img (875,560,960 B). ★ **sha256 = 4a0735f5017e21cd0a32e80aa08fd47ab19c648b413a89641361aaed734e481f** and HOST sha == IN-CONTAINER export sha ⇒ reproducible export verified. ★ **self-prune WORKS**: phoenix-rpi image + builder cache reclaimed after export (disk 124G→93G used, 351G free, 21%) — the disk-full root-fix (56541ef) is proven. This is the G5 release prerequisite MET (full reproducible build green + artifact produced); **G5 publish stays HELD for owner sign-off** (org README flip + artifact upload). NEXT: O1 glamor-flip HW-grab (1 Pi cycle, Pi free) IF the fixed Xphoenix-glamor-daemon is deployable to the netboot NFS root — checking deploy path next; else bank O1 as source-fixed high-confidence + move to next queue item.

2026-08-25 (session ~281 — Q2 capture: render PROVEN (hook fired 12x); chased sink-connect blockers to the wire):
 No owner feedback. Big step: `+map demo1` (live in-game) RENDERS 3D + the capture hook FIRED 12x (the +demomap+timedemo path hangs at ca_active — demo-playback-specific, filed as a possible regression). Sink connect had TWO bugs, both fixed: (1) host firewall — NetworkManager's shared chain only whitelists the netboot subnet's UDP; opened TCP 5599 via iptables; (2) the long ~200-char UART command DROPPED a char (`scr_capture_port 559` not 5599) → moved cvars to baseq2/capmap.cfg. A serial glitch ("multiple access on port", 0-byte log) killed one cycle (transient host USB-serial). Relaunched q2cap-cfg2 = `quake2 +exec capmap.cfg +map demo1` (proven render + reliable port). DISCIPLINE: this is the LAST capture attempt — if it streams frames, analyze + GL_LINEAR fork; if not, bank the strong finding (TFU mismatch = confirmed false-positive on mip jobs) + hand the trivial gl_texturemode GL_LINEAR discriminator to the owner + pivot to another Tier-2 item (don't over-invest in a minor cosmetic bug). Recipe/decode: docs/inprogress/2026-08-25-quake2-malformed-textures.md.

2026-08-25 (session ~289 — Q2 texture bug ROOT-CAUSED + FIXED + deployed; verify cycle in flight):
 No owner feedback. ★★★ Owner's #1 Q2 malformed-texture bug: REPRODUCED (12 live frames off Pi via +map demo1 coherent capture — the hanging Strogg banner + gib model skins render scrambled, BSP walls fine), SCOPED (model skins), ROOT-CAUSED (all Q2 model skins are NPOT — banner 260x195, soldier 288x195, gibs...; close blaster=base-level=fine, distant banner=small-mip=scrambled ⇒ NPOT MIP-LEVEL mistiling: the TFU multi-level generator infers levels>=1 tiling, only correct for POT bases). Subagent confirmed Mesa v3d tiling math is pristine upstream ⇒ upstream TFU-NPOT-mip limitation. FIX (external/mesa e4be1163240): v3d_generate_mipmap declines NPOT ⇒ st_generate_mipmap render-fallback (per-level-correct); POT keeps fast TFU. Also fixed a blocking C23 build error (f2fee16: GetGameAPI() empty-paren=void under gcc-16). Rebuilt libGL (RC=0) + relinked yquake2 (140/140) + deployed to NFS root. VERIFY cycle q2fix (+map demo1) running. GOTCHAS this session: background test-cycle launches with inline pkill/setsid compound cmds → exit 144 (harness kills); NFS stale-lease after ~15 cycles → restart nfs-server; long UART cmd lines drop chars (use cfg files); +demomap+timedemo hangs at ca_active (use +map for live render); HDMI can't capture page-flipped GL (use scr_capture TCP sink + host firewall TCP 5599). Recipe/root-cause: docs/inprogress/2026-08-25-quake2-malformed-textures.md. NEXT: verify banner renders clean → if so, owner #1 FIXED; commit integration manifest.

2026-08-25 (session ~297 — ✅✅✅ Q2 NPOT-mip texture bug FIXED + HW-VERIFIED end-to-end):
 No owner feedback. Owner's #1 Quake2 malformed-texture bug is RESOLVED. Re-ran +map demo1 coherent capture after rebuilding the CORRECT lib (libv3d-phoenix.a — v3d_blit.o lives there; my first rebuild wrongly did libGL only, so the fix wasn't in the binary and the banner stayed scrambled). With the real fix deployed: the banner renders CLEAN (green/red speckle + blocky NPOT-mip garbage GONE, coherent skull/emblem texture; BSP walls unchanged, no regression). Before/after: artifacts/q2-texbug/banner-BEFORE-AFTER.png. Fix = external/mesa e4be1163240 (v3d_generate_mipmap declines NPOT → st_generate_mipmap render-fallback, per-level-correct; POT keeps fast TFU) + coord f2fee16 (C23 GetGameAPI build fix). Build chain lesson: v3d gallium driver (v3d_blit.c) compiles into libv3d-phoenix.a via build-v3d-phoenix.py, NOT libGL. FULL arc this session: reproduced→scoped→root-caused (NPOT mip mistiling)→subagent-confirmed→fixed→built→HW-verified, all autonomous. NEXT: push mesa fix to org if publish remote clean; snapshot manifest; then next Tier-2 item (owner #1 done).

2026-08-25 (session ~298 — Q2 NPOT-mip fix now SHIPS IN RELEASE too; owner #1 fully closed):
 No owner feedback. Closed the release-integration gap: the fix was a local external/mesa commit (e4be1163240) + hand-deployed netboot binary, but the reproducible Docker release applies patches/mesa/phoenix-rpi4-v3d.patch onto upstream mesa-26.2.0 — the stale 835-line patch lacked the fix. Regenerated it (`git diff mesa-26.2.0 HEAD` = 924 lines, incl. the NPOT fix + other recent committed v3d fixes the stale patch was missing); VERIFIED `git apply --check` CLEAN onto a fresh mesa-26.2.0 worktree. Committed fdfd11e + pushed to org. ⇒ Owner's #1 Q2 malformed-texture bug is FULLY closed: fixed + HW-verified on netboot AND shipping in a fresh release build. NEXT heartbeat: next Tier-2 item (G-FFMPEG-HW scoping / G-UPSTREAM B-fixes / q3dm7 GPU wedge). Note: the NPOT fix is a general GL improvement — likely helps Q3/quakespasm NPOT skins + possibly the V3DV/Vulkan path has an analogous TFU inferred-mip issue (v3dvx_meta_common.c:954-970) worth checking if vkQuake shows NPOT-tex artifacts.

2026-08-25 (session ~299 — post-Q2-fix: ruled out V3DV analog; scoped G-FFMPEG-HW blocker):
 No owner feedback. Owner #1 (Q2 NPOT-mip) stays FULLY CLOSED. Two scoping results this turn:
 (1) ★ V3DV/Vulkan does NOT share the NPOT-mip bug: v3dvx_meta_common.c meta_emit_tfu_job emits SINGLE-level TFU jobs (no NUMMM multi-level HW mip-gen); Vulkan apps mipmap via explicit per-level vkCmdBlitImage (render-based, per-level-correct). ⇒ vkQuake unaffected; the NPOT fix was gallium-GL-specific. Cheap ruling, no fix needed.
 (2) ★ G-FFMPEG-HW (E6, VideoCore h264 HW decode) BLOCKER identified: Linux reaches the BCM2711 decoder via bcm2835-codec (V4L2 M2M) → MMAL over VCHIQ to the VideoCore firmware. Phoenix has ZERO VCHIQ/MMAL groundwork (only a mention in tools/ffmpeg-port/README.md). ⇒ HW decode = a major greenfield port (VCHIQ transport over mailbox-doorbell+GPU-shared-slots, then MMAL components) — multi-session, not boundedly completable. Phoenix DOES have the mailbox (rpi4-vcmbox) + GPU-reachable alloc (v3d winsys) as partial foundations. Recommend: keep ffmpeg SOFTWARE decode (already plays moving h264 on HDMI) as the shipped path; treat HW-decode as a big-future item pending a VCHIQ port decision.
 Tier-2 reality: G-WIFI blocked (fw-opaque), G-FFMPEG-HW = VCHIQ greenfield, G-UPSTREAM B-bugs = NEEDS-HW (Pi-smoke gated), G-STK/G-XORG = FUTURE. NEXT heartbeat: a G-UPSTREAM B-bug code-fix (accept Pi-smoke needed) or begin VCHIQ groundwork if owner wants ffmpeg-HW.

2026-08-25 (session ~304 — OWNER: gigabit switch/NIC test → found a REAL Phoenix GENET gigabit-RX bug):
 Owner changed to a 2.5Gb switch + (after swapping a defective RTL8153) a new USB-C NIC that negotiates GIGABIT. HOST side fully set up for gigabit: rebound pi-netboot NM conn + .env.local RPI4B_NETBOOT_IFACE to enx00e04c680f3e (1000Mb/s), dnsmasq/TFTP+NFS restarted on it. NIC diagnosis banked in [[project_pi4_netboot_100mbps_cable]] (old RTL8153 unit was stuck advertising 10/100; new unit does gigabit; NIC now on USB3).
 ★ NEW REAL BUG (owner-reported "eth driver doesn't like gigabit"): Pi netboot works at 100Mbps but FAILS at gigabit — link negotiates 1000 full-duplex, VideoCore firmware does gigabit TFTP fine (kernel boots), but the PHOENIX kernel GENET/ephy driver corrupts RX frames at gigabit line rate ⇒ DHCP fails ⇒ Pi falls back to 169.254 link-local ⇒ NFS mount to 10.42.0.1 aborts (rpc socket error connection abort). Low-volume DHCP failing = per-frame RX corruption, not overrun.
 ★ ROOT-CAUSE HYPOTHESIS (high conf): the BCM54213PE PHY's RGMII RX clock-delay is NOT configured. DT phy-mode=rgmii-rxid ⇒ the PHY must add the ~2ns RX delay; ephy.c ephy_restartAN() only advertises 1000M-FD, no shadow-reg RX-delay write. At 100Mbps (25MHz) loose timing survives; at gigabit (125MHz DDR) MAC samples RX on wrong edge → every frame corrupts. Files: sources/phoenix-rtos-lwip/drivers/ephy.c (BCM54213PE init/restartAN), bcm-genet.c genet_configRgmii (~534, clears ID_MODE_DIS). Subagent ade274e2 comparing vs Linux broadcom.c bcm54xx_config_clock_delay for the exact PHY register/bits. NEXT: subagent → implement ephy.c fix → rebuild --scope core → netboot gigabit boot + dd test (host already gigabit-ready). Deploy chain: edit ephy.c → commit lwip → rebuild-rpi4b-fast.sh --scope core (updates TFTP boot image) → test-cycle netboot.

2026-08-25 (session ~307 — gigabit GENET fix: DHCP FIXED, NFS still fails = residual large-frame RX):
 The BCM54213PE RGMII RX-delay fix (lwip commit) WORKS for small packets: Pi DHCP now succeeds at gigabit (gets 10.42.0.12, host DHCPACK; was 169.254 before). But NFS mount still times out (24 attempts) — RULED OUT host firewall (added NFS TCP/UDP 2049+111 ACCEPT for 10.42.0.0/24, still fails) and host NFS health (showmount/rpcinfo/nfsd all OK on 10.42.0.1). ⇒ residual PHOENIX large-frame RX corruption at gigabit: small UDP (DHCP ~300B) passes, larger NFS-TCP frames time out. Failure mode changed connection-abort→command-timeout (progress). NEXT: host-side tcpdump on enx00e04c680f3e (host 10.42.0.12) during a boot to isolate TX-vs-RX + the frame size at which it breaks; then RX-DMA/coherency/skew-value dig (subagent secondary lead). NIC swap done (gigabit-capable), host gigabit-ready.

2026-08-25 (session ~311 — gigabit eth: RGMII fix = DHCP+partial-NFS; residual RX loss found via pcap):
 Owner made gigabit-eth correctness a priority (verify no packet loss + match Linux-Pi4; keep going till correct). Host-side tcpdump of a gigabit boot (host tshark/tcpdump, -Z root for savefile): the NFS4 TCP flow (port 691<->2049) HANDSHAKES + exchanges COMPOUND ops both ways, BUT the Pi missed one host->Pi segment (seq 489:657 retransmitted 203ms later = TCP RTO) and the mount STALLS after seq 657 -> "command timed out". ⇒ RGMII RX-delay fix eliminated most RX corruption (DHCP + TCP handshake + partial NFS now work, vs total failure before) but RESIDUAL RX packet loss remains. The 26983-pkt "storm" was a RED HERRING = normal firmware TFTP boot (4B ACK<->516B DATA = TFTP 512+4; ~8600 blocks = 4.4MB loader.disk). Firmware TFTP at gigabit works (thousands of pkts) so the Pi HW RX is fine; the residual loss is Phoenix-kernel-GENET-RX-specific. Occasional (not systematic) loss smells like RX ring/DMA drops OR marginal RGMII skew. NEXT: (a) enable GENET_RXSTATS_LOG to measure dropped/copyfb directly, (b) subagent deep-dig RX DMA/ring + exact Pi4 rgmii-rxid delay ownership (ID_MODE_DIS SET vs CLEAR) vs Linux bcmgenet, (c) then Linux-Pi4 netboot throughput comparison. RGMII fix committed+shipping (real progress).

2026-08-25 (session ~316 — gigabit RX loss ROOT-CAUSED = EEE/AutogrEEEn LPI; fix committed + building):
 Linux-driver-comparison subagent found the residual gigabit RX loss root cause = EEE (Energy Efficient Ethernet) / AutogrEEEn Low-Power-Idle on the BCM54213PE. Phoenix's RGMII clock-delay config is REGISTER-IDENTICAL to Linux (so delay/timing was NOT the residual cause — refuted), but Phoenix advertises EEE + leaves AutogrEEEn on. The PHY sleeps in idle gaps; the first RGMII RX frame after each wake is occasionally FCS-corrupted → dropped. Fits ALL evidence: occasional/random (first-after-idle), sparse-NFS-loses-1in10 / dense-TFTP-clean (TFTP never idles), host→Pi direction (Pi advertised EEE→host TX-LPI), gigabit-only, size-independent; matches the Pi dtparam=eee=off remedy. FIX (lwip ephy.c): don't advertise EEE 7.60 on BCM54213PE (mirrors the RTL8211 branch line 590). Committed + rebuilding --scope core (bv1w4a1kk). Subagent also noted a minor real defect (missing RX memory barrier in genet_drainRxRing between the MMIO PROD_INDEX/BD read and Normal-NC frame read) — hygiene, unlikely root cause, will add separately if EEE fix insufficient. Confirming measurement available: HW MIB RX-FCS-err counter (UMAC 0x438) + RBUF_OVFL (0x94). Second subagent (FreeBSD+NetBSD cross-analysis, owner-suggested) still running to corroborate. Linux-Pi4 gigabit baseline booting (bju28gzge). NEXT: build → boot + NFS/dd test → if works, gigabit FIXED; then Linux throughput comparison + FCS-counter zero-loss verification.

2026-08-25 (session ~318 — EEE fix insufficient; BSD subagent found the REAL cause = RX lost-wakeup WEDGE):
 EEE-advertise fix (committed) did NOT fix gigabit NFS alone (still 24x mount timeout). FreeBSD+NetBSD cross-analysis subagent (owner-suggested) REFRAMED it: symptom is a WEDGE not a loss rate — one missed 168B segment → whole mount stalls (a healthy peer retransmits past 1 loss). Root cause = RX interrupt LOST-WAKEUP: genet_irqHandler (bcm-genet.c:944) runs in interrupt ctx, sets state->irq_events + signals irq_cond WITHOUT holding irq_lock (can't block); on 4-core SMP the signal is lost if it fires between the irq_events==0 test and condWait(...,0=infinite) parking → RX_DMA_DONE stays masked forever → RX dies → host retransmits unanswered → NFS stalls. SAME class as libphoenix semaphore lost-wakeup e75c4fe. Neither FreeBSD (gen_intr) nor NetBSD (genet_intr) masks RX during service, so they can't wedge — strong multi-ref confirmation. FIX: bound condWait to 100ms (0→100000) so a lost wakeup = <=100ms hiccup + recover (re-read irq_events, drain). Committed + rebuilding (b3p7t3lxd, EEE+wedge). Linux-Pi4 baseline CONFIRMED: Linux 6.18 boots+NFS-roots over gigabit (VFS Mounted root 8.8s) = host perfect, 100% Phoenix driver. Subagent also flagged OOB_DISABLE (3-way BSD/Linux agreement Phoenix differs) but Phoenix set it to fix a prior Tier-3 RDMA-stuck bug → NOT changing (regression risk, low conf). NEXT: build → boot+NFS/dd test. If fixed, gigabit DONE → Linux throughput comparison + FCS-counter loss verification.

2026-08-25 (session ~325 — BREAKTHROUGH reframe: gigabit NFS fail is NOT eth-driver loss, it's a SOCKET/POLL wakeup stall):
 HW counters + tshark are decisive. Phoenix RX HEALTHY (RXSTATS: seen climbs, prod==cidx always=drain lockstep, drop const 4, rbuf_ovfl=0, free=256). Host NIC rx_errors=0 (Pi TX clean, no FCS-bad frames). ⇒ NO packet loss/corruption either direction. tshark of the NFS4 :2049 stream: 16 pkts exchanged in ~14ms all ACKed <1ms, THEN the Pi received host seq 489:657 (168B) but DELAYED THE ACK 203ms until the host RTO-retransmitted, then ACKed. 203ms==host RTO; stall ended only when a NEW packet arrived ⇒ MISSED SOCKET-READINESS WAKEUP: received data doesn't wake the poll()/nfs thread until the next packet nudges it. Connects to known Phoenix poll issues [[project_pi4_poll_readiness]] [[project_nfs_poll_stall_fix]] (posix_poll remote-fd readiness; nfs_set_poll_timeout). Gigabit's tighter packet timing hits the wakeup window; 100Mbps didn't. ⇒ RGMII/EEE/wedge fixes were real driver improvements but NOT the blocker; blocker is lwip-socket/kernel-posix_poll readiness. Owner idea (adopted): build a live in-system network diagnostic in the netboot RAM image (no NFS) — TCP ping-pong RTT probe (shows the ~200ms wakeup spikes in seconds not 120s) + raw send/recv loss counter — to iterate the poll/socket fix fast. Linux-Pi4 baseline works (host perfect). NEXT: build the TCP latency probe program + host echo server → confirm the wakeup stall → fix in lwip socket poll / kernel posix_poll. TEMP diag RXSTATS-every-5s in bcm-genet.c (uncommitted) — revert before finalize.

2026-08-25 (session ~332 — pivot from net-test diag integration to direct socket-poll fix; subagent dispatched):
 net-test TCP ping-pong RTT probe BUILT + committed (lwip 0d86d0b, client mode -C/-P + connect-retry), compiles clean (aarch64). But INTEGRATION into the netboot loader.disk was a rabbit hole: loader.disk is a curated syspage image (rootfs-overlay doesn't reach it), and the syspage-app route (+prog.stripped +user.plo.yaml gated RPI4B_NETDIAG) hit a --scope-project nfs build error (nfs_ops.c:26 fatal error — separate, likely a --scope-project header-staging issue; --scope core/auto build fine). Given STRONG existing evidence (RX healthy via HW counters, TX clean via host rx_errors=0, tshark 203ms delayed-ACK-until-retransmit = socket-readiness lost-wakeup), STOPPED the diag integration and dispatched a subagent (a99a1355) to find+fix the lost-wakeup directly in sources/phoenix-rtos-lwip/port/sockets.c (socket readiness signal to blocked recv/poll) + kernel posix_poll, cf. prior fixes [[project_pi4_poll_readiness]] [[project_nfs_poll_stall_fix]]. Verify via existing NFS test (--scope core, not project). Cleaned tree: RXSTATS diag gated off+committed (5d64804, richer rbuf_ovfl/prod-cidx print for future), net-test yaml reverted. net-test source kept for future (fix the loader.disk integration + --scope-project nfs error separately). NEXT: subagent fix → --scope core rebuild → NFS test → if mounts, gigabit DONE.

2026-08-25 (session ~339 — REAL ROOT CAUSE found: edge-dependent GENET RX drain (not socket layer); fix building+testing):
 Subagent DISPROVED the socket-layer hypothesis (sockets.c re-scans after linking select_cb + waits on a COUNTING sem; mbox.c re-checks predicate under mutex; posix_poll single-inet path blocks <=20ms & self-heals — none can stall 203ms) and RELOCATED the bug precisely to the GENET RX driver. ROOT CAUSE = edge-dependent RX drain: INTRL2_0_RX_DMA_DONE is a SINGLE latched bit for the whole ring; genet_irqHandler CLEARs it on entry (destroys arrival info), genet_drainRxRing snapshots RDMA PROD_INDEX only ONCE, genet_irqThread drains→unmask→park with NO post-unmask ring re-check. A frame that latches the bit but whose producer-index update isn't visible to that single snapshot (sub-us window under 1Gbps micro-burst) is STRANDED in the ring with no pending IRQ → thread parks → drained only when the NEXT frame re-fires RX_DMA_DONE = the peer's ~203ms RTO retransmit → delayed ACK → NFS stalls. RECONCILES EVERYTHING: RX counters clean (nothing dropped, drained late), tcpdump 203ms-delayed-ACK-until-retransmit, gigabit-only (100Mbps inter-arrival >=14us hides the sub-us window). This is the REAL sibling of the earlier wedge fix (which was defensive, couldn't recover a missed HW edge). FIX (bcm-genet.c genet_irqThread): unmask FIRST, then re-read PROD + loop-drain while ring non-empty before parking (level-triggered close) — a frame past the re-check re-fires the live IRQ, before it is caught here. + rx_rearm_stranded counter (proves the mechanism non-zero under load). Building --scope core + NFS/dd test (b5h2on2yn). NEXT: if NFS mounts + dd runs → gigabit DONE; commit + Linux throughput compare + revert RXSTATS-diag gate.

2026-08-26 (session ~340 — libnfs BUILD REGRESSION root-caused + FIXED; restore rebuild in flight):
 Owner flagged: "libnfs build broke this afternoon, built cleanly for months — a regression to roll back, not fix." ROOT CAUSE (log-proven, NOT a source-code commit): the ENTIRE ports staging (libnfs/zlib/curl/lua — all headers+libs) was wiped from .buildroot; libnfs is just the first port a core build trips on (nfs-fs #include <nfsc/libnfs.h>). Chain: (1) today's windowmaker/xorg port work built xorg_server, whose port.def.sh compiles libmd (SHA1) IN-PLACE in the source tree → left sha1.o + libmd.a UNTRACKED in sources/phoenix-rtos-ports; (2) those untracked files made phoenix-rtos-ports permanently "dirty"; (3) rebuild-rpi4b-fast.sh auto-scope saw a dirty build-INFRA repo → force-selected a `clean` rebuild — but that branch ran `clean host core project image`, OMITTING `fs` and `ports`; (4) the clean wiped .buildroot incl. all staged ports, never restaged → nfs-fs fatal "nfsc/libnfs.h: No such file". Smoking gun in rebuild-nettest.log: "Build args: clean host core project image / Reason: build-infra repos dirty: phoenix-rtos-ports". full-clean was ALREADY hardened for this exact error (a79b55d) but the auto/dirty-infra branch was missed. FIXES: (a) phoenix-rtos-ports 4fcd15c — .gitignore the in-tree libmd products (sha1.o/libmd.a) + removed strays → repo clean; (b) coord 2e6b55b — dirty-infra branch now runs `clean host fs core ports project image` (matches full-clean) so a forced clean never strands ports again. Restore rebuild (--scope core --with-ports, bfn4yjj7d) in flight: restores ports staging + compiles in the genet RX re-drain fix (uncommitted lwip) → same cycle resumes gigabit NFS verification. NEXT: when built, one Pi cycle = NFS mount + dd throughput + host tcpdump (zero RTO retransmits) + rx_rearm_stranded counter.

2026-08-26 (session ~340 cont. — re-drain fix VERIFIED-REAL-but-NOT-the-NFS-cause; pivoting to host-pcap localization):
 Build regression fixed + pushed (ports 4fcd15c, coord 7e92336). Restore rebuild GREEN, genet re-drain fix + RXSTATS diag shipped in loader.disk. Gigabit netboot cycle result: NFS takeover STILL times out (13 attempts) BUT lwip fully up (link 1000, DHCP bound, RXSTATS printing). RXSTATS: seen=203 drop=4 copyfb=0 free=256 rbuf_ovfl=0 prod=203 cidx=203 rearm_stranded=5. VERDICT: my level-triggered re-drain fix IS real (rearm_stranded=5 → it caught 5 genuinely-stranded frames) but is NOT the NFS root cause — RX ring is HEALTHY (prod==cidx fully drained, zero rbuf overflow, drop constant at 4). So the failure is NOT RX-drain/overflow. U-Boot research agent CONFIRMED Phoenix RX init is a strict SUPERSET of U-Boot (no missing register/threshold); pushes diagnosis off the RX ring. NEW LEAD: only 203 RX packets over 13 mount attempts+DHCP is suspiciously LOW → either TX (Pi's NFS SYN not reaching host) or NFS-TCP round-trip failing at a higher layer. DECISIVE NEXT EXPERIMENT (running, bykfr528f): full-traffic host tcpdump (/tmp, -Z root, no filter) during netboot to see ARP→SYN→data — localizes TX-fail vs TCP-handshake-fail vs RPC-hang. Fixed capture gotchas: AppArmor blocks tcpdump -w to home dirs (use /tmp); need -Z root. Linux + BSD research agents still running. Keep re-drain fix (defensible hardening) but it's not the fix.

2026-08-26 (session ~340 cont. — PCAP REFRAME + convergent fix: missing GENET RX HW-timeout register):
 Host pcap (working this time, /tmp -Z root) of a netboot cycle DECISIVELY reframed the bug. Findings: (1) TFTP netboot = UDP 516/4 ping-pong works (plo stack). (2) Phoenix lwip DHCP works — Pi=10.42.0.12 gets IP, re-DHCPs every ~38s. (3) lwip ARP works BOTH ways post-boot: at t=30:16 "who-has 10.42.0.1 tell 10.42.0.12" → reply 24us later. (4) ★ ZERO TCP packets in the ENTIRE capture — no SYN, no port-2049, nothing. So NFS mount NEVER emits a TCP SYN despite lwip resolving the server's MAC. lwip re-ARPs the server every ~38s (each failed mount cycle). MECHANISM (reconciles all): the host's tiny UNICAST ARP reply (arrives 24us after request AT GIGABIT) is STRANDED on RX → lwip never processes it → the SYN queued behind ARP is never flushed → NFS never connects. Same RX-strand class, but it bites the ARP-reply/SYN-ACK, not an established flow — which is why "no SYN on wire" not "slow NFS". Gigabit-only (24us reply hits the sub-us snapshot window; 100Mbps repro'd working before). TWO independent research agents (Linux + NetBSD/BSD) INDEPENDENTLY ranked #1: Phoenix omits the RX DMA ring interrupt-coalescing TIMEOUT register (DMA_RING16_TIMEOUT, global RDMA block +0x6C = 0x30AC), leaving it at reset 0 = disabled. Linux writes DIV_ROUND_UP(50us*1000,8192)=7 ticks; NetBSD ~57us. It's a HW liveness backstop: re-raises the ring RX IRQ within ~57us whenever >=1 buffer undelivered — rescues the stranded ARP reply. FIX IMPLEMENTED (lwip bcm-genet.c + regs.h): genet_write(GENET_RDMA_REGS_OFF + 0x6C, 7) in genet_initRxRing. Minimal single-register change (kept SW re-drain as backup) so a pass unambiguously credits the HW timer. Building --scope core (bqiqpeeq8) → netboot+pcap verify: expect SYN emitted + NFS mounts. Also on tap if needed: #2 pause-frame flow control (only if rbuf_ovfl>0; currently 0), + RAM-disk net-test suite for direct lwip-TCP isolation.

2026-08-26 (session ~340 cont. — RX-timeout fix HELPED (SYN now emitted) but revealed DEEPER bug: unicast RX dropped BEFORE the ring):
 RX HW-timeout register fix (0x30AC=7) built+tested. PROGRESS: TCP SYN is NOW emitted (was zero before) → the fix rescued the stranded ARP reply, unblocking connect. But NFS STILL times out. pcap port-709 full trace: Pi SYN → host SYN-ACK (111us later) DROPPED (Pi never sees it) → host RETRANSMITS SYN-ACK 1s later (RTO) → Pi finally ACKs → Pi sends RPC call → host sends 28B RPC reply (seq542:570) DROPPED → Pi keeps ACKing 542, never 570 → Pi FIN→RST (libnfs mount timeout). ROOT: unicast RX frames arriving ~111us AFTER a Pi TX are dropped; only the peer's ~1s RTO retransmit gets through; libnfs times out first. KEY: RXSTATS prod==cidx THROUGHOUT the stall → the dropped frames NEVER REACH THE DESCRIPTOR RING (lost before it), so neither the HW ring-timeout NOR the SW re-drain can catch them (rearm_stranded=12 = a DIFFERENT, genuinely ring-stranded set that IS rescued). rbuf_ovfl=0. So the loss is at MAC-RX / RBUF→RDMA, TX-timing-correlated, gigabit-only. Suspects: (a) RX checksum checker — Phoenix UNIQUELY sets RBUF_64B_EN+RBUF_RXCHK_EN (NetBSD/Barebox/FreeBSD-default don't); (b) RX buffer-aliasing (known, project_pi4_genet_rx_perf); (c) RDMA not moving FIFO→ring for a frame arriving during/after TX. KEEP the RX-timeout fix (real, correct — SYN now emitted). NEXT experiments: disable RBUF_RXCHK_EN+64B_EN (match BSD); and/or build RAM-disk net-test (run_pingpong) for controlled lwip-TCP RX probing without libnfs timeout confound.

2026-08-26 (session ~340 cont. — advisor caught flaw: it's DELIVERED-BUT-CORRUPT, not before-ring; fix = missing RX DMA read barrier):
 Advisor refuted my "lost before ring" read: (1) prod==cidx on 5-10s snapshots can't distinguish "never DMA'd" from "delivered+rejected" (strand is ~1s). (2) COUNTER-EVIDENCE in my own pcap: Pi emits a dup-ACK (ack 542) 586us AFTER the host's 28B reply — a frame that never arrived elicits NOTHING, so something DID reach lwip TCP and failed to advance rcv_nxt = delivered-but-corrupt. ROOT: genet_drainRxRing reads PROD_INDEX then reads the (uncached Normal-NC) payload with NO load barrier → CPU speculatively reads payload ahead of the PROD observation → sees pre-DMA stale/partial bytes → frame delivered CORRUPT → lwip checksum rejects → peer only progresses on ~200ms RTO. Explains the TX-timing correlation EXACTLY: right after a TX the irq thread is already awake → drain runs at MIN latency → tightest race; the 1s retransmit lands on an idle ring and wins. BSD agent's #4 (both BSDs issue BUS_DMASYNC_POSTREAD here); = Linux dma_rmb() in its RX desc loop. FIX: added `dmb ld` after the PROD read in genet_drainRxRing (orders all payload reads after DMA-completion observation). Also added rxtmo readback to RXSTATS (confirm 0x30AC timer write took effect). Kept the RX-timeout register (both ref drivers ship it; it's what got the SYN emitted). Building --scope core (bmucsr8j5). Verify: SYN-ACK ACK'd promptly + NFS mounts; then MULTI-TRIAL (test-cycle-bench.sh) not n=1 (advisor guard: 2 premature victory claims already tonight).

2026-08-26 (session ~340 cont. — barrier DID NOT fix; failure is DETERMINISTIC delivered-but-corrupt; adding discriminating IP-hdr-checksum log):
 DMA read-barrier (dmb ld) build+tested: NFS STILL times out (24 attempts → takeover aborted 120s). rxtmo=7 (timer write confirmed effective), rearm_stranded=0 this run. Same exact pcap pattern: 1st SYN-ACK dropped→1s RTO retransmit→Pi ACKs→RPC call→host 28B RPC reply→Pi re-ACKs old seq (803 not 831)→FIN→RST. KEY NEW INSIGHT: ALL 24 mount attempts fail IDENTICALLY → this is DETERMINISTIC, not a sub-us timing race (a race would let some attempts succeed). So the barrier theory (timing race) is likely WRONG. But the RPC-reply dup-ACK still proves the frame IS delivered to lwip and rejected = delivered-but-corrupt, deterministically, for frames arriving shortly after a Pi TX. Candidates now: buffer aliasing (stale/wrong buffer content, known project_pi4_genet_rx_perf), 66-byte-prefix misparse, or the RX checksum checker (Phoenix uniquely sets RBUF_RXCHK_EN+64B_EN). EXPERIMENT (building bmbn7q9sq): gated per-frame log GENET_RXFRAME_LOG in genet_drainRxRing — for TCP:2049 frames, logs ports/flags/len + the IP-header checksum computed over the bytes AS READ FROM THE DMA BUFFER. iphdrck==0xFFFF ⇒ bytes intact (lwip rejects for another reason); !=0xFFFF ⇒ frame CORRUPT in memory (aliasing/partial-DMA). Deterministic failure ⇒ log won't perturb it away. Correlate RXF log vs host pcap to kill the ambiguity in one cycle. Kept barrier + timeout register (both correct/upstream-aligned regardless).

2026-08-26 (session ~340 cont. — RXF DIAG: GENET RX delivery EXONERATED (IP hdrs intact); narrowing to TCP-tail vs post-handoff/lwip):
 GENET_RXFRAME_LOG cycle: ALL 109 delivered NFS/TCP:2049 frames have iphdrck=0xffff (IP headers INTACT). Driver delivers EVERY frame to lwip incl. both SYN-ACKs (flags 0x12), the RPC reply (flags 0x18 len=82), FIN (0x11). So GENET RX path is EXONERATED: no before-ring loss, IP headers clean, frames reach lwip. Yet lwip rejects (dup-ACK). Problem is downstream of RX-delivery: either the TCP TAIL (bytes after the IP hdr) is corrupt while the IP hdr survives, OR post-handoff buffer overwrite, OR lwip mis-rejects. Pool is 512 buf (256 ring + 256 slack) — ample for 1 mount conn, so simple exhaustion-aliasing unlikely. NEXT (building bywykghk2): extended RXF to compute the FULL TCP checksum over the delivered bytes in-driver. tcpck==0xffff ⇒ frame FULLY intact in driver's hands ⇒ corruption is post-handoff (buffer reused/aliased after pbuf handoff, gigabit-fast churn) or lwip mis-rejects → investigate custom-pbuf/free-list re-arm + lwip checksum config. tcpck!=0xffff ⇒ TCP tail corrupt in memory (DMA partial-write past the IP hdr) → DMA/write-posting, barrier insufficient. This splits the last two hypotheses. NOTE: driver RX correctness now strongly supported — reframe from "eth driver RX loss" to "TCP-payload integrity/lwip". Owner: NFS worked at 100Mbps (same lwip) ⇒ gigabit-fast-arrival trigger.

2026-08-26 (session ~340 cont. — GENET DRIVER FULLY EXONERATED (frames 100% intact); bug is lwip/socket gigabit-timing race):
 Extended RXF: ALL 125 NFS frames have iphdrck=0xffff AND tcpck=0xffff → EVERY delivered frame is byte-perfect (IP+TCP) in the driver's hands. netif->input returns OK (drop=4 constant). No "TX timeout" ever (TX poll-to-completion succeeds). pcap: Pi sends SYN, then NOTHING for 1.06s until right after the host's SYN-ACK RETRANSMIT, then ACKs. So lwip receives a VALID SYN-ACK #1 (~111us after SYN) but does NOT emit the completing ACK until the retransmit (SYN-ACK #2). Both SYN-ACKs delivered valid; lwip processes #2 not #1. CONCLUSION: GENET RX+TX are correct; the bug is in lwip TCP/socket processing — a ~100us race where SYN-ACK #1 arrives too fast after the SYN (gigabit) and is dropped/unmatched before the PCB/receive path is armed; the 1s retransmit finds it ready. 100Mbps's slower serialization hides the window (owner: worked at 100M). netif=threaded tcpip_input. NEXT (building bdrs26ltf): TXF log in genet_linkOutput — confirms whether lwip GENERATES the ACK after SYN-ACK#1 (→ TX-drop, driver) or NOT (→ lwip TCP/connect race). Then investigate lwip connect/SYN_SENT vs incoming-seg matching + Phoenix socket→tcpip threading. Diag builds keep RX-timeout register + dma_rmb barrier (both upstream-correct). Owner reframe: this is NOT an eth-driver RX bug — driver proven correct by on-Pi checksum.

2026-08-26 (session ~340 cont. — CORRECTION + timestamped RXF/TXF to split strand vs lwip-wakeup):
 Advisor caught an overclaim: prior "GENET FULLY EXONERATED" is too strong. Proven: RX DELIVERY is byte-perfect (iphdrck+tcpck both 0xffff on all 125 frames). NOT yet proven: RX delivery LATENCY (my RXF grep stripped timestamps — frames arrive valid but maybe NOT on time) and TX wire EGRESS (no-TX-timeout != actually transmitted; there's a code-comment precedent "descriptor consumed via cons_index but MAC never transmitted"). Within one connection all 5 RXF frames log CONTIGUOUSLY → hints frames were drained in a BURST (strand delivered late together with the retransmit's IRQ) rather than as they arrived → would reopen the RX-timeliness/ring-strand theory (rxtmo=7's effect on ring-16 MBDONE was flagged UNVERIFIED by the Linux agent). DECISIVE cycle building (b406scoe3): RXF+TXF now carry gettime() us timestamps. Branches: (1) two SYN-ACK RXF ~1s apart ⇒ timely delivery ⇒ lwip sat on it ⇒ lwip side; back-to-back ⇒ ring strand persists ⇒ driver, mitigate via drain-on-condWait-timeout (U-Boot #1). (2) TXF ACK right after RXF#1 but absent from pcap ⇒ TX silent-drop; TXF ACK only after RXF#2 ⇒ lwip late; no TXF ACK ⇒ lwip never responded. If lwip: prime suspect = lost wakeup in lwip-port sys_mbox_post/sys_arch_mbox_fetch post-while-blocking window (matches gigabit-only/deterministic/#2-not-#1; project hit this twice: semaphoreUp wasZero e75c4fe, posix_poll). Verify deployed lwip links FIXED libphoenix (stale .toolchain trap). Dropped "PCB-not-armed" (tcp_connect registers PCB before tcp_output, same thread). CAUTION: TXF printf ~8ms/line inside tx_lock — a surprise mount = perturbation, not fix; confirm by removing log + multi-trial bench.

2026-08-26 (session ~340 cont. — ★ ROOT CAUSE CONFIRMED via timestamps: RX interrupt LOST for frame arriving ~100us after a TX → 1s strand; FIX = poll-drain backstop):
 Timestamped RXF/TXF DECISIVE. Connection dp=600: TXF SYN t=12909925 → RXF SYN-ACK delivered t=13918764 = gap 1,008,839us ≈ 1.009 SECONDS. Host replies to SYN in ~111us (every pcap), but the DRIVER doesn't DELIVER the SYN-ACK to lwip until ~1s later. lwip is fine — it ACKs 7.6ms after DELIVERY (TXF t=13926409). So: the SYN-ACK arriving ~111us after the Pi's SYN (a TX) has its RX_DMA_DONE interrupt LOST; the frame is DMA'd into the ring but the irq thread is never woken → strands until the next inbound frame (peer's ~1s RTO retransmit) re-fires the IRQ → mount times out. Confirmed RING-STRAND / lost-IRQ, NOT lwip, NOT delivery-corruption (frames byte-perfect), NOT before-ring loss. rxtmo=7 (HW DMA_RING16_TIMEOUT) does NOT rescue ring-16 (Linux agent flagged unverified) nor does the post-unmask re-drain. TX-after-RX interrupt-loss = gigabit-only (at 100Mbps inter-arrival hides it; owner: worked at 100M). FIX (building bwudqmyo1): irq-thread condWait was 100ms with NO drain on timeout; changed to a 2ms poll-drain backstop — on every wait return, unconditionally re-read PROD_INDEX and drain any stranded frame (U-Boot's poll-robustness by construction). Bounds RX latency to 2ms regardless of the lost-IRQ root cause → mount survives (TCP RTO 200ms+). Added rx_pollrescue counter. This is a SYMPTOM-bound fix that gets NFS FUNCTIONAL; the lost-IRQ root cause (TX/RX INTRL2 interaction) still to be found for full PERF. Verify: NFS mounts + pollrescue>0 + dd throughput; then multi-trial bench; then root-cause the lost IRQ; then gate off all diag + push.

2026-08-26 (session ~340 cont. — poll-drain fix FAILED w/ pollrescue=0 → frame is stuck BEFORE the ring (PROD frozen), not a ring strand):
 Poll-drain backstop built (loader.disk has 'pollrescue' string) + tested: NFS STILL fails, pollrescue=0 — the 2ms poll NEVER found PROD != cidx. So during the 1s strand the stranded frame is NOT in the descriptor ring; PROD is frozen. Pcap confirms the SYN-ACK is on the wire ~111us after the SYN (early) but the driver delivers it to lwip ~1s late (RXF timestamp) — so the frame is received by the MAC but NOT moved from RBUF/RX-FIFO into the descriptor ring for ~1s. rbuf_ovfl=0. My ring-based mechanisms (poll-drain, post-unmask re-drain, HW DMA_RING16_TIMEOUT) CANNOT help — they all operate on the ring, and the frame never reaches it. This is an RX DMA STALL: a frame arriving ~100us after a Pi TX gets stuck before the ring; PROD un-freezes ~1s later (coincident with the host RTO retransmit's arrival) and both drain together. CAVEAT to rule out: confirm the poll thread actually reaches its 2ms condWait timeout (add an invocation counter) — if condWait small-timeout doesn't fire, pollrescue=0 is a false negative. NEXT: either (a) prove the poll runs, or (b) investigate why RX DMA PROD freezes after a TX (RDMA enable/CONS-visibility/BD re-arm interaction; TX path disturbing RDMA). Keep RX-timeout reg + dma_rmb barrier. Consulting advisor on direction.

2026-08-26 (session ~340 cont. — plo/RBUF dead-ends ruled out; fork-diagnostic targets INTRL2 latch):
 Subagent (plo diff) DEBUNKED two premises: (1) plo has NO GENET driver — netboot TFTP is done by the CLOSED VideoCore firmware (EEPROM+start4.elf, polled); plo is just a TFTP payload. No in-repo plo NIC to diff. (2) RBUF_64B_EN+RXCHK_EN is BYTE-IDENTICAL to Linux (bcmgenet.c:2624/2629) — Phoenix is NOT unique; the regs.h:114-117 "only driver in existence" comment is WRONG (fix later). RBUF ruled out. Subagent confirms current HEAD already carries mitigations for all 3 historical modes (wrong-offset FIXED, lost-IRQ MITIGATED, barrier FIXED) and redirects to the UNCONFIRMED root cause = INTRL2 RX_DMA_DONE latch / IRQ-delivery (mode b). TENSION to resolve: mode-b assumes PROD advanced but IRQ lost — but my pollrescue=0 says PROD FROZEN (frame not in ring). Fork-diagnostic (b1taui4my, running) resolves it: polls counter (does 2ms poll run?) + RXPOLL dump during strand (prodfull incl. top16=HW discard counter, istat/imask = is RX_DMA_DONE latched while ring empty?, rdma_ctrl = RX DMA_EN dropped?). That one register dump distinguishes discard vs latched-not-woken vs DMA-disabled → points at the actual fix.

2026-08-26 (session ~340 cont. — ★ FORK CLOSED: RX DMA engine STOPS PRODUCING after a TX (not ring-strand, not lwip, not discard)):
 Fork-diagnostic decisive. polls=63757 (2ms poll RUNS constantly), pollrescue=1 (frame NOT in ring — confirmed, not a false negative). RXPOLL register dump DURING the strand: prodfull=0x00000002 cidx=2 (ring EMPTY, PROD==CONS), top-16-of-PROD=0 (ZERO HW discards), rdma_ctrl=0x00020001 (RX DMA_EN bit0 SET + ring16 enable bit17), imask=0x07ffdfff (ONLY bit13 RX_DMA_DONE unmasked), istat bit13 (RX_DMA_DONE) NEVER set during strand (set bits 16=TX_DMA_DONE/17/18/23=MDIO are all MASKED+irrelevant). CONCLUSION: RX DMA is enabled, ring empty (free BDs), no discards — yet a frame arriving ~100us AFTER a Pi TX is NEVER produced into the descriptor ring and NEVER raises RX_DMA_DONE, for ~1s until the NEXT frame (peer RTO retransmit) arrives → then both produce+drain together. So the GENET RX DMA ENGINE STALLS/STOPS PRODUCING after a TX; a subsequent RBUF arrival re-kicks it. NOT ring-strand (frame never reaches ring), NOT lwip (proven byte-perfect + ACKs 7ms post-delivery), NOT discard (counter=0), NOT DMA-disabled (EN set), NOT masked-IRQ (bit13 unmasked but never asserts because HW never produces). This is a TX→RX DMA-engine interaction in GENET. Ring-level mitigations (poll/re-drain/HW-timeout) CANNOT help — frame never reaches ring. NEXT: need an RX-DMA re-kick (re-publish CONS_INDEX / toggle ring-enable / RBUF nudge) in the 2ms poll to force RBUF→ring flush; OR root-cause why TX stalls the RX DMA producer. Consulting advisor on kick strategy.

2026-08-26 (session ~340 cont. — strand-correlation CONFIRMED (PROD frozen in-window); MIB counters to prove frame reaches MAC):
 Advisor: my earlier RXPOLL dumps were head -6 = idle pre-mount (not strand-correlated) → "RX DMA stops producing" was unconfirmed. Line-ORDER re-correlation on the same log FIXES this: conn dp=520, TXF SYN (line 206) → RXPOLL polls=5000 IN-WINDOW (line 208): prodfull=0x0000000d cidx=13 (PROD==cidx, ring EMPTY, top16=0 no discards) → RXF SYN-ACK delivered (line 209) 1.04s after SYN. So DURING the strand PROD is genuinely frozen + ring empty — SUPPORTS "frame not produced into ring" (n=1 in-window dump; dumps every ~1.4s vs ~1s strand so few land in-window). DECISIVE next (build bhazi8ao7): MIB HW counters in RXPOLL dump — rx_pkt (UMAC+0x428 total received) + rx_fcs (UMAC+0x438 CRC err), + t= timestamp, + read-and-clear sticky RXDMA_PDONE/BDONE bits14/15. Branch: rx_pkt climbs while PROD frozen ⇒ frame in MAC/RBUF, DMA stall ⇒ kick justified; rx_fcs climbs ⇒ gigabit corruption ⇒ RGMII (U-Boot#4 OOB_DISABLE/ID_MODE_DIS polarity, fix in ephy.c); neither climbs ⇒ frame never reached MAC ⇒ PHY-RX dead-window / dump firmware PHY regs at handoff. Refinement: DHCP OFFER (after DISCOVER TX) works — dnsmasq replies ~1-2ms vs SYN-ACK 111us/ARP 24us → vulnerable window is SUB-MS after TX. VideoCore firmware proves board/PHY/link do sustained gigabit TX↔RX (HW not broken). No kicks/RGMII/EEE experiments until the MIB branch decides.

2026-08-26 (session ~340 cont. — process-contention cleanup; last MIB cycle was CORRUPTED, re-running clean):
 PROCESS HYGIENE BUG: my first gigabit-cap.sh wrapper (pid 3112112) hung ~73min and never exited; every subsequent turn's `pgrep -f gigabit-cap.sh | head -1` matched that STALE pid, so all `tail --pid` waiters stuck on a dead-end AND cycles overlapped → the MIB cycle's UART capture (log 000620) came out 8MB of garbage w/ ZERO lwip content (two picocoms contending for the single /dev/ttyUSB0). So the MIB result is INVALID — no data yet. FIX: killed pid 3112112 + its group + 4 stale tail waiters + any picocom/capture/test-cycle by explicit PID; UART now free (fuser empty). Re-running the MIB cycle CLEAN (bb18g1fn6) with a ROBUST waiter — capture the launched PID via `$!` in the SAME shell (not pgrep, which self-matches stale procs). LESSON: never `pgrep -f <script> | head` to find "the current cycle" across turns — accumulates + matches stale; use `$!` in-shell. MIB build confirmed in loader.disk (mib_rxpkt string present). Awaiting clean rx_pkt/rx_fcs during a strand window to split DMA-stall vs RGMII vs frame-never-reached-MAC.

2026-08-26 (session ~340 cont. — ★★ ROOT CAUSE = EEE/LPI: frame dropped BEFORE the MAC after a TX, no CRC; NO DMA stall):
 CLEAN MIB cycle DECISIVE (conn dp=757): PROD and mib_rxpkt advance in EXACT LOCKSTEP across the strand — pre-SYN PROD=11/mib=8970, mid-strand PROD=15/mib=8974 (+4/+4), post-delivery PROD=19/mib=8978 (+4/+4). So every frame the MAC receives IS immediately produced into the ring → NO DMA stall, NO RBUF hold. mib_rxfcs=0 always → NO CRC/RGMII corruption. Yet the SYN-ACK is delivered 1.04s late → its ORIGINAL copy (arriving ~111us after the Pi's SYN) NEVER REACHES THE MAC (not counted in mib_rxpkt, not in PROD); only the ~1s RTO retransmit reaches the MAC → produced → drained → delivered. So a frame arriving SUB-MS after a Pi TX is DROPPED BEFORE THE MAC, with NO CRC error. + host ethtool: the Pi ADVERTISES EEE (100/1000baseT) while ephy.c only disables EEE ADVERTISEMENT (7.60), NOT the LPI function → classic EEE/AutogrEEEn LPI: PHY enters low-power-idle after TX, a frame during the sub-ms LPI→active wake is dropped at the PHY. Fits ALL evidence: gigabit-only (LPI wake ~16us), sub-ms-after-TX (DHCP@1-2ms works, ARP@24us/SYN-ACK@111us fail), no CRC, no DMA stall, HW-capable (firmware TFTP works — firmware disables EEE). FIX: FULLY disable EEE/AutogrEEEn on the BCM54213PE PHY (not just advertisement) in ephy.c. Ruled out: DMA stall (lockstep), RGMII corruption (fcs=0), ring strand, lwip, delivery. This is THE root cause. Building the EEE-disable now.

2026-08-26 (session ~340 cont. — netem 2ms did NOT fix (window>2ms); implementing FULL EEE/AutogrEEEn/APD disable, direct test):
 netem 2ms host-egress-delay test: NFS STILL fails, SYN→SYN-ACK gap still ~1.05s (=TCP RTO). So IF a post-TX blackout, it's >2ms (unusual for EEE LPI-wake ~16us, but AutogrEEEn/APD power-cycles can be longer). Rather than binary-search netem, DIRECTLY test the hypothesis with the real fix (upstream-correct regardless). Subagent delivered the exact recipe. IMPLEMENTED: PHY-side (ephy.c bcm54213pe block, after reset/before BMCR-autoneg which is NOT a soft-reset so writes survive): (1) AutogrEEEn OFF = TOP_MISC expansion 0x0D00 bit0 clear [the autonomous-LPI source, currently MISSING]; (2) EEE LPI FUNCTION OFF = MMD7 0x803D bits15,14 clear (LPI_FEATURE_EN[_DIG1000X]) [gates LPI gen, distinct from adv, MISSING]; (3) EEE adv OFF 7.60 (already had); (4) APD OFF = shadow 0x0A bit5 clear. MAC-side (bcm-genet.c after RBUF cfg): RBUF_ENERGY_CTRL (RBUF+0x9C) clear EEE_EN|PM_EN [Linux: "RBUF EEE/PM can break the RX path on GENET. Keep it disabled"] + UMAC_EEE_CTRL (UMAC+0x064) clear EEE_EN bit3. Building boe2f4e6y → direct test. If it mounts → EEE/LPI CONFIRMED = root cause (netem-2ms just too short); multi-trial bench + strip diag + push. If it fails too → EEE refuted, reconsider (netem-neg + fix-neg would be strong refutation) → consult advisor. Fix is upstream-aligned (Linux bcm54xx_config_init) so keep regardless.

2026-08-26 (session ~340 cont. — EEE FIX FAILED too → EEE likely REFUTED (netem-neg + fix-neg); reconsidering):
 Full EEE/AutogrEEEn/APD/MAC-EEE disable built+tested: NFS STILL fails, SYN→SYN-ACK gap STILL ~1.04s (=TCP RTO). So BOTH the netem-2ms test AND the direct EEE fix are negative → EEE/LPI is likely NOT the cause. CAVEAT: have NOT verified my PHY register writes (exp 0x0D00, MMD7 0x803D, shadow 0x0A) actually took effect — need host `ethtool --show-eee` w/ Pi UP (does it still advertise EEE?) before fully refuting EEE. CONFIRMED-STILL-TRUE across all clean runs: original reply (~100us after Pi TX) delivered ~1s late (at peer RTO retransmit); PROD/mib_rxpkt lockstep (no DMA stall); fcs=0 (no corruption); frame not in ring during strand (poll pollrescue~1). Hypotheses now FALSIFIED: ring-strand, delivered-corrupt, lwip, DMA-stall, RGMII, EEE/LPI. REMAINING attribution gap: does the ORIGINAL SYN-ACK reach the MAC (counted in mib_rxpkt) or not? 1s-granularity polls can't resolve. NEXT: (a) verify EEE writes took effect (ethtool, Pi up); (b) add mib_rxpkt to the RXF line to definitively attribute the SYN-ACK's MAC-receipt timing; (c) reconsider host/switch (2.5G switch added) — though owner says assume Phoenix SW. Kept EEE disable (upstream-correct regardless). Consulting advisor — 2 strong negatives need reconciliation.

2026-08-26 (session ~340 cont. — ★★ STRONG LEAD: macSetSpeed runs EVERY 1s link-poll tick (unconditional) → periodic MAC RX disturbance):
 Advisor corrected my reasoning: (1) EEE writes likely NEVER took effect — ephy_mmdRead was __attribute__((unused)), never exercised on HW; RMW with a bad read wrote garbage → EEE NOT actually refuted. (2) netem test INVALID — same 1.04s gap means the 2ms delay never applied (need ping proof-of-delay); so I had ZERO valid negatives, not two. (3) DMA-stall NOT truly falsified (lockstep was ±4 on 1s samples — can't see one frame held 1s). ★ #5 SOURCE READ HIT: genet_setLinkState (called from the 1Hz link poll) calls genet_macSetSpeed UNCONDITIONALLY (bcm-genet.c:1248) — the change-check `if` above only guards the printf, NOT the macSetSpeed call. So every 1s it does an RMW write to UMAC_CMD, which momentarily re-syncs the MAC and drops a frame in-flight across the write (no FCS, no discard, not produced into the ring) — EXACT signature: deterministic loss of the first reply after a request, recovered on peer ~1s RTO; strand ~1s ≈ link-poll period. FIX: gate macSetSpeed on an actual speed/duplex CHANGE (still runs on first link-up + real renegotiation). Building bvzdsvvd0 → direct test (existing SYN-ACK-gap logging shows fix). If gap collapses ~1s→~ms + NFS mounts → THIS was the root cause. If not → add ephy readback (verify EEE) + mib_rxpkt-in-RXF (attribution) + re-do netem with ping proof. Moved to unresolved: DMA-stall, EEE (both untested-not-refuted).

2026-08-26 (session ~340 cont. — macSetSpeed fix NEGATIVE; adding mib-attribution to RXF/TXF to answer "does original reply reach the MAC"):
 macSetSpeed-gate-on-change fix built+tested: NFS STILL fails, gap STILL ~1.0s. Real bug, but NOT the cause (kept the fix — correct regardless). So the tested-and-negative list is still just this + (untested) EEE. The ONE unresolved decisive question: does the ORIGINAL reply (~100us after Pi TX) REACH THE MAC, or only the RTO retransmit? Added mib=%u (UMAC+0x428 MAC total-received) to BOTH the TXF and RXF log lines (build b6a5x9jhc). Read: for a stranded SYN-ACK, compare mib at its RXF delivery vs mib at the preceding SYN TXF — if delivery-mib ≈ TXF-mib+small (fresh) → original NEVER hit the MAC → PHY/wire-side loss (post-TX window; re-do netem WITH ping proof, or PHY RX); if delivery-mib jumped long before delivery → original received but HELD → MAC/RBUF/DMA side. This splits PHY/wire vs MAC/DMA definitively in one cycle. Also should verify EEE writes took effect (ephy_mmdRead looks correct — standard MMD indirect — but was never HW-exercised; add readback if EEE resurfaces). Falsified-tested: ring-strand, delivered-corrupt, lwip, RGMII, macSetSpeed-per-tick. Untested/unresolved: EEE, DMA-hold, PHY-post-TX-window.

2026-08-26 (session ~340 cont. — ★★ ATTRIBUTION: loss is PHY/WIRE-side (original reply NEVER reaches the MAC); only RTO retransmit does):
 mib-attribution DECISIVE (sp=688): SYN TXF mib=8971; SYN-ACK RXF delivered 1.05s later mib=8974. With PROD/mib lockstep (received=produced=delivered immediately, no hold), an original SYN-ACK reaching the MAC ~100us after the SYN would deliver at ~mib 8972 immediately — but it delivers at 8974 after 1.05s. So the ORIGINAL reply (~100us after the Pi's TX) is NOT received by the MAC; only the ~1s RTO retransmit is. LOSS IS PHY/WIRE-SIDE, BEFORE THE MAC. Rules IN: PHY-config post-TX RX loss (Phoenix ephy.c). Rules OUT (all downstream of MAC): ring/DMA/RBUF hold, lwip, drain. This resurrects EEE/LPI (PHY post-TX LPI blackout) BUT my EEE fix didn't help — CAVEAT the writes may not have applied (untested ephy_mmdRead). Candidates: EEE/AutogrEEEn LPI, Energy-Detect-Power-Down (EDPD, separate from APD), auto-MDIX/downshift re-detect after TX. NEXT: (1) add ephy register READBACKS (exp 0x0D00, MMD7 0x803D, shadow 0x0A) to prove whether my EEE/AutogrEEEn/APD writes took effect — validates whether EEE is truly refuted; (2) re-do netem WITH ping-proof + larger delay (retransmit@1s gets through so blackout<1s; a proper host delay past the blackout should fix it → confirms post-TX window). Owner "assume Phoenix SW" holds — PHY init is ephy.c.

2026-08-26 (session ~340 cont. — EEE/AutogrEEEn/APD VERIFIED-OFF (readback) + still fails → genuinely REFUTED; mechanism = post-TX deafness (NOT idle-based)):
 ephy readback CONFIRMS my writes applied: autogreeen(exp0x0D00)=0x8002 (bit0 cleared), eee(7.803D)=0x0000 (bits15,14 cleared), apd(shd0x0A)=0x0001 (bit5 cleared). NFS STILL fails. So EEE/AutogrEEEn/APD are now TRULY refuted (verified-disabled). KEY LOGIC: reply after SHORT post-TX idle (~100us) DROPPED, reply after LONG idle (~1s RTO retransmit) RECEIVED — OPPOSITE of Energy-Detect-Power-Down (which drops after LONG idle). So it's a POST-TX deafness (the Pi's TX causes RX-deaf for 100us<D<1s, >2ms per netem), NOT idle/energy-based. Frame never reaches the Pi MAC (mib). Full-duplex confirmed (so not HD deferral). Pi-side power-saving EXHAUSTED. FORK: (a) another Pi-PHY/MAC post-TX RX-gate register (GENET EXT_ENERGY_DET? RGMII? MAC TX→RX turnaround?); (b) the NEWLY-ADDED 2.5G switch (owner changed setup — switch is new; "worked at 100Mbps" may have been the OLD direct cable) — switch EEE/store-forward could drop the reply that closely trails the Pi's frame; (c) host NIC. Owner "assume Phoenix SW" but attribution can't tell Pi-PHY-drop from switch-never-sent. NEXT: netem-WITH-ping-proof (does a proper host delay fix it → timing/switch) + consult advisor on Pi-PHY-other vs switch. Refuted-verified now: ring-strand, corrupt, lwip, RGMII(fcs=0), DMA-stall, macSetSpeed-tick, EEE/AutogrEEEn/APD.

2026-08-26 (session ~340 cont. — ADVISOR REFRAME: Linux-Pi4 gigabit-NFS baseline = SAME Pi/switch/host works → 100% Phoenix PHY/MAC CONFIG bug; diffing firmware's working config):
 Advisor: the Linux-Pi4 netboot baseline NFS-roots at gigabit over THIS EXACT Pi + switch + host NIC [[project_linux_pi4_netboot_reference]] → switch/host/wire PROVEN-working at gigabit → the ~30-cycle fork (switch/host) is CLOSED; bug is 100% Phoenix PHY/MAC config (stays inside owner "assume Phoenix SW"). METHOD CHANGE: stop guessing registers one-at-a-time — DIFF the VideoCore firmware's WORKING gigabit config (it did gigabit TFTP seconds before Phoenix starts; registers are proven-good on the same silicon) vs Phoenix's. Committed lwip WIP (60cff42) to protect the night's work (fixes + diag). Added GENETCFG dump: at driver entry BEFORE reset/config (firmware = golden) + after Phoenix config on link-up — EXT_RGMII_OOB_CTRL, EXT_PWR_MGMT (energy-det), SYS_PORT_CTRL, UMAC_CMD, EXT_GPHY_CTRL. Build bdwahgxfr → cycle → diff = candidate. TOP suspect: EXT_RGMII_OOB_CTRL — Phoenix SETs OOB_DISABLE + clears ID_MODE_DIS (matches Linux, DIVERGES from U-Boot which does inverse); dismissed earlier as "corruption→FCS" but my attribution shows frames vanish before MAC with ZERO FCS = exactly wrong in-band/OOB RGMII status config. SCRUB earlier ">2ms window" (invalid netem); DHCP OFFER received ~1-2ms after DISCOVER TX → window plausibly sub-2ms (check OFFER L2 dest broadcast via tcpdump -e). Also queued: netem-5ms-WITH-ping-proof.

2026-08-26 (session ~340 cont. — ★★★ CONFIG DIFF vs firmware: TWO divergences (OOB_DISABLE + pause-ignore); testing OOB_DISABLE-clear):
 GENETCFG fw(WORKING) vs phx dump — only 2 registers differ:
  • EXT_RGMII_OOB_CTRL: fw=0x00f00050 vs phx=0x00f00070 → ONLY diff = OOB_DISABLE (bit5): firmware CLEARS it, Phoenix SETS it. (ID_MODE_DIS=0 in both — RGMII delay matches.)
  • UMAC_CMD: fw=0x1000010b vs phx=0x0000000b → fw sets CMD_RX_PAUSE_IGNORE(bit8)+CMD_TX_PAUSE_IGNORE(bit28); Phoenix HONORS pause (clears both).
  • pwrmgmt/portctrl/gphyctrl identical.
 OOB_DISABLE is the RX/link-relevant divergence + advisor's top suspect + diverges from BOTH firmware AND U-Boot; pause affects TX so it's a weaker RX hypothesis. TESTING OOB_DISABLE-clear first (match firmware: v |= RGMII_LINK|RGMII_MODE_EN; v &= ~(ID_MODE_DIS|OOB_DISABLE)). Directly contradicts the old Phoenix comment ("OOB_DISABLE must be SET or RX dies, PROD stuck at 0") — but the firmware clears it and receives fine on THIS silicon → context-specific. Build beoiufr7s. Outcomes: NFS mounts → OOB_DISABLE was the bug (post-TX RX-drop from static-link RX mishandling); total RX-fail/no-DHCP → OOB must be set, revert + test pause; same partial-fail → revert + test pause-ignore next. This is the methodical firmware-diff approach (not register-guessing).

2026-08-26 (session ~340 cont. — OOB-clear NEGATIVE (didn't break RX, didn't fix); testing last MAC diff = pause-ignore):
 OOB_DISABLE-clear: did NOT fix (gap ~1.0s) but did NOT break RX either (DHCP+mount attempts ran) → disproves the old "OOB must be set" comment; OOB not the cause. Kept OOB-clear (matches firmware, harmless). Last remaining MAC-register divergence = UMAC_CMD pause-ignore: added CMD_RX_PAUSE_IGNORE|CMD_TX_PAUSE_IGNORE in macSetSpeed to match firmware's 0x1000010b. Build b7xoczkhx. Now Phoenix MAC config matches firmware on ALL dumped regs. If NFS mounts → firmware-match (pause-ignore) was it. If not → the cause is PHY-side (couldn't dump firmware's PHY config — phyHardReset wipes it before MDIO is up); next = read firmware PHY regs BEFORE phyHardReset, or Linux-netboot PHY dump [[project_linux_pi4_netboot_reference]]. Rationale for pause: with the new 2.5G switch, a PAUSE frame trailing our TX that Phoenix HONORS (vs firmware IGNORES) could gate the MAC and drop the following reply — fits post-TX + gigabit + the switch being the new variable.

2026-08-26 (session ~340 cont. — MAC config FULLY MATCHES firmware, still fails → cause is PHY-side; will dump firmware PHY config):
 pause-ignore: negative (gap ~1.04s). So BOTH MAC divergences (OOB_DISABLE + pause-ignore) tested → Phoenix MAC config now FULLY matches the firmware's proven-working gigabit config (all 5 dumped MAC regs identical), yet the post-TX RX drop persists. By elimination the cause is PHY-SIDE (the registers I couldn't dump — genet_phyHardReset wipes the firmware's PHY config before MDIO was up). Found genet_mdioRead (bcm-genet.c:470) = low-level MDIO primitive usable BEFORE register_mdio_bus. PLAN: dump the firmware's WORKING PHY regs (BCM54213PE @ MDIO addr 1) BEFORE genet_phyHardReset (bcm-genet.c:1609), and Phoenix's PHY regs after ephy_init, diff. Suspect PHY regs for post-TX RX drop: AUXCTL(0x18) misc/RGMII-RX-skew shadows, 0x1C shadows (clk ctl / power / RX gating), RGMII RX config. Alt: genet_phyHardReset itself may be harmful (wipes firmware's good PHY config, ephy_init doesn't fully restore). MAC-side changes committed (OOB-clear + pause-ignore match firmware, harmless, keep). Consulting advisor on efficient PHY-diff (which regs, MDIO timing before hardReset).

2026-08-26 (session ~340 cont. — ADVISOR: 1000BASE-T echo-cancel/DSP-training NOT register-visible → ADOPT firmware PHY (skip reset/retrain)):
 Advisor's dominant insight: 1000BASE-T runs all 4 pairs bidirectionally with ECHO CANCELLATION — a PHY with wrong analog/DSP training state gets BLINDED BY ITS OWN TRANSMISSION → own-TX→RX-deaf, no FCS, inherently gigabit-only (100BASE-TX uses separate TX/RX pairs → immune → why 100M worked). That DSP/training state is NOT register-visible — a config diff reads clean while DSP differs. So the dominant test is ADOPT the firmware's already-trained PHY: GENET_PHY_ADOPT_FW=1 gates OFF genet_phyHardReset (bcm-genet.c) + ephy_reset + ephy_restartAN (ephy.c), keeping all READS (phyid, ephy_linkSpeed) + MAC-side configRgmii/macSetSpeed. Firmware leaves link up@1000FD (UMAC_CMD had TX_EN|RX_EN), so link poll reads 1000FD + macSetSpeed programs MAC. Build bg9sfifrt. Outcomes: NFS mounts → PHY reset/retrain (DSP/echo-cancel state) WAS the root cause + working baseline to bisect (re-add PHY writes group-by-group; if hard-reset alone re-breaks → reset-then-retrain itself); still fails → PHY config eliminated wholesale → runtime behavior (1Hz link-poll MDIO). Committed WIP-2 (43ecb98). VERIFIED progress to present owner: attribution chain (reply vanishes pre-MAC, no FCS, post-own-TX, gigabit-only) + ~8 verified eliminations (ring-strand/corrupt/lwip/RGMII-FCS/DMA-stall/macSetSpeed-tick/EEE-AutogrEEEn-APD-verified/MAC-config-matches-firmware).

2026-08-26 (session ~340 cont. — PHY-adoption test was INVALID (stale image); fixed unused-function build errors, re-running):
 CAUGHT: the prior GENET_PHY_ADOPT_FW cycle tested a STALE image — the build had -Werror=unused-function errors (ephy.c:212 ephy_reset, bcm-genet.c:538 genet_phyHardReset became uncalled when gated out), lwip failed to compile, and the cycle ran the PREVIOUS loader.disk (adoption string absent from loader.disk, count=0; no "adopting firmware PHY" in UART). So "PHY eliminated wholesale" was UNEARNED — adoption was never actually active. STALE-IMAGE TRAP: "Verification: OK" is the image checksum, passes even when lwip didn't rebuild. FIX: marked genet_phyHardReset + ephy_reset __attribute__((unused)). Rebuild b4l4t1w7j (verifying adoption string IS in loader.disk this time) → re-run the REAL adoption test. Still the dominant test: adopt firmware's trained PHY (skip reset/retrain) → mounts = PHY DSP/echo-cancel training was root cause; fails = PHY eliminated → runtime (link-poll). LESSON for the arc: always confirm the change compiled into loader.disk (strings grep) before trusting a cycle result — several tonight's builds may warrant re-check.

2026-08-26 (session ~340 cont. — REAL adoption REFUTES PHY (still ~1s w/ firmware PHY adopted); pivot to LINUX CONTROL over the NEW switch):
 REAL GENET_PHY_ADOPT_FW test (verified compiled + "adopting firmware PHY" printed): NFS still fails, gap still ~1.04s. So with the firmware's trained PHY genuinely adopted (no reset/retrain, DSP/echo-cancel state preserved), the post-TX RX drop PERSISTS → PHY config/training/DSP ELIMINATED wholesale; echo-cancellation hypothesis refuted. CRITICAL REALIZATION: the advisor's premise "Linux baseline proves this exact switch/host works at gigabit" is UNVERIFIED for the NEW setup — the owner ADDED the 2.5G switch at the START of this session; the [[project_linux_pi4_netboot_reference]] Linux-gigabit baseline predates it. AND: firmware TFTP (reply-after-TX, ~100us turnaround, dense) works over the switch — but that's the FIRMWARE's stack, and dense≠sparse. So the true control is MISSING: does real LINUX NFS-root at gigabit over the CURRENT 2.5G-switch setup? Owner explicitly offered "netboot Linux on Pi4 and do a comparison." NEXT: netboot Linux over current setup → Linux works = Phoenix runtime bug (continue: link-poll MDIO / TX-path); Linux ALSO fails = switch/HW-level (report to owner, not a Phoenix bug). Eliminated: driver-ring/DMA/lwip, MAC-config, PHY-config/training. Remaining Phoenix-side suspect if Linux works: runtime behavior (1Hz link-poll MDIO, single-slot polled TX).
