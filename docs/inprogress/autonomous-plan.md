 ...
 **Ran the FIRST q2 Pi capture (2 cycles):** cycle 1 = transient boot fail (USB/xHCI dump, no psh — retried). cycle 2 = booted, quake2 ran the demo (ca_active, Map demo2), sink listening on 5599 — but **0 frames streamed.** ★ BLOCKER CHARACTERIZED: UART shows `phxgl: capture FBO 1920x1080 status=0x8cd5`(=GL_FRAMEBUFFER_COMPLETE) then NOTHING — neither the hook's `CAPTURE: tcp` nor `CAPTURE: cap_` message → the hook fired + called `phxgl_capture_gl` (sources/phoenix-rtos-ports/sdl2/glue/sdl_phoenix_glctx.c) which then **stalls/fails on the Pi readback** (the subagent's explicitly-flagged unverified path), so flow never reaches the stream/file branch. TWO facts: (1) render is **1920×1080 not 1024×768** — Phoenix SDL2 has no WM so `r_customwidth` is ignored (always fb-native) ⇒ regenerate host refs at 1920×1080 to pair; (2) phxgl_capture_gl "proven for Q1" per the subagent but Q1 may have run at a smaller res — so it may fail specifically at 1920×1080. NEXT: read phxgl_capture_gl in sdl_phoenix_glctx.c (blit+glReadPixels of the scanout FBO); add a diagnostic AFTER the readback; compare vs Q1's proven usage/res; Pi-test at 1920×1080 with host refs regenerated to match. NOT a TCP-transport problem (that path is fine); it's the scanout-FBO readback at fb-native res. thread-B (q3dm7 wedge) + O1 glamor-flip HW-confirm still pending.
