...
2026-08-23 (session ~233 — flaky-CDN night: SECOND CDN (freedesktop) down; build b_died at fontconfig; recovering):
 ...
 - **IN FLIGHT:** subagent fetching fontconfig-2.14.2.tar.xz + cairo-1.16.0.tar.xz (+ freetype/libpng/jpeg for completeness) from working historical-tarball mirrors into the shared cache ($HOME/.phoenix-distfiles/xorg). Direct mirror probes this turn failed (fossies keeps only current; freebsd distcache/gentoo use hashed subdirs; cairographics 000) → delegated the multi-mirror hunt.
 **NEXT (this/next turn):** (1) when subagent lands fontconfig+cairo in the cache → restart `--scope auto --with-ports --with-showcase --with-vkquake` (RPI4B_BUILDROOT=.buildroot-gcc16) → resumes at fontconfig via cache HIT → cairo (cache) → libXft (cached) → xorg_server (artfiles) → X + image. (2) DURABLE FIX for the Docker release build (fresh empty cache): point xorg_fonts' fontconfig/cairo (+ sourceforge/ijg) fetches at RELIABLE mirrors as primary or fallback, so a from-scratch build doesn't depend on a single flaky CDN — the local cache alone doesn't help a fresh Docker container. Do this once the subagent confirms which mirrors are reliable. Then stage + boot-verify → manifest/tag → G3.2 Docker (needs host buildx) → G5 publish.
