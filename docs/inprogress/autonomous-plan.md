 ...
 **NEXT:** (1) monitor pid 2450917 → confirm gcc-16 nfsroot image built clean; (2) assemble a servable gcc-16 bootfs (gcc-14 RPi firmware + gcc-16 Phoenix files, per the memory's proven recipe) + stage the gcc-16 NFS rootfs with the (gcc-14 static) game binaries → owner-testable; (3) then the "full clean --with-ports skip-xorg" gcc-16 rebuild for completeness; (4) gcc-16-games stretch (teach tools scripts the toolchain override + verify Mesa under gcc-16). Then the deferred renderer features F1/F2. Remaining game-bug digs (Q1 textures, vkQuake fire/light, USB, Q2 slow-load) interleave.
