 ...
 **2c-test (NEXT after 2c-server):** the winsys-as-client refactor (an #ifdef/build-variant so v3d_phoenix_winsys.c's phoenix_v3d_ioctl routes to the daemon via libv3d-client, gallium glue reads BO cpu/va from client responses) → relink gl_frontend_smoke against it → HW-test a GL render-clear through the daemon (first CL proof on HW). Then M2 (2 clients) + M3 (glamor-X + GLQuake concurrent).
