 ...
 **NEXT (check each heartbeat):** poll docker-release-g33.log for BUILD_RC=0 + docker-out/rpi4b-sd-2part.img. If clean → G3.3 boot-verify → G5 (manifest + tag known-good/gcc16-release-2026-08-25 + org README flip + publish). If it fails again, diagnose from the log + relaunch. Docker build = HOST CPU not API tokens (safe under spend-limit).
