 ...
 **NEXT:** read the Pi cycle UART — if bash-nojc reaches/prints a PS1 prompt (vs JC-on returning to (psh)%), the JC root-cause + Fix A are HW-CONFIRMED → PUSH the 5 local commits (libphoenix/tests/ports) + make JC-off the bash default + correct stale KNOWN-ISSUES/pty-run "fd 0 EOF" text. gcc-16 full build already SUCCEEDED (image exported); after bash, gcc-16 boot-verify → manifest/tag → G3.2 Docker release (launch-ready) → G5.
