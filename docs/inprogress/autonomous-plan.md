 ...
 **NEXT (post owner-test, Pi free):** `./scripts/rebuild-rpi4b-fast.sh --with-showcase --with-vkquake` → verify `.buildroot/_fs/.../usr/bin/rpi4-quake` + `rpi4-vkquake` carry the fixes (strings/mtime) → `netboot-server-up.sh` (syncs to export) → one Pi cycle each to confirm flagship rpi4-quake renders clean + rpi4-vkquake shows torches. Then Issue 3 (X11 offset repro) + Tier-1 P8 migrations. Act on any owner test-result report first.
