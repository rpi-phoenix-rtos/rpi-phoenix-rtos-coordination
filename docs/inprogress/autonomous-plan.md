 ...
 **NEXT:** relink one GL game (quakespasm/quake2) against the fresh libv3d, stage into /srv/phoenix-rpi4-nfs, netboot + HDMI grab → confirm shred gone → commit (coord + mesa branch phoenix-v3d-port-26.2, push publish when verified). Then Issue 2 (vkQuake torches — may relate to the #51 binner wedge) + Issue 3 (X11 GL Y-offset). Also fixed quake2-launcher video args (c411b87) + nfs-fs readdir ./.. (ee32037) earlier this session.
