 ...
 **NEXT (G5 publish — the release declaration):** tag known-good/gcc16-release-2026-08-25 across the 4 pushed repos + coord (rollback anchors, reversible) → org README flip to "gcc-16.2.0 released" + publish the image artifact (sha256 29e20cd8…). The README-flip + artifact-publish is the public "released" declaration; weigh doing it now vs a light owner touch given G3.3's full-image-boot is the one banked residual. Follow-ups: wire bash into ports.yaml so the image ships it; fix the sync-netboot-tree/-gcc16 export mismatch (sync writes default dir, Pi mounts -gcc16). Both Monitors fired + ended.
