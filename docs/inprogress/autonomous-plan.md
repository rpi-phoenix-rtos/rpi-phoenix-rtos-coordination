 ...
 **NEXT (per gcc16-release-plan.md):** G1.1 finish/verify the running full build → G2.1 bump build-phoenix-toolchain-linux.sh to gcc-16.2.0 → G2.2 promote `.toolchain`→gcc-16 (tag gcc-14 rollback first) → G2.3/G1.2 verify all scripts + GPU/games under gcc-16 → G3 Docker --no-cache gcc-16 (BUILD_RC=0) + boot-verify → G4 docs → G5 manifest+tag+publish gcc-16 release. Interleave: open game-bug digs (Q1 tex, vkQuake fire/light, USB, Q2 slow-load) + deferred renderer features F1/F2.
