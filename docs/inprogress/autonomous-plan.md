 ...
 **NEXT (monitor fires):** stage the themed wmaker + /usr/share/{WindowMaker,WINGs} + /etc/WindowMaker to the -gcc16 netboot root → Pi cycle `startx` → confirm HDMI shows a THEMED WindowMaker (dock icons, styles, swback/logo now that PNG is on) → if good, COMMIT the port change + push + update README.
