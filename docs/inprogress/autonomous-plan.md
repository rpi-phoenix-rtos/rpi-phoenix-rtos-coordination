...
2026-08-25 (session ~260 — ✅ startx_gpu (GPU-accelerated X) DONE + HW-verified; Quake2 autonomous repro not tractable):
 ...
 - **wmaker hang confirmed by OWNER** (black+cursor, mouse works) — wmaker-specific (twm/xterm render perfectly via startx term). Likely WINGs phxfile/Xft/freetype font-load hang; needs a hung-process backtrace (rebuild WINGs w/ font-load prints). Default X = software fbdev (not GPU) — answered.
