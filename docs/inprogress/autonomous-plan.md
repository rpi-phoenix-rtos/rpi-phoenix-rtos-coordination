 ...
 **NEXT (Pi free — EXECUTING step-by-step):** (1) combined build — relink yquake2 (fb-recovery) + rebuild vkquake (8M stack) + `--scope core` image (nfs print) → restage → Pi-validate the 3 ready fixes; (2) Q3 u_vbuf crash; (3) Q1 textures; (4) USB warm-reboot; (5) Q2 slow-load; (6) vkQuake 2a/2b. THEN gcc-16 migration → clean rebuild → gcc-16 NFS rootfs. Also still-pending flagship rpi4-quake/rpi4-vkquake rebuild (folds into the gcc-16 clean rebuild anyway).
