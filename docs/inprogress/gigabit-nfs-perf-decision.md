...
### REMAINING — the raw ceiling itself (§3-A, HIGHEST leverage, careful)

Per the lwip-perf research (`2026-08-26-lwip-perf-optimization-research.md`) + our own root-cause: the 37.5 raw ceiling = 39.5 µs/frame, ~27 µs of which is **`SYS_ARCH_PROTECT` = a raw Phoenix `mutexLock` syscall** (`port/protect.c`) — a PORT ARTIFACT, not an lwIP property (lwIP's contract is a lightweight critical section; XAPP1026 shows lwIP RAW hitting 943 Mb/s on a 666 MHz A9 with a cheap lock). Giving `sys_arch_global_lock` an uncontended userspace fast-path (est. raw 2–2.5× → ~75–90 MB/s) is the biggest remaining prize and is lwip-process-scoped (NOT the kernel-wide mutex). BUT a naive userspace spinlock risks **priority inversion** (high-prio same-core spinner vs low-prio holder = deadlock) and an adaptive/futex lock needs a Phoenix primitive + concurrency proof → **needs a careful design + advisor concurrency review before implementing** (not a rush). Secondary: §3-A2 static memp pools (`MEMP_MEM_MALLOC=0`, needs pool sizing); §3-B moderate `LWIP_WND_SCALE` re-test. Multi-connection (NFS `nconnect`) striping is the other real bulk lever (composes with lwip). Kernel-IPC-latency + multi-core stay owner-gated.
