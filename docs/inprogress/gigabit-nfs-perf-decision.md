...
**NEXT (bounded, confirmed direction):** profile the credit chain / socket_thread service rate — the question is now *"which serial stage saturates at ~30 MB/s"* (prime suspect: socket_thread `pbuf_copy_partial` + per-recv cross-process IPC), not "which hop adds latency." Timestamp on the Pi: data-arrival (recv_tcp) → recvmbox-post → socket_thread-wake → copy → `netconn_tcp_recvd`/ACK-emit. Multi-core / mutex-fast-path (#3) remain owner-gated. (Caveat for the flip-on checklist: the GENET_RX_INPUT_BATCH=32 config was validated rate-only, never sha256 — the bit-exact gate applies at batch-ON before any default flip; the shipped default-off state is bit-exact.)
