...
- **fb0/GPU Tier-1 groundwork** — diag-udp `V` probe (lwip `fdbec01`) proves the
  ...
  written up in `docs/inprogress/2026-06-05-fb0-attended-decisions.md`.
...
- **Rootfs ext2 image + 2-partition SD builder** (`scripts/build-rpi4b-rootfs-ext2.sh`,
  ...
  `docs/done/2026-06-03-sd-rootfs-plan.md` "Morning steps".
...
- **Testing note:** the SD driver can only be exercised by SD-boot (a card in
  ...
  netboot. See `docs/done/2026-06-03-sd-rootfs-plan.md`.
...
- **Next focus:** SD card (EMMC2, #119) + real persistent rootfs (#120) —
  detailed plan in `docs/done/2026-06-03-sd-rootfs-plan.md`.
...
`docs/done/2026-06-02-p1p2p3-postfix-10boot.md` (and the prior baseline study
`docs/done/2026-06-02-10-boot-consistency-study.md`).
...
`docs/done/2026-06-02-p1-ab-verdict.md`).** The earlier framing here ("enum went
...
detail: `docs/done/2026-05-29-usb-reanalysis.md`. Headlines:
...
`docs/done/2026-05-29-usb-reanalysis.md`.
...
`docs/done/2026-05-23-usb-bridge-cooldown-curve.md`.
...
- **TD-13-spawn-cap**: verified inactive across 232 captured
  ...
  `docs/done/2026-05-23-td13-spawn-cap-analysis.md` with the
...
2. **Erratum 1319367 register-encoding fix** — Phoenix had been writing
   ...
   `docs/done/a72-errata-sweep.md` is `CPUACTLR_EL1[46]=1`
...
- `scripts/bootstrap-linux-host.sh` + `docs/knowledge/linux-host-bootstrap.md` —
...
designed (`docs/done/2026-05-15-cache-enable-c-approach-design.md`) and
...
* iter-12-diag (image `b597c1f7…1e743807`) proved the walker uses
  ...
  [docs/done/2026-05-13-iter-11-12-cache-walker-finding.md](../done/2026-05-13-iter-11-12-cache-walker-finding.md).
...
`docs/inprogress/TEMPORARY-FIXES-AND-FUTURE-CLEANUP.md`). Until that lands, both
...
`docs/done/plo-el2-mmu-fix.md` plan recommended Path A: generalise
...
`docs/done/early-boot-diagnostic-instrumentation.md`) must land
...
plus `docs/knowledge/round3-cache-enable-synthesis.md` integrating
everything.
...
`docs/done/canonical-idiom-step2-step3-plo-patches.md` for
...
`docs/done/canonical-idiom-step5-mci-flip.md`.
...
Single source of truth: **`docs/done/roadmap-cache-ram-smp.md`**.
