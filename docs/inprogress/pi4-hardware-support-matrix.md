...
| Audio (PWM / I²S / HDMI) | 🟡 partial | PWM driver `/dev/audio0` (`audio/rpi4-audio/`): clock+FIFO bring-up + DMA-paced feed proven (PWM1=DREQ 1); **Quakespasm SNDDMA backend works** — feeder thread → `/dev/audio0`, "Audio: 16 bit, stereo, 44100 Hz", demo renders 40+s w/ mixing live, 0 faults (2026-06-17) | audible jack sign-off ⏸ (headphones); continuous-DMA-streaming (efficiency, so write() doesn't PIO-spin); vkQuake reuses the backend |
