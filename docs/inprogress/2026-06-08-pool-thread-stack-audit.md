...
| **lwip — genet irqThread** | `phoenix-rtos-lwip/drivers/bcm-genet.c:1229`; buffer `:130` `irq_stack[2048]` (`uint32_t` ⇒ **8 KB**) | **8 KB** | Moderate — IRQ → ring drain → `pbuf`/netif input | **RISK (borderline)** | bump to **16 KB** for margin; empirically stable (ping RTT ~0.9 ms) but it is *exactly* the 8 KB that bit #120 |
| **lwip — genet linkPollThread** | `phoenix-rtos-lwip/drivers/bcm-genet.c:1243`; buffer `:102` `link_poll_stack[1024]` (`uint32_t` ⇒ **4 KB**) | **4 KB** | Shallow — MDIO poll loop | **OK** (shallow, no fs/DMA recursion) | none (could bump to 8 KB for uniformity) |
