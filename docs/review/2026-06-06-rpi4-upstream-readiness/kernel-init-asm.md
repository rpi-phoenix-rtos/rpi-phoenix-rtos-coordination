...
But `_init.S:515-533` ("Phase Z3 … TD-04 NC override DELETED") removed that override, and the syspage TTL3 pages (`PMAP_COMMON_KERNEL_TTL3`/`_TTL3_1`) are now filled with `DEFAULT_ATTRS` (0x703 = cacheable) at `_init.S:501-513`. `NC_ATTRS` is now referenced **only** for the TD-15 mailbox alias at `_init.S:492`. `docs/inprogress/TEMPORARY-FIXES-AND-FUTURE-CLEANUP.md` confirms TD-04 RESOLVED (cleanup complete). So the syspage copy now goes through a **cacheable** mapping; the comments claiming it bypasses the D-cache are factually inverted.
