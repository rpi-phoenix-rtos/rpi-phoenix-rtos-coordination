...
> **Related**:
> - `docs/research/gpu-vc6.md` — Linux-flavoured VC6 GPU research brief
>   (HVS / PV / V3D / mailbox / Mesa).
> - `docs/research/gpu-vc6-non-linux.md` — non-Linux community work on
>   the same hardware (FreeBSD / NetBSD / Circle / Ultibo / LdB-ECM).
> - `docs/wifi-bringup-plan.md` — template for tier shape.
> - `docs/bluetooth-bringup-plan.md` — template for "we may stop at
...
This document is the same shape as `docs/wifi-bringup-plan.md` —
...
3. **A keyboard input path.** USB HID is parked (USB-HCD is task
   ...
   flaky today — see `docs/status.md`). **Decision: skip input for
...
- **TD-15 still open.** The VC4/V3D + Phoenix DRAM hygiene work
  (`docs/TEMPORARY-FIXES-AND-FUTURE-CLEANUP.md` §TD-15) is pending.
...
- **Firmware ownership of HDMI scanout.** The VPU keeps the HDMI
  pipeline alive after handoff (per `docs/rpi4-os-development-guide.md`
...
- **No JTAG today**, so any V3D crash that hangs the SoC requires a
  ...
  `docs/status.md`) makes this less painful but iteration is slow.
...
- **Phoenix-RTOS in-tree references**:
  ...
  - `docs/research/gpu-vc6.md`, `docs/research/gpu-vc6-non-linux.md`
  - `docs/rpi4-os-development-guide.md` (mailbox, framebuffer,
