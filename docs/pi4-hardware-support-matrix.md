...
| vkQuake (Vulkan Quake) | ✅ HW-validated | framework port `vkquake` → **`/usr/bin/vkquake`** (ships on the SD image); textured 3D via Vulkan on V3D; torch/alpha-scanout fixed (d3e329c); the earlier **post-menu hang is FIXED** (a libphoenix counting-semaphore lost-wakeup, `e75c4fe`). Re-verified on the clean image: **renders the start map** (`artifacts/hdmi/20260903-040557-vkq-rep2-tick.png`). The port's link carries `--build-id` (V3DV's `init_uuids()` needs the note or no Vulkan device is created) and a 32 MiB `PT_GNU_STACK`. Residual: an intermittent V3D binner wedge on long GPU runs, not Vulkan-specific (`project_vulkan_v3dv_port`) |
