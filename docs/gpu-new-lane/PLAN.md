...
| E2c | What do the same STK/quakespasm settings give on Raspberry Pi OS on this board (fps, V3D render ms/frame)? | [E2c-pios-baseline.md](E2c-pios-baseline.md) | prepared: Pi OS rootfs has STK 1.4 (GLES2 build) + Mesa 26.2.2 + quakespasm/glmark2/kmscube/vkmark, Phoenix's STK data + configs; `scripts/e2c-cycle.sh --arm stock\|phxclk250\|phxclk500` (restores Phoenix netboot in a trap). Not run |
