...
| **xz 5.4.7** (2024-05) | ~2 y | Current is **5.8.3**. Flagged explicitly because the choice was made on **2026-08-27**, when 5.8.x already existed, and **no reason is recorded**. It is *plausible* this was deliberate caution around the 5.6.x supply-chain incident — 5.4.x being the maintained pre-incident branch — but **that is not written down anywhere, so I am not asserting it.** Either write the reason down or bump it. | Consumed only by the `python-3.14.4` port (its `lzma` module); no other dependent. |
