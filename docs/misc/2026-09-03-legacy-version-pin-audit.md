...
|---|---|---|---|---|
| 1 | **Delete `mc`'s `mc-support` mntent/langinfo stub + the `hasmntopt=no` cache override** | High — removes a live *correctness* divergence (stub says `CODESET="UTF-8"`, libphoenix deliberately says ASCII) and a whole staged-header shadowing hack; exactly the nano pattern at accommodation level | **Low** | libphoenix `getmntent` family — `mntent/mntent.c`, commit **`29f5373`** (2026-08-13); `nl_langinfo` — `locale/langinfo.c`, **`7bf090f`** (2026-07-12, codeset corrected in `491618c`) |
