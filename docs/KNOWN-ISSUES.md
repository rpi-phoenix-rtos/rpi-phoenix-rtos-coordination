...
| allocator-double-free | libphoenix's allocator reports a double free and the process exits **70** (`EX_SOFTWARE`). **Not AF_UNIX-specific**, which is how this row used to describe it: the report appears in **21 logs from 2026-06-28 → 09-09**, across **xcalc (×7), Dillo, vkQuake (×2), the glamor desktop / Window Maker / showcase (×5)** and `test-libc-unix-socket` (×4). It is a **generic heap-corruption symptom** with several distinct causes, some already fixed separately (the `vasprintf` overflow behind glib2/mc/Dillo; `free()` of `.rodata` in X11). Older logs say `Double free detected`, newer ones carry a named report. **Block class is now known from evidence**: one occurrence records `size = 0x410` = 1040 = `PATH_MAX` + chunk overhead, i.e. a `resolve_path` buffer, un-coalesced (so both neighbours were in use). Three mechanisms eliminated: duplicate hand-out (refuted over ~300 instrumented children), a bin-abandon regression (fixed; it postdated every occurrence), and a server over-writing the buffer (impossible on a path with no symlink component). Ownership audit of `resolve_path`/`unlink`/`sys_clear` is clean. | Open, rare. `free()` now prints **its caller** (`__builtin_return_address(0)`), resolvable with `addr2line` against the unstripped `prog/` copy — so the next occurrence names the offending free site. |
