...
| Q2-underwater-stopgap | Quake II's underwater Y-mirror is fixed by a **conditional correction on a conditional bug**, which over-corrects at `viewsize <= 71`. The clean fix is to replace Mesa's `>=1024x768` FlipY size gate with a real scanout predicate. ⓘ Its performance justification was **measured and refuted** (see the STK note in the changes doc), so what remains is code hygiene: five per-app compensators today, and the next port with a large offscreen FBO renders upside down until someone adds a sixth. | Open (stopgap shipped); the real fix touches every GL app plus X. |
