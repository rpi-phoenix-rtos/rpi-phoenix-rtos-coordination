...
| vkQuake-input | **vkQuake keyboard/mouse input is not wired** — events don't reach the game. | Partly resolved: the earlier post-menu **hang is FIXED** (it was a libphoenix counting-semaphore lost-wakeup, `e75c4fe`) — vkQuake now renders the full textured 3D start map on the GPU (3150+ frames, HW-confirmed). Remaining: un-wired SDL input (owner-attended) and an intermittent GPU binner wedge under investigation. |
...
| GPU Vulkan (V3DV) | Full Vulkan (V3DV) is available on Phoenix with GPU acceleration on real BCM2711 V3D 4.2 hardware: init, texture upload (no-WSI buffer→image copy resolved), SPIR-V vertex/fragment/compute shaders, and render passes all execute on the GPU. vkQuake runs GPU-accelerated (fb0 scanout, no WSI) and renders the full textured 3D start map (the earlier post-menu hang is fixed); **keyboard/mouse input is not yet wired** (see the `vkQuake-input` row above). | Driver works (GPU-accelerated); vkQuake renders but is not yet interactive. |
