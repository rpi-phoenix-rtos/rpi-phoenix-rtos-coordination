...
|---|---|---|
| v3d-submit-race | ~~vkQuake froze permanently at present #26 in ~3/8 boots, with 14 CSD timeouts, and ~3/8 test captures were unusable~~ **FIXED 2026-09-03** (devices `e8a72e9`). `phoenix_v3d_ioctl` had **no serialization** around shared V3D MMIO (CT0/CT1 queues, CSD queue, MMU, cache control) or global BO/VA/page-table state, while vkQuake submits render and compute from **detached** worker threads (`Quake/tasks.c:439`). Linux never hits this: its kernel driver locks and the DRM scheduler keeps one job per queue in flight. The race is as old as the file; it became visible when libphoenix changed detached-thread exit/stack reclamation on 2026-09-01 (`4c97a79`, `c8ee89e`). Verified n=8: **0/8** freezes, **0/8** degraded, **0** CSD timeouts, **0/8** unusable captures, presents 3,510–3,780 (early-August levels). | Fixed. |
