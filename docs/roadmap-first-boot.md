...
is touched. The roadmap is aspirational; `docs/status.md` is authoritative
for current state.
...
steps) in the sense of `docs/execution-control.md`. Snapshot an integration
...
- Marker `f` reached (per `docs/status.md`'s recorded marker legend).
...
`docs/implementation-dossier.md`. Pi 4 uses GIC-400 (GICv2), which is
...
- **Cannot** run in parallel: anything touching the boot path. The
  single-active-step rule (`docs/execution-control.md`) still holds.
