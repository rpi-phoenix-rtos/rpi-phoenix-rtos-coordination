...
| Is the firmware writing into its own boot-time area after hand-over? `config.txt` loads `loader.disk` at `0x08000000` (4 872 064 B in build 3 ⇒ ends `0x084a5780`); the `c1cold` window `0x08408000`–`0x084e3000` **straddles that end** (3 victim pages inside the image tail, 3 just past it, where the firmware also places boot data such as the DTB). ⚠ "inside the initramfs" was refuted earlier on the wider 0.86–5.83 MiB band with 4.22–4.62 MiB images — re-check each archived victim against **its own build's** image size before reviving it. | per-run image size vs victim PA (archive only, no Pi); then the DTB address the firmware passes in `x0` | open — 2026-09-26 |
