...
2. [docs/inprogress/WEEK-2026-W39.md](docs/inprogress/WEEK-2026-W39.md) — the CURRENT WEEKLY LOG: decisions needed from the owner, plan for the next days, in-progress work, progress, blocked items. This is the single planning/progress doc (see [docs/inprogress/README.md](docs/inprogress/README.md) for the weekly convention; history is in `docs/done/`)
...
- `./scripts/syntax-check.sh <sibling-repo> <path/in/repo.c>` to compile ONE file under the real
  flags (`-Werror` included) **without producing any artifact** — the only way to validate a core
