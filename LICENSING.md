...
| Quake / vkQuake platform glue (derivative of the GPL game engines it links into) | `tools/quakespasm-port/`, `tools/vkquake-port/` | **GPL-2.0-or-later** (each dir's `COPYING`). Optional, opt-in showcases; the BSD core does not depend on them. NOTE: vkQuake is a work in progress; its engine is **published** as an org fork (`rpi-phoenix-rtos/vkquake`, branch `phoenix-rpi4-port`) and cloned into `external/vkquake` by the bootstrap when `--with-vkquake` is passed. |
...
| vkQuake engine | published as an org fork `rpi-phoenix-rtos/vkquake` (branch `phoenix-rpi4-port`, fetched into `external/vkquake` by `bootstrap-linux-host.sh` when `--with-vkquake` is passed) | **GPL-2.0** (upstream). Work-in-progress showcase; our port commits are GPL-compatible. |
