...
| SuperTuxKart source + assets | not in this repo — the port fetches the pinned stk-code 1.4 tarball, and `scripts/stage-game-data.sh` fetches the pinned 1.4 asset package | **GPL-3.0-or-later** (code, per the port's `port.def.sh`); the asset roots carry their own upstream terms. Fetched at build time, not redistributed here. |
