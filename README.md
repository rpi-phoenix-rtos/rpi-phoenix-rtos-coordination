...
| WiFi (BCM43455 SDIO) | 🟡 | **Carries real IP traffic through the network stack** (2026-09-26): joins a WPA2 AP, gets a DHCP lease on its own netif (`wl2`), and ordinary sockets reach that network — `ping` over WiFi 5/5 at 2–9 ms, confirmed by packet capture on the access point. Works today on the **netboot lane**, with credentials in `/etc/wifi.conf` and the `rpi4-wifi` daemon started from the shell. Remaining: it ships in no image yet (the firmware blobs are under a Cypress EULA), credentials are read only at boot, there is no user tool to scan and join at runtime, and the default route stays on Ethernet — so **use wired Ethernet** for general networking |
