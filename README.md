...
| GPU new lane (DRM-shaped stack) | 🟡 | **In development, not on the default image; the shipped apps above still use the current stack.** An asynchronous multi-queue render server (`rpi4-v3d-async`) already runs a cloned Quakespasm **+26 % faster** (38.2 vs 30.4 fps timedemo, on HDMI, 0 GPU wedges); firmware display planes with a real 60 Hz vblank interrupt are proven for a KMS-style display server; zero-copy buffer sharing between processes works (kernel `memExport`). Plan and results: [docs/gpu-new-lane/PLAN.md](docs/gpu-new-lane/PLAN.md) |
