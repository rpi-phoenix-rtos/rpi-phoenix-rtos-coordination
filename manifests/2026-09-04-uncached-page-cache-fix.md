# Integration State: 2026-09-04-uncached-page-cache-fix

## Summary

- Date: 2026-09-04
- Note: Kernel retires stale dirty cache lines when a page is mapped Normal-NC (_pmap_cacheOpAfterChange). Root cause of #67: _pmap_destroy released a process's pages with no cache maintenance, so dirty lines were evicted later over uncached DMA writes and corrupted V3D control lists at 64-byte granularity. Torches 7/7 with the winsys workaround, 4/4 with the kernel fix ALONE; 0 wedges across all boots. Affects every uncached DMA mapping (SD, genet, USB, WiFi).
...
| --- | --- | --- | --- |
| _build | main | 23cc8738c (dirty(2)) | https://github.com/houp/phoenix-rpi.git |
...
| phoenix-rtos-ports | master | 6356a70 (clean) | https://github.com/phoenix-rtos/phoenix-rtos-ports.git |
...
_build	23cc8738c8437e1027c41144545b6a0cc5623117	main
...
phoenix-rtos-ports	6356a701663426c275d9ecfbd9842d952bf0a59c	master
