# Integration State: 2026-05-24-eth-tier3-tx-wire-ok

## Summary

- Date: 2026-05-24
- Note: Tier 3 WIP — TX now physically reaches the wire (confirmed via
  host tcpdump on the netboot bridge: our 02:b8:27:eb:00:01 ARP REQUEST
  for 10.42.0.1 was sent and the bridge replied unicast). Two fixes
  combined got us there: SYS_PORT_CTRL = PORT_MODE_EXT_GPHY (3) and
  dropping the DMA_OWN bit from the TX BD status. RX-visible-in-UART
  deliverable still pending — capture cycles keep terminating before
  lwip prints make it into the picocom log.
...
| --- | --- | --- | --- |
| _build | main | 0853b8d (dirty(5)) | https://github.com/houp/phoenix-rpi.git |
...
| phoenix-rtos-lwip | agent/rpi4-genet | 8e5af39 (clean) | https://github.com/phoenix-rtos/phoenix-rtos-lwip.git |
...
_build	0853b8d5a906e78aabc501af3b8109373135075a	main
...
phoenix-rtos-lwip	8e5af3937a5d8a5321eeea073afa6fe88c34a2b1	agent/rpi4-genet
