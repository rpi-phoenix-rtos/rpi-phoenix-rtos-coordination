# Integration State: pcie-bridge-intact-pmap-bug

## Summary

- Date: 2026-05-21
- Note: End-of-iteration snapshot. PCIe path now does the right
  thing end-to-end: outbound window programming, mailbox firmware-
  notify (after the page-align fix), BAR0, MEM+BME, and re-programming
  of WIN0 after the VC's xhci-reset. Codex-guided Diag #2 confirms
  `MISC_CPU_2_PCIE_MEM_WIN0_LO` reads back as `0xf8000000` (our
  programmed value) — bridge state is intact. The remaining 0xdead
  poison xhci sees is a **CPU-side pmap bug** in the Phoenix kernel,
  not in the PCIe driver. Boot stability preserved at 3/3 cycles.
  See `docs/notes/2026-05-21-pcie-bridge-ageing-codex.md` for the
  full diagnostic record + recommended kernel-side fix direction.
...
| --- | --- | --- | --- |
| _build | main | eb7638b (dirty(5)) | https://github.com/houp/phoenix-rpi.git |
...
_build	eb7638b7ebeda664835d71b5596f01e72c7374a3	main
