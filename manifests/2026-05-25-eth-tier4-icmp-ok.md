# Integration State: 2026-05-25-eth-tier4-icmp-ok

## Summary

- Date: 2026-05-25
- Note: Eth Tier 4 COMPLETE — Phoenix-RTOS now has a working
  TCP/IP stack on the Pi 4 over the built-in GENET v5 + BCM54213PE
  PHY. End-to-end validation: 5/5 host pings to 10.42.0.99 succeed
  with RTT 3.7–16.8 ms. Full chain validated: host ARP-request →
  our ARP-reply → host ICMP echo → our ICMP echo-reply. Final fix
  was stripping ETH_PAD_SIZE (2 bytes) from the TX pbuf before
  copying to the DMA buffer; lwip's ethernet_output left the head
  pad on the pbuf when calling netif->linkoutput.
...
| --- | --- | --- | --- |
| _build | main | 213c137 (dirty(6)) | https://github.com/houp/phoenix-rpi.git |
...
| phoenix-rtos-lwip | agent/rpi4-genet | 3311140 (clean) | https://github.com/phoenix-rtos/phoenix-rtos-lwip.git |
...
_build	213c1372fb4b7b7f9d44ab1944ff72801199ef0d	main
...
phoenix-rtos-lwip	331114045eb8d42807128026f8b309d15b4673f8	agent/rpi4-genet
