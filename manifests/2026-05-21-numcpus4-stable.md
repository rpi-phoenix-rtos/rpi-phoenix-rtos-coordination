# Integration State: numcpus4-stable

## Summary

- Date: 2026-05-21
- Note: **NUM_CPUS=4 on the kernel side now**. spinlock.c, GIC mask,
  and the scheduler all run their NUM_CPUS>1 code paths. Combined
  with plo `PLO_SMP_ENABLE=1`, cores 1-3 wake from the armstub spin-
  table, branch into kernel `_start`, complete their per-CPU
  `_set_up_vbar_and_stacks` + `_hal_interruptsInitPerCPU` +
  `_hal_cpuInit` (atomic increment of `nCpusStarted`), and park in
  the bare WFI loop in `_other_core_virtual`. The scheduler iterates
  over all 4 CPUs but threads are still dispatched only via CPU 0's
  timer interrupt — Phase C (scheduler entry on secondaries) is next.

  Boot stability across 3 cycles: **3/3 reach `(psh)%`**.
...
| --- | --- | --- | --- |
| _build | main | 4696d7e (dirty(5)) | https://github.com/houp/phoenix-rpi.git |
...
_build	4696d7e92bff74bbb8ec189b7c340d2bcdb75dcc	main
