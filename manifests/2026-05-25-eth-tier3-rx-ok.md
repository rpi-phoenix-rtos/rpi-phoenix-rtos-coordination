# Integration State: 2026-05-25-eth-tier3-rx-ok

## Summary

- Date: 2026-05-25
- Note: Tier 3 RX VALIDATED on Pi 4 hardware. RDMA_PROD_INDEX advances
  the moment the bridge's ARP reply (or any other inbound multicast)
  hits our MAC; poll thread consumes the BD and writes CONS_INDEX. The
  RX[0] (126 B) and RX[1] (226 B) frames were captured from real
  network traffic. Root cause of the multi-iteration wedge was using
  the TDMA register layout for RDMA per-ring offsets (PROD/CONS are
  mirrored between the two DMA blocks).
...
| --- | --- | --- | --- |
| _build | main | ad517a5 (dirty(6)) | https://github.com/houp/phoenix-rpi.git |
...
| phoenix-rtos-lwip | agent/rpi4-genet | 0ede54a (clean) | https://github.com/phoenix-rtos/phoenix-rtos-lwip.git |
...
_build	ad517a571c6a4d9fd56ddc70fd084467eaeebc64	main
...
phoenix-rtos-lwip	0ede54a4dae5afffd8044b4a9e70ad057f511724	agent/rpi4-genet
