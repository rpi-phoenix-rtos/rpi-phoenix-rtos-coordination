...
# (tools/hevc-decode/hevc-m2.c) with the aarch64-phoenix toolchain.
#
# Produces a single static ELF (hevc-m2). Links the canonical libvcmbox client
...
OUT="${OUT:-$HERE/hevc-m2}"
CFLAGS="-O2 -static -Wall -Wextra -std=gnu11 -I$VCMBOX_DIR"

for f in "$GCC" "$VCMBOX_DIR/libvcmbox.c" "$VCMBOX_DIR/libvcmbox.h"; do
	if [ ! -e "$f" ]; then
		echo "hevc-m2/build: missing required input: $f" >&2
		exit 1
	fi
done

echo "hevc-m2: compiling M2 decode harness (-O2 -static)"
"$GCC" $CFLAGS -o "$OUT" "$HERE/hevc-m2.c" "$VCMBOX_DIR/libvcmbox.c"
echo "hevc-m2: built $OUT"
