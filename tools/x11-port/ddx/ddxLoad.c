...
#ifdef __phoenix__
/*
 * Phoenix-RTOS has no xkbcomp binary and no xkeyboard-config data, so the stock
 * RunXkbComp() (which forks xkbcomp) always fails and aborts the server. Instead
 * we materialize a precompiled keymap (.xkm) embedded at build time to the exact
 * path the server's LoadXKM() reads, then return its name — no fork required.
 *
 * The embedded bytes are generated on the host by tools/x11-port/xkb/
 * gen-builtin-keymap.sh (which also asserts the XkmFileVersion matches the
 * in-tree xkmread.c). See tools/x11-port/ddx/ddxLoad.c (this durable copy) +
 * build-xfbdev.sh, which mirror this patch the way fbdev.c is the DDX
 * source-of-truth.
 */
#include "builtin_keymap.h"

/* Forward decl: defined later in this file. */
static void OutputDirectory(char *outdir, size_t size);

/*
 * Write the embedded .xkm to "<OutputDirectory>server-<display>.xkm" — the same
 * path XkbDDXOpenConfigFile()/LoadXKM() will later open — and return a strdup'd
 * "server-<display>" mapName on success (NULL on failure), matching RunXkbComp().
 */
static char *
PhoenixWriteBuiltinKeymap(void)
{
    char keymap[PATH_MAX], xkm_output_dir[PATH_MAX], path[PATH_MAX];
    FILE *out;
    size_t written;
    ...
    if (snprintf(path, sizeof(path), "%s%s.xkm", xkm_output_dir, keymap)
        >= (int) sizeof(path)) {
        LogMessage(X_ERROR, "XKB: builtin keymap path too long\n");
        return NULL;
    }

    out = fopen(path, "wb");
    if (!out) {
        LogMessage(X_ERROR, "XKB: could not open %s for builtin keymap\n", path);
        return NULL;
    }
    written = fwrite(phoenix_builtin_xkm, 1, phoenix_builtin_xkm_len, out);
    fclose(out);
    if (written != phoenix_builtin_xkm_len) {
        LogMessage(X_ERROR, "XKB: short write staging builtin keymap to %s\n", path);
        return NULL;
    }
    LogMessage(X_INFO, "XKB: using compiled-in keymap (no xkbcomp): %s\n", path);
...
#ifdef __phoenix__
    /*
     * Phoenix-RTOS has no xkbcomp and no xkeyboard-config. The stock RMLVO path
     * (XkbCompileKeymapForDevice -> XkbRMLVOtoKcCGST -> XkbDDXNamesFromRules)
     * fopen()s the rules file FIRST and fails before any compile, and the
     * KeymapOrDefaults fallback forks xkbcomp — both abort the server. Short-
     * circuit here with a keymap precompiled on the host (.xkm embedded via
     * builtin_keymap.h): stage its bytes to the path LoadXKM() reads, then load
     * it directly. No rules lookup, no fork. If anything goes wrong we fall
     * through to the stock path (which will then fail as before, unchanged).
     */
    {
        char *mapName = PhoenixWriteBuiltinKeymap();

        xkb = NULL;
        if (mapName) {
            LoadXKM(XkmAllIndicesMask, need, mapName, &xkb);
            free(mapName);
        }
        if (xkb)
            return xkb;
        LogMessage(X_ERROR, "XKB: builtin keymap load failed; "
                            "falling back to stock (xkbcomp) path\n");
    }
#endif
