...
def build_mesh_vs():
    """#67 Stage-4: a VS that reads a vec2 POSITION vertex attribute (location 0) instead of
    deriving position from gl_VertexIndex — so the harness can draw a vertex buffer of many
    DISTINCT triangles across many DISTINCT tiles (the one axis the earlier stages never
    exercised). Color = vec3(x, y, 0.5) so the band signal covers varied geometry. No
    arithmetic ops beyond CompositeConstruct — kept as robust as the vertexless VS."""
    m = Mod()
    ...
    main_fn = m.id()
    gl_Position = m.id()   # output vec4 (BuiltIn Position)
    inPos = m.id()         # input vec2 (Location 0) — vertex attribute
    vColor_out = m.id()    # output vec3 (Location 0)
    m.emit(OpEntryPoint, EM_VERTEX, main_fn, "main", gl_Position, inPos, vColor_out)
    m.emit(OpName, main_fn, "main")
    m.emit(OpDecorate, gl_Position, DEC_BUILTIN, BI_POSITION)
    m.emit(OpDecorate, inPos, DEC_LOCATION, 0)
    m.emit(OpDecorate, vColor_out, DEC_LOCATION, 0)
    ...
    p_out_v4 = m.id(); m.emit(OpTypePointer, p_out_v4, SC_OUTPUT, tv4)
    p_out_v3 = m.id(); m.emit(OpTypePointer, p_out_v3, SC_OUTPUT, tv3)
    p_in_v2 = m.id(); m.emit(OpTypePointer, p_in_v2, SC_INPUT, tv2)

    m.emit(OpVariable, p_out_v4, gl_Position, SC_OUTPUT)
    m.emit(OpVariable, p_in_v2, inPos, SC_INPUT)
    m.emit(OpVariable, p_out_v3, vColor_out, SC_OUTPUT)
    ...
    m.emit(OpFunction, tvoid, main_fn, 0, tfn)
    m.emit(OpLabel, m.id())
    pos = m.id(); m.emit(OpLoad, tv2, pos, inPos)
    px = m.id(); m.emit(OpCompositeExtract, tfloat, px, pos, 0)
    py = m.id(); m.emit(OpCompositeExtract, tfloat, py, pos, 1)
    posv4 = m.id(); m.emit(OpCompositeConstruct, tv4, posv4, px, py, f0, f1)
    m.emit(OpStore, gl_Position, posv4)
    col = m.id(); m.emit(OpCompositeConstruct, tv3, col, px, py, fp5)
    m.emit(OpStore, vColor_out, col)
    m.emit(OpReturn)
    m.emit(OpFunctionEnd)
    return m.blob()
...
def main():
    ...
    open(os.path.join(outdir, "mesh_vert.spv"), "wb").write(mvs)
    hdr = ("/* GENERATED by gen-triangle-spirv.py -- do not edit. Vertexless solid-color\n"
           ...
           " * the interpolated color. mesh_vert_spirv reads a vec2 position attribute\n"
           " * (location 0) for #67 Stage-4 varied-geometry (vertex buffer). No vertex\n"
           " * input state for the vertexless VS. */\n"
           "#ifndef TRIANGLE_SPIRV_H\n#define TRIANGLE_SPIRV_H\n#include <stdint.h>\n\n"
           + c_array("triangle_vert_spirv", vs) + "\n\n"
           + c_array("triangle_frag_spirv", fs) + "\n\n"
           + c_array("mesh_vert_spirv", mvs) + "\n\n#endif\n")
    open(os.path.join(here, "triangle_spirv.h"), "w").write(hdr)
    print("wrote tri_vert.spv (%d bytes), tri_frag.spv (%d bytes), mesh_vert.spv (%d bytes), triangle_spirv.h"
          % (len(vs), len(fs), len(mvs)))
