#!/bin/bash
#
# gpu-mlp-daemon.sh - run ON THE Pi (via `bash /gpu-mlp-daemon.sh` at the psh
...
#   rpi4-v3d, mlp-gpu-daemon, gpu-mlp-daemon.sh
#
# Copyright 2026 Phoenix Systems  %LICENSE%

echo "gpu-mlp-daemon: starting v3d-server (GPU owner) in background"
/rpi4-v3d &
srv_pid=$!
...
while [ ! -e /dev/v3d-srv ]; do
	i=$((i + 1))
	if [ "$i" -gt 20 ]; then
		echo "gpu-mlp-daemon: /dev/v3d-srv not visible after ${i}s - running client anyway (it retries)"
		break
	fi
	sleep 1
done
if [ -e /dev/v3d-srv ]; then
	echo "gpu-mlp-daemon: /dev/v3d-srv is up (server owns the GPU)"
fi

echo "gpu-mlp-daemon: running mlp-gpu-daemon (client of /dev/v3d-srv)"
/mlp-gpu-daemon
echo "gpu-mlp-daemon: client done"

# Best-effort cleanup (ignored if kill/\$! unsupported).
kill "$srv_pid" 2>/dev/null
echo "GPU-MLP-DAEMON-DONE"
