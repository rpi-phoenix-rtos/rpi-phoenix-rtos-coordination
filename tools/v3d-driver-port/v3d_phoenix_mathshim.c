...
/* Phoenix libm lacks hypot (double) too — compute it. */
static double hypot(double a, double b) { return sqrt(a * a + b * b); }
...
float roundf(float x) { return (float)(x < 0.0f ? (long long)(x - 0.5f) : (long long)(x + 0.5f)); }
